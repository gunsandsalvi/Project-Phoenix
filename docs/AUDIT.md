...
`docs/BUGS.md`, `docs/SWEEP.md`, `docs/VERIFY.md` and `docs/plan/14-polity.md` were merged in and
deleted. This file supersedes them and sits ahead of `docs/WORKLIST.md` rows 13k–17, which it
...
**This model has exactly one rich type system, and it is the instrument/settlement layer.** Kinds
with profiles, legs, causes, lots with basis, units, tick grids, a register indexed both ways, one
writer per fact. It is genuinely excellent, and it is why money is conserved on the wire.

**Above it there is no type system at all.** Every time a new economic category was needed it was
expressed as a *configuration of an existing instrument-layer primitive* rather than as a new
primitive:

| the real distinction | what it was encoded as | what broke |
|---|---|---|
| wage / interest / dividend / gain / return of capital / borrowing | `Instruction.cause`, nine settlement labels | tax on gross proceeds and on returned principal |
| what a claim is worth | `outlook('price.<id>')` — the price's own history | equity has no fundamental buyer; funds structurally excluded |
| service vs manufacture | `portable: false` + `spoilage: 1` | services carry and write off inventory |
| who owns vs who decides | one `fund` party | no separate account, no leverage, the manager decides nothing |
| something owed | nothing | unpaid tax, wages, severance and losses evaporate |
| who controls whom | a large holding | a takeover buys shares and absorbs nothing |
| what is public | one module's private bag | nobody can read a company's accounts |
| alive or dead | a boolean | a bank in resolution is `alive: true` |

The proof is the kernel's own scratch door. It offers modules a typed store for the one noun it
knows and an untyped bag for everything else —

```ts
// world/context.ts:461
state<T extends object>(name: string, initial: () => T): T;
```

— and its docstring enumerates the missing primitives without noticing: *"a register of employment
rows, a book of invoices, a party's outlooks."* **Seventeen modules use that bag, in nineteen slots.**

**Why it went unnoticed for so long** is a second, independent fault, and it is item 1 below: this
world records every refusal — `market.noView`, `noDemand`, `noSupply` — and **nothing reads them.**
Grep `audit/` and `observer/`: zero hits. Every one of these findings was in the journal, in plain
language, every period.
...
## What already works, and is not to be touched
...
One item at a time, in the order below (Law 10). Each item states **why**, **what it is**, **what it
deletes**, the findings it closes, and its **exit**. An item is done when its exit holds, its
findings are struck, `docs/RECORD.md` has its entry and `docs/COVERAGE.md` is re-marked — one commit
(Law 14). When an item closes, delete its section from this file; the record says what it closed.

A finding under an item is **evidence for the change**, not a task of its own. Do not chase one.
...
is trustworthy without it. Stage B (**3–15**) is the economic type layer, one noun at a time. **16**
is the dimension sweep, which is large, mechanical and compiler-driven. **17** is loose repairs. **18**
is the existing worklist, re-pointed.
...
and stored (`prices/price-store.ts:36`, `world/world.ts:1921`). Grep `audit/` and `observer/` for any
of them: **zero hits.** Nothing consumes the record of silence. Meanwhile **224 bare `return [];`
across 47 mechanism files** make "I considered it and declined" and "this code has never executed"
the same value — `banks/dealing.ts:258` has seven such guards in a row, each individually correct,
and together they mean no bank quotes in any market in this world.

**What it is.** One audit family. For every declared capability — participant, venue participant,
derivative class, instrument kind, market, party kind, audit family, and after **0** every declared
noun — periods since it last produced an outcome. Never-produced is a finding with an owner, size and
citation like any other. Unbuilt reports "not built", never green: the rule that already governs
families, extended to mechanisms. And a participant returns `{posted}` or `{declined: reason}`, so
`[]` stops being expressible.

**What it deletes.** The need to read sixty thousand lines to discover that a sector is dead.

**Exit.** The family reports every period. Every capability it names as never-reached is fixed here
or carried, by name, into the item below that owns it. `docs/COVERAGE.md`'s `MET` derives from this
...
**What it is.** A lint rule in `tools/eslint-rules` (which has exactly one file today): no `?? 0`, no
`|| 0`, no numeric default, no optional-means-unset number anywhere in the engine. `Missing` at every
site that has one.

**Why it is this early.** A `Measure<D>` that can default to zero defeats **C1**, and a reach family
that reads a defaulted zero as an outcome defeats **1**.

**Exit.** The rule is in the lint; the five sites below are `Missing`.


### Findings this closes (5)
...
bag** — and a module may not import another module. **So nothing in this world can read a company's
accounts.** The household's own docstring says it out loud without noticing: *"a saver reading the
tape is not a saver reading accounts."* There are no accounts to read.

**What it is.** A kernel store. What a party published, as of when, with what lag, and what it said.
Reporting is its one writer; anybody may read it, subject to Observer A1–A4 — public is public, and
private state stays private.

**Why it is first among the nouns.** It blocks **4**, and **4** is what the equity question was
actually about.

**Exit.** `reporting` keeps no private state; a participant in another module reads a firm's last
published accounts and their date, and cannot read anything unpublished.
...
legally and how does it settle*. **Not one answers why anyone would hold it.** `cashFlows()` is the
issuer's contractual promise, so this model has exactly one theory of value: discount the promise.
A share has no promise. Everything follows from that single fact:

- `funds.eligible()` (`funds/index.ts:1120`) calls `cashFlows` and returns false when there are none,
  so **no fund in this world can ever hold a share** and every fund is a bond fund at the type level;
- a household therefore values a share at `outlook('price.<id>').expected − confidence`
  (`households/portfolio.ts:133`) — an adaptive extrapolation of that share's own price history;
- no bank quotes one (**A-60**);
- and the one earnings-based valuation in the codebase — `worthAt`, `control/index.ts:80`, *"what it
  expects to get out of it against what its own money costs it"*, the same question asked of a
  machine — sits inside a module that has never produced anything.

**The equity market is a closed loop of price-extrapolators with no fundamental side.** That is why
the price collapsed in the year-long run and why item 12c patched an anchor at the seed. The anchor
is a buyer, not a seed price.

**What it is.** Two behaviours on the profile that already exists. `expectedStream(holder, instrument)`
— what THIS holder expects to receive: for a bond, the promise haircut by its own view of the borrower
(**5**); for a share, what it expects the firm to distribute, read from published accounts (**3**).
And `liquidity` — how fast the claim becomes money, absent entirely (`grep liquid` in
`registry/kinds.ts`: zero) and half of every funding decision. Then one generic read,
`worth(stream, requiredReturn)`: the profile answers the stream, the party brings the return. No
formula in the kernel; Law 3 intact, because the required return is a preference and the output is a
bid, not a print.

**What it deletes.** `funds.eligible`'s maturity test standing in for "can I value this", and 12c's
seeded anchor, which names the buyer that replaces it.

**Exit.** A fund bids for a share on its own number; a household still extrapolates, and the two
disagree structurally and permanently, which is §46 A3 doing the work it was built for.
...
in the entire tax is `r.instruction.cause === 'coupon'`, and `Cause` is a nine-value **settlement**
enum (`trade | production | issuance | coupon | maturity | corporateAction | default | transfer |
seed`) — the wire's label for why bytes moved, pressed into service as a tax taxonomy. So a household
is income-taxed at the wage rate on share-sale proceeds (gross, not the gain), on a maturing bill's
**principal**, on a fund redemption, on a probate distribution, and on a loan drawdown. **Borrowing is
income here.**

The base for a real gain is already in the register and unread: lots carry `basisPerUnit` and a debit
reports the basis of the units it drew (`register/register.ts:39,64`). The word "gain" appears
eighteen times in the engine and **every one is a revaluation mark**. There is no disposal in this
model, only a money leg.

**What it is.** A discriminated union with a profile per kind — `wage | rent | interest | dividend |
disposal{proceeds, basis} | returnOfCapital | borrowing | transfer` — registered by the modules that
create them and **stated by the payer**, which knows: the wage phase knows it is paying a wage. The
kernel refuses a money leg into a party that names no receipt kind, the same way it already refuses a
one-sided leg. The treasury sums bases and never walks legs; `disposal.taxBase = proceeds − basis` by
construction, dispatched with `assertNever`.

**What it deletes.** The `cause === 'coupon'` special case and the ledger walk behind it.

**Exit.** The tax is a dispatch over the union. A `returnOfCapital` cannot be taxed as income because
its base is zero by construction. The consumption base is non-zero (**A-63**).


### Findings this closes (6)
...
extrapolation of an observable. **There is no noun for what one party thinks of another party** —
which is what a PD, a credit decision, a rating opinion, a dealer's adverse-selection charge, a
depositor's confidence and an acquirer's view of a target all are. Appendix B requires one PD model
per borrower; there is nowhere for one to live, so there is none, and there are 96 loans across 9,006
firms. Three of the nineteen private stores are this noun trying to exist: `outlooks`, `ratings`,
`research`.

**What it is.** A typed view held by a party ABOUT a party, formed adaptively from that holder's own
history (§46, XI-16), carrying its own dispersion and surprise. Disagreement about a counterparty,
not only about a tape.

**Exit.** A bank's credit decision reads its own view; two banks disagree about one borrower; ratings
and research write views instead of private books; XI-13's second opinion has two opinions to be
made of.
...
```ts
PartyStatus      = { alive: true } | { alive: false; ceasedIn; successor }
InstrumentStatus = { live: true; performing: boolean } | { live: false; ceasedIn }
```

Binary, plus one boolean. No distressed, no restructuring, no resolution, no administration, no
forbearance, no grace period, no wound-up-but-not-distributed. **A bank in resolution is `alive: true`
in this type.** That is why `estates` invents `Winding` in a private bag, and why XI-1 ("a loss is an
event"), §25 (capital and resolution) and XI-8 (the estate) each built half a lifecycle of their own.

**What it is.** A declared state machine per noun: the states, the transitions, and who may make each.

**Exit.** Resolution, forbearance and winding are states rather than module bags; three of XI-15's
five weight events fire (**A-17**); the treasury's immortality is conditional where the kernel says it
is (**A-36**).


### Findings this closes (2)
...
facility are one noun: two named parties, dated terms, a state. Seven modules independently invented
it — `EmploymentRow`/`EmploymentBook`, `Lease`/`LeaseBook`, invoices, `StockLoan[]`, loans, covenants,
deals — each with a different shape, none visible to the kernel.

What its absence costs: **nothing ranks in an estate.** An unpaid severance leaves no obligation
anywhere and the record says nothing is owed (**A-41**); a failed probate transfer becomes an
unhandled throw because the unpaid part has nowhere to live (**A-20**); a central bank's loss is
forgotten at the next remittance (**A-62**); a failed levy evaporates (**D-1**). No audit family can
reach any of them, because there is nothing to iterate. And an insurance policy is modelled as an
*instrument kind* — a tradeable security — which is why the insurers module has no seed, no phase and
no participant: nobody knows how to WRITE one, because writing an agreement is not something the
kernel does.

**What it is.** A kernel store. An agreement has two parties, terms, a schedule, a state
(formed → performing → breached → terminated) and a rank if it is owed.

- **`Obligation` is a state of an agreement, not its own noun.** What is owed and unpaid is an
  agreement in breach, and it ranks where the law says.
- **`Mandate` is a kind of agreement** — a decider, a pool, and a rule profile (`mayHold`, `requires`,
  `charges`, `leverage`). This is what splits the fund into the three things it actually is: a **pool**
  that holds and has no opinions, a **mandate** that rules, and a **manager** that decides. A separate
  account is then a mandate whose pool is the client's own balance sheet — assets never on the
  manager's books. An ETF and an MMF are pools with different redemption rules. A hedge fund is a
  mandate with leverage, and `borrows` moves off `fundKind`, where it is hard-coded `false` — which is
  what 13h claimed to have built hedge funds on.

**What it deletes.** Seven private books, and `fundManagerKind`'s existence as a party whose only
behaviour is answering where it banks. The management fee stops being the placeholder that admits
*"no manager competes for the mandate"*, because two managers can now bid for one.

**Exit.** A policy is written and an insurer exists (**B-2**); a stock loan is an agreement and
securities lending is reachable (**A-67**, **B-3**); arrears rank in an estate (**D-1**); a manager
decides for a pool it does not own.


### Findings this closes (9)
...
more. So a takeover buys the shares and nothing happens — `combine` is called by nobody, because
there is nothing for it to write to. No consolidation, so a group's accounts are the parent's; no
ring-fencing; a resolution cannot transfer a subsidiary; and **private equity is definitionally about
control and is therefore inexpressible.**

**What it is.** A relation in the register beside ownership and encumbrance: who controls whom, from
when, on what basis. Ownership and control are different facts and 51% of the votes is not 51% of the
economics.

**Exit.** A takeover changes who controls the target and the target's board answers to it; a group
consolidates; §29 becomes buildable.
...
13k is filed as periodicity. **It is not.** `Periodicity` is a proper discriminated union in
`core/rate.ts` and `reporting/fiscal.ts` is fully built and unused. The reason 162,615 dividend
payouts are 65% of everything period 5 does is that **there is no declaration to be separate from the
payment.**

**What it is.** A noun with an announcement date, a record date, an ex-date and a payment date. A
declared dividend is a liability between declaration and payment and the share trades ex, which is
why total return and price return are different numbers. Kinds: dividend, split, rights issue,
buyback, spin-off, merger.

**Exit.** A dividend is declared on a fiscal calendar and paid later; the ex-date moves the price by
it; 13k closes or is re-scoped to whatever is genuinely periodicity (the rating fee, the tax
assessment).
...
manufacture from a service."* The economics are then simulated with `spoilagePerPeriod: 1`, so a
service line produces speculatively into stock, carries it as an asset for the period, and `perish()`
destroys the remainder with a `kind: 'destroy'` leg (`goods/inventory.ts:134`). **A school holds
unsold teaching hours as inventory and writes them off.** The file's own comment describes the right
economics — *"an hour nobody bought is gone, and the wages were paid anyway… that is what gives a
service business its operating leverage"* — and the mechanism reaches the number by an unrelated
route, which puts it in the wrong line of the accounts and makes the output decision a demand
forecast rather than capacity posted against demand.

**What it is.** The line declares `output: 'stock' | 'capacity'`. A capacity line posts availability
into the session and produces only what clears: no WIP, no inventory, no destroy leg, and the cost of
unsold capacity lands as operating leverage on fixed cost, where the comment already says it belongs.
`portable` goes back to being a freight fact.

**Exit.** No service line holds inventory; a service firm's output is demand-determined up to
capacity; A-64's storage rent buys capacity that exists.
...
resource yield, which proves the mechanism is understood; it has nowhere to attach for **built**
space, so a tile carries its thousandth building at the price of its first.

**Exit.** Building consumes land and gets dearer; a port has an owner, a berth and congestion;
commercial property, leases with a term and CRE lending become buildable; a retail firm sells from
somewhere.
...
party kind with nothing to guarantee with), a clearing house interposing, a sovereign backstop, a
letter of credit — none is expressible. Unlike the rest this is **tripartite**, so **8** does not
absorb it.

**Exit.** Deposit insurance is a guarantee rather than a party with no mechanism; LOLR's four
classical conditions have something to attach to; a guaranteed claim ranks differently from an
unguaranteed one.
...
issue, a resolution and a restructuring are the same shape, and each would invent its own.

**Exit.** An estate is a declared process with steps and a rule at each; A-21's arbitrary heir becomes
a step with a stated basis rather than whichever cell the iteration reached first.
...
what a party is FOR.** A firm maximises nothing; a bank has no franchise to protect; a manager has no
career. Every participant's reason is hard-coded inside its own module's `orders()`: **21 participant
declarations, 21 private answers to "why does this party do anything"** — none declared, none
comparable, none checkable.

**Why last.** It needs the others in place to be expressible at all, and Law 2 constrains it hard: the
objective is a declared PREFERENCE per kind, never a utility function the engine optimises and never a
representative agent. What it buys is that two kinds with different objectives behave differently
under one shock, visibly and by declaration.

**Exit.** A party's reason is declared on its kind and read by its participant; the difference shows
under a shock.
...
**950 call sites** (842 in mechanisms, 108 in the kernel). Each passes two bare numbers and a
human-readable string, and the string is the only place the dimension lives. Nothing validates it.
`mul(money, money, 'the premium')` compiles — that is **A-65**. `add(usd, eur, 'wealth')` compiles —
**A-23**. `mul(perMember, headcount, …)` on one side of a flow where the other side used
`perMember × weight` compiles — that is **A-39**, the largest conservation break in the model.
**594 fields are typed as bare `number`.**

`core/num.ts:236` already says what the fix is, in the codebase's own words: *"A caller mixing a `Qty`
with a plain number is refused by the compiler, which is the question the brand exists to ask."*
`Brand<T,B>` exists at `core/ids.ts:8`. It is applied to identifiers and to `Qty`, and to nothing else.

**What it is.** `Measure<D>` — a phantom-typed number whose `D` carries what it counts, its currency or
unit, its periodicity, and per-member-or-total.

- `add` / `sub` take two `Measure<D>` of the same `D`. Two currencies stop adding.
- `mul` composes dimensions and `div` decomposes them: `Money/Qty = Price`, `Money/Money = Ratio`,
  and money² has no inhabitant that any leg, price or account accepts.
- `Ratio` can never be an `amount`, so a spread cannot be used as a level.
- `PerMember<D>` and `Total<D>` are different types and `weightOf` is the only conversion.
- `D` carries a period, so last period's number cannot be read as this period's.

This is Law 8 — *periodicity, price level and unit are part of the number* — **enforced** rather than
requested.

**Not a rewrite.** The dimension is already declared at all 950 sites; it moves from the third
argument into a type argument, and the compiler generates the migration list by erroring at every
site that was lying. Kernel first, then module by module, each independently completable. Gate every
stage on the instruction digest the Law 18 items already use: mechanisms, economics and boundaries do
not change.

**Exit.** `what: string` is gone from the arithmetic; each of the eighteen findings below is a build
...
| **14** The polity | Carries **D-1** (arrears → **8**) and **D-2**. Its plan is below, unchanged. |
| **15** The recipe | Unchanged; last, deliberately. |
| **16** Measure | Carries **C-1**'s 82 red tests. Most are the rig naming the world; the reach family (**1**) changes what a green suite means before this runs. |
| **17** The app and the APK | Unchanged. |


### Findings this closes (4)
...
- [ ] Worklist row 14 → done; delete this section from `docs/AUDIT.md`; commit and push
...
## Index — every finding and where it landed

| finding | item | |
|---|---|---|
| **A-1** (B) | 16 | `reseat` books an issuer's whole liability against a PER-MEMBER equity account |
| **A-2** (B) | 17 | the merge guard compares liens by COUNT, and its own docstring says it compares them |
| **A-3** (B) | 17 | the merge writes the weight before the guard runs |
| **A-4** (B) | 1 | a claimed assembly guard on `exposedTo` does not exist |
| **A-5** (C) | 16 | `payable`, `cashFor` and `deliverable` take a unit and ignore it |
| **A-6** (C) | 16 | `labourParam` is declared in minutes and holds hours |
| **A-7** (C) | 17 | the seed reads a capital kind's life off the declaration, not the register |
| **A-8** (C) | 17 | a doc block describing a declaration that is gone |
| **A-9** (A) | 8 | the whole insurer sector cannot open a single policy |
| **A-10** (A) | 1 | the currency audit family compares a number against itself |
| **A-11** (C) | 1 | `flows` switches Money D3 off for a whole cell for a whole period |
| **A-12** (C) | 1 | a comparison in `names` that can never be true |
| **A-13** (B) | 1 | the population identity is checked for households and for nothing else |
| **A-14** (A) | 1 | that same household check is an identity that cannot fail |
| **A-15** (C) | 17 | the register has five per-party stores and the cell doors know about three |
| **A-16** (C) | 17 | a reverse split throws halfway through and leaves the register restated for some holders and not others |
| **A-17** (A) | 7 | three of XI-15's five weight events never fire, and nobody in this world is ever born |
| **A-18** (A) | 16 | the fraction of a person is discarded every period, and the world fragments into frozen micro-cells |
| **A-19** (B) | 16 | probate takes in every currency and pays out one |
| **A-20** (A) | 8 | a recorded settlement failure in probate becomes an unhandled throw |
| **A-21** (B) | 14 | one arbitrary cell inherits everything in a region |
| **A-22** (C) | 17 | `lifecycle.ts`'s header says dying is not in this file, directly above the dying |
| **A-23** (C) | 16 | `wealthOf` and `atRisk` add currencies; the function beside them refuses to |
| **A-24** (C) | 17 | the household's own plan round-trips through `unknown` and drops what it cannot parse |
| **A-25** (C) | 2 | an unpriced physical leg is valued at zero inside an audit family |
| **A-26** (A) | 8 | a household redeems its whole shortfall from every fund it is in, and subscribes its whole cushion to every fund there is |
| **A-27** (A) | 4 | the cell counts a commodity fund as money it can spend today |
| **A-28** (A) | 17 | a seller's margin is on the buyer's side |
| **A-29** (B) | 17 | a bid is capped at the issued amount to stop an overflow |
| **A-30** (C) | 2 | three numeric defaults where the discipline is `Missing` |
| **A-31** (A) | 6 | a depositor's stickiness decays to zero, because the test is a sunk cost that grows without bound |
| **A-32** (C) | 16 | `levelsBelow` claims a refinement invariance its own arithmetic does not have |
| **A-33** (B) | 16 | `lastOwn` has no period bound, and four firm reads treat a stale wage bill as this period's |
| **A-34** (B) | 2 | a firm with no wage history bids for inputs as if labour were free |
| **A-35** (C) | 17 | `buying` is computed twice, two ways |
| **A-36** (C) | 7 | the treasury's immortality is unconditional where the kernel says it is conditional |
| **A-37** (A) | 5 | selling an asset counts as income, and the comment above the code says it must not |
| **A-38** (B) | 16 | a cell's outside option is every kind of money it received |
| **A-39** (A) | 16 | the wage bill capitalised into inventory is bigger than the wage that was paid, every period, for every row |
| **A-40** (B) | 5 | the labour print publishes a volume nobody was hired for |
| **A-41** (C) | 8 | `shed` says oldest first and does newest first; a trading employer that fails to pay severance records that nothing is owed |
| **A-42** (A) | 1 | two `units` families switch themselves off in any period with a weight event, which is every period |
| **A-43** (C) | 17 | the labour module builds the household's schedule, through the door the architecture says not to |
| **A-44** (B) | 16 | the household's liquidity premium is declared "over what a deposit returns" and is used as an absolute rate |
| **A-45** (C) | 2 | a bank that owes nothing has a cost of funds of zero |
| **A-46** (A) | 5 | income tax is levied on the return of a household's own capital |
| **A-47** (A) | 16 | a fund's mandate has no currency in it, so it bids its own money at a foreign price and its NAV adds two moneys |
| **A-48** (C) | 1 | `equityIsZero` cannot fail for the reason it says it checks |
| **A-49** (C) | 2 | `offeredYield` takes the last curve family, not the best, and answers zero when there is none |
| **A-50** (C) | 16 | `moneyOf` hand-builds an account id, which is the one thing `accountOf` exists to prevent |
| **A-51** (A) | 16 | the kernel converts currencies and five module reads of the same books do not |
| **A-52** (C) | 5 | an audit family recovers a fact by parsing the human-readable reason string |
| **A-53** (B) | 17 | a household bids its entire income as rent, and its spending plan does not know rent exists |
| **A-54** (A) | 1 | `gather` is called by exactly one module, so two other modules' venue schedules are never asked for |
| **A-55** (A) | 1 | nobody in this world can buy a dwelling, so the whole of Housing B1–C4 never runs |
| **A-56** (A) | 1 | three of this world's lines have a firm, a recipe and a market, and no buyer; they open at zero and never move |
| **A-57** (A) | 5 | a securitisation vehicle keeps the whole interest stream of its pool, for ever, and nobody owns it |
| **A-58** (A) | 16 | the price a bank will pay for a note is a leverage ratio squared, called a cost of funds |
| **A-59** (B) | 17 | the resolution auction is decided on a price that is never paid |
| **A-60** (A) | 1 | no bank makes a market in anything, because no bank employs anybody |
| **A-61** (A) | 16 | every central bank remits its income to the same treasury, in its own money |
| **A-62** (B) | 8 | a central bank's loss is forgotten at the next remittance, and the comment says it is carried |
| **A-63** (A) | 5 | every household buys its groceries on thirty-day credit; the consumption-tax base is therefore always zero, and the households audit family fires on every purchase |
| **A-64** (A) | 11 | a firm pays rent for storage space and receives nothing for it |
| **A-65** (A) | 16 | every option premium is a money-squared number, and it does not depend on the strike |
| **A-66** (A) | 1 | eight of the nine derivative books can never produce a first print |
| **A-67** (A) | 8 | nothing ever borrows a security: the whole securities-lending mechanism is unreachable |
| **A-68** (A) | 16 | loading a cargo writes off what the cargo cost |
| **A-69** (C) | 1 | nine exported entry points and reads that nothing calls |
| **A-70** (A) | 9 | a takeover buys the shares and never absorbs the company |
| **B-1** (A) | 1 | `corporate.bond` is a kind, an id function and a covenant test, and nothing ever issues one |
| **B-2** (A) | 8 | the insurance sector has no seed, no phase and no participant |
| **B-3** (A) | 8 | securities lending is claimed to clear a fee, and has no way in |
| **B-4** (A) | 9 | the market for control buys shares and never combines |
| **B-5** (A) | 1 | the tenancy venue has never had an order in it |
| **B-6** (A) | 1 | item 9's dealers quote nothing |
| **B-7** (A) | 1 | the derivative layer and its nine classes have never produced a contract |
| **B-8** (A) | 1 | the securitisation waterfall never allocates a loss |
| **B-9** (C) | 17 | `docs/BUGS.md` contradicts its own header about the four countries |
| **B-10** (A) | 1 | "checks green" has been satisfied by families that cannot fail |
...
| **C-3** (—) | 18 | four central banks administer one policy rate (carried from `SWEEP.md`; worklist **13l**) |
| **C-4** (—) | 18 | the sectors that are not there (carried from `SWEEP.md`) |
| **C-5** (—) | 1 | 12,519 declared parameters are never read (carried from `VERIFY.md`) |
| **C-6** (—) | 1 | the margin gate: 31,640 admission decisions, 31,640 refusals (carried from `VERIFY.md`) |
...
thing claimed is in the code and produces anything. 15 findings. **Part III** carried in three
deleted holding pens, de-duplicated. 7 findings. **Part IV** was item 14's own plan. 2 findings.
...
**The ontology enumeration** that produced items 0 and 3–15 came later and by a different method:
each of the five axes of an ontology — entities, relations, events, states, properties — enumerated
mechanically from the structures the code declares. Entities from the 19 `ctx.state` slots; relations
from what the register expresses (two, plus three fields on `Party`); events from the 9 wire causes
and 23 free-text journal kinds; states from `PartyStatus` and `InstrumentStatus`; properties from the
10 fields on the instrument profile and 6 on the party profile. The last two axes yielded one noun
each and the two before them came back confirmatory. **Four of the nouns above close no finding at
all** — 3, 12, 13 and 15 — because a primitive nobody attempted leaves nothing for a read to find.
That is the argument for item 0.
