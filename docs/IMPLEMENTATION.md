...
- [x] 14.1 Capital: the seed floats each insurer (a share line subscribed by household cells at cost) so `equity() > 0`. **And (12b.5):** a fund manager bids for analysts what an hour is worth to it — its whole expected earnings over the hours it needs — and, as the only bidder in the venue, pays that (Labour D1: the marginal bid is the print): `manager.bank.c` in the funds scale model hires four at 43.7 an hour, pays 6,116 out of 5,245, and is gone the period after (`funds.test.ts:227`). A desk is worth what it lets the manager keep, not everything the pools pay it. **Done:** the foundation seed creates one insurer per place that has a bank and endows its opening surplus in cash — a SHAPE per head of the region's households (`seed.insurer.openingSurplusPerHead`, death at 14.8/22a) — before the equity seed runs, and the equity seed floats its line like any company's: shares worth its book at the opening price, held by the savers at cost, every piece with a holder (`insurer-float.test.ts`: cash, equity positive, holders all household cells, Σ held = issued, what they subscribed ≤ its cash by less than a piece a member, alive after twelve periods). The insurers module's own seed leaves a party the foundation made alone, and a scale model without the module gets none. The manager bids for analysts its expected fee INCOME over the hours, not its earnings (the marks on what it holds). **Found and fixed on the way — every one of them a party that had never lived to reach the code:** (a) the buyout fund's commitments were amounts drawn blind (40–140m) against an investor that opened with nothing, so every insurer defaulted on the first call in period 1 and died; a commitment is a SHARE of what the investor holds when the promise is opened (`funds/commitment.ts openCommitments`, `PRIVATE_EQUITY_SPREAD.committed`); (b) `funds.strike` read its own `fund.performanceFee` and `insurers.allocate` read `fund.struck` without declaring either (Clearing F1.a throws); (c) a pool passed on its WHOLE account as a payout — a subscriber's eighty million came straight back as a "dividend", leaving shares on a book worth nothing, and the same money went round the trackers at a NAV falling a decade a period until a household divided by nothing (`seed-B`, period 19); a pool passes on the dividends and interest that reached it this period, read off the legs, and no more than it has (`funds/index.ts distribute`); (d) a named party's cease reseated its agreements and not the paper it had issued, so the arrear a fund's last fee made was redeemed against a party that had ceased (`world.ts cease`, Register F2: live issued instruments reseat to the successor, as a cell's do); (e) a bank booked a drawing to the party that drew it and not to the party it is now (`banks/index.ts bookDraws` resolves; `ctrl-pe`, period 23). **Measured:** all gates green; eighteen suites at 90 red of 196, as before — two greens gained (the treasury collects the tax on interest; `expectations.test.ts` B3) and two lost: `equity-anchor.test.ts`'s two measurements have nothing to measure because the one listed firm of the (4, 40) rig, which reported in period 29 before, now fails in period 21 unable to pay 44.8m that fell due — the phantom income (c) removed from every saver's account is what was holding it up; positioned at 22.3.
- [x] 14.2 The buyer: a firm standing in a physical fact bids for cover at its own outlook of the loss (the environment condition against its plant's `standsWind`); a household cell bids for life cover from the mortality its cohort observes. Bids are schedules (`rungsUpTo`), not points. **And (12d.3):** the condition is read through `registry/environment.ts conditionsStanding` from the buyer's own view, the door the household basket reads its warmth through; nothing here parses the event. **Done:** two subjects join the outlook vocabulary and every party observes them off the public record (`condition.<fact>.<region>`: the weather of the place it stands in, every period; `mortality.<cohort>`: the share of its cohort that died, for a cell) — so a firm's expectation of the wind and a cell's of its own mortality are §46 outlooks like any other, adaptive at their own memory and anchored on the public level. A firm with plant bids for cover on what its plant is on its books at, at the share of it the wind it expects would take over the term — the one survival relation the weather scraps plant by, now in the registry and read by both (`registry/physical.ts survivesWind`, Law 4) — as a ladder down from that price out of the cash it holds (`firms/index.ts insure`, `firms.insured`); a cell bids for life cover on what a member owes, at the chance a member dies within the term off its own mortality outlook, per member and over its members (`households/index.ts insureLife`). The names of cover are the registry's (`registry/insurance.ts`), the book is opened at the seed so buyers can post before the quote, and every session is on the record with its outcome and who was in it. **Found and fixed on the way:** `rungsUpTo` made a count of what the money would buy before comparing it with what was wanted, and at a premium of a few millionths a unit that count passed the grid (Law 8 throw, both worlds). **Not built here:** the insurer's quote — with no claims experience and no debt-funded capital it prices cover at nothing and writes none (`noSupply`, said every period), which is 14.4's return on the capital it holds. **Measured:** eight periods of the scale model — every living party holds an outlook on its region's wind and every cell one on its cohort's mortality; every firm with plant records what it has at risk, what it expects to lose (a share, never all) and its ladder; the bids are in the book every period and nobody writes them (`cover.test.ts`, three green). Nothing new is red: eighteen suites at 90 of 196.
- [x] 14.3 The claim: an environment period that destroys a covered party's plant or inventory settles the schedule's amount from the insurer; `insurer.claim` is that instruction's event. **Done:** the capital programme's `capital.weathered` now says what the lost units were on the holder's books at (`atCost`, drawn off the lots first-in-first-out as the register draws), the registry reads the period's losses (`registry/physical.ts weatheredIn`), and the insurers' `claims` phase — after the weather at the same anchor — pays each covered loser what it lost, up to the cover it holds, oldest cover first, and takes the cover used back in the same numbered instruction (an asset leg to the issuer is a redemption, so the cover is gone; `insurer.claim` is that instruction's event, paid or failed). A claim the insurer cannot pay fails on the wire and its arrear ranks as a policyholder's (`receipt: { of: 'claim' }`, a new receipt class, no income to anybody). One storm is one period's list of losses, and every line of it reaches the phase (B4). Inventory is not weathered by anything yet; when a mechanism destroys stock by weather its event joins the same read. **Measured:** cover written by hand to a carrier for a million and a storm written by hand that takes 137 million of its hull: the insurer pays the million, the policy's outstanding is nil, both legs are one instruction, and the claim is on the record (`claim.test.ts`). Nothing new is red: eighteen suites at 90 of 196.
- [x] 14.4 Experience: `claimsSeen` is the insurer's outlook on claims per unit of cover from its own history (never the last claim); the price of cover = experience + cost of equity (XI-4 read) × capital per unit; no ratio becomes a level. **Done:** a `claims` outlook subject — every party with cover outstanding observes each period what a unit of its cover cost it in claims, read off its own `claim` legs over the cover it has out, a period with no claim observed as one — and `quoteCover` prices a unit as that outlook over the term plus what its capital costs it per annum (`registry/capital.ts costOfCapital`: its equity where a market prices it, its debt at the margin, the sovereign curve when nothing else has said) over the term's fraction of a year on the surplus behind a unit (`capitalPerUnit`, a ratio entering as a level of the same thing); capacity is its surplus; an insurer that writes nothing says why in public (`insurer.unquoted`); `claimsSeen` (the last claim over the cover written) is deleted. **Measured:** in the one seed of forty whose insurer's line the draw listed (`quote-37`), cover clears in the world's own book from period 2 at a few hundredths a unit, the rig's own weather pays real claims in period 3, and after a hand-written storm the insurer's experience forms and it quotes higher (`experience.test.ts`, two green). **Finding (XI-4):** the listed insurer refused to quote in two of six periods for "a price of nothing" — `requiredOnEquity` reads a company's cost of equity as its earnings yield, so one with no earnings yet is told its equity costs nothing (positioned at 21.23). Nothing new is red: eighteen suites at 90 of 196.
- [x] 14.5 A policy is an AGREEMENT with the schedule on it (the `POLICY` instrument kind deleted); fills paired by the solver's allocation. **Done:** the kernel marks rows — an `AgreementKindDecl` may say what a live row of its kind is worth now (`valued`: a schedule at a curve, with the debtor's own outlook to read), and `revalue` moves the creditor's account up and the debtor's down by the change since the row's last mark, in the same pass, each in its own money, unwinding the mark when the row ends; nothing is stored but the last mark (`register/agreements.ts mark/markOf`, `world/revalue.ts revalueRows`). A policy is a row of the kind `insurers.policy` (`registry/insurance.ts POLICY_ROW, PolicyTerms {cover, to, discountedAt}`), worth the insurer's expected claims on it — its `claims` outlook on the cover for every period left of the term — at the curve (B2, B2.a); it owes nothing now and what a loss makes a claim for is paid off it (14.3), the cover restated down and the row ended when paid out or when its day passes (`expireCover`). Fills are paired by the solver's allocation: every buyer's fill is spread over the insurers that filled the other side in proportion to what each wrote, and each pair is one premium paid and one row opened in the same pass. The `POLICY` instrument kind, its profile, `coverTerms`, `policyIdFor`, the instrument `isPolicy` and the last-claim `claimsSeen` are gone; `longestPromise`, the `names` family and the `claims` observation read the rows. **Measured:** the four insurer suites green (fourteen); nothing new is red: eighteen suites at 90 of 196, the same set.
- [x] 14.6 Pensions: a second profile behind the dispatch table — a sponsor (an employer), contributions as two wage-linked legs in the labour pay phase, a schedule per retired cohort over its remaining life (12.3's mortality), the funding ratio as a read (assets at mark over promises at the curve), a shortfall as an arrear on the sponsor. **Done:** a `pension` party kind beside `insurance` behind the module's one dispatch table (`insurers/pensions.ts pensionKind`: fails on cash, not solvency — a fund short of its promises has a shortfall, not a lost licence), one fund per place made empty by the foundation. Membership is a lattice dimension of the household (`pension: member | none`, opened off the record like a hire): the worker cell of every employment row is enrolled whole the period its payroll first runs, and carries the key through every job and cohort after, so nobody the seed placed is promised what nobody paid in for (22a). Every employer with a payroll owes the fund of its place a SPONSORSHIP row (`pensions.enrol`, before `labour.pay`), and the wage instruction reads it: three legs — the wage net of the member's share, the member's share and the employer's share to the fund — one instruction, so a payroll the employer cannot meet pays nobody and contributes nothing (`labour/matching.ts payFrom`, `registry/insurance.ts contributionsOn`; two POLICY shares, parliament's). A member cell that reaches the last cohort is owed a PENSION row by the fund (`pensions.promise`, after `households.lifecycle`): so much per member per period — a replacement share of what a week of the trade most of the place works in earns NOW, read at every payment and never carried — for as long as its members live; the kernel marks it (14.5's `valued`) at the members alive times what one is expected to draw in each period to the end of the mortality table, decayed at the fund's own outlook of the cohort's mortality (§46: a fund that owes a cohort observes its deaths, `expectations exposed`), at the sovereign curve; `RowValuationReads` grew the weight, the going rate and the params a schedule reads. The pension is paid every period (`pension.paid`, a `pension` receipt: income to the cell, a wage's rank in arrears). The funding ratio is a read of the marks and the equity account, published every period (`pension.funded`), and when the fund's equity is negative it calls a recovery period's share (POLICY, ten years) from the sponsors whose payrolls fed it this period, each in proportion, on the wire; a call not paid is the kernel's arrear on the sponsor, class `contribution` (`callSponsors`, `pension.call`). `mortalityParam`'s spelling moved to the registry so the schedule can read where the table ends. **Measured** (`pensions.test.ts`, three green): a wage instruction in the rig has the two contribution legs to the fund of the payer's place, the payer sponsors it, the worker is a member, nothing reaches the fund but contributions and it holds what came in less what it put to work; a hand-written crossing of a member cell opens one row, the fund pays its pension every period at whole pieces a member, the kernel's mark is positive and is what `pension.funded` reports as the promises, equity is below nothing and the ratio below one, the sponsors are called for at most a recovery period's share and every call is money that moved or an arrear the sponsor owes; the schedule decays with mortality and a lower rate makes the same promise worth more (B2.a). In the rig's own draw the first member retires in period 15 and is paid from period 16, indexed to the public service. Two assertions brought to the payroll's new shape (`insurers.test.ts`: the scheme's rules are POLICY beside the module's one convention; `labour.test.ts`: the employer's share is part of what a firing costs through the notice); nothing new is red: eighteen suites at 90 of 196, the same set. **Findings:** (a) the sponsor's covenant is not on the sponsor's books — the deficit shows on the fund and on the members, and on the employer only as the calls arrive (positioned at 21.24); (b) the fund puts a period's contributions to work the period AFTER they arrive — the allocation is before the strike and the payroll after the markets — which is the lag a subscription has and not a finding (the first read of it said otherwise; the probe at 14.7 corrected it); (c) a promise indexed to a trade nobody in the place works in any more is worth nothing and pays nothing until somebody does, said in the row's mark and in no event (positioned at 21.25).
- [x] 14.7 `insurers/allocate.ts`: buffers from outlooks (`investable`), `meetCalls` via `lastOwnSince`; a fund door is live only while its fund is. **Done:** what an institution keeps back is three named reads of its own book and never the last thing that happened to it (`buffersOf`): the claims it EXPECTS a period — its `claims` outlook on the cover it has out — the pensions its rows say it pays next period (a schedule, no outlook needed), and the calls it expects — a new own-observation subject, `called` (§46 A2.a: an investor observes what the pools called of it each period, and once called a period with no call is observed as one, so the buffer decays as well as rises; `registry/funding.ts callsOn` is the read, `expectations exposed` the observer); `insurer.allocated` publishes the three beside what it put to work, so the whole account is on the record. `meetCalls` asks the registry for LAST PERIOD's calls with the period in the ask (`callsOn(journal, investor, p−1)`) rather than walking every call ever made and testing the period itself — every call of that period and not the last one, because two pools may call at once, which is why it is the period-bound registry read and not `lastOwnSince`. A door is live only while its fund is (`doors` skips a venue whose fund has ceased). And a fund's longest promise reads its pension rows — to the end of the mortality table from the age the cohort enters at — so its duration test is against the long promise C2.a says it holds (`longestPromise`). **Measured** (`institutional-allocation.test.ts`, three new green, twelve in all): every allocation in eight periods of the rig puts to work exactly its account less the three buffers, each a number ≥ 0, through the door of a living fund; a hand-written call the insurer cannot meet forms its `called` outlook (its own, decaying over the quiet periods after), it asks the pools it is in for what it missed the period after at no price, and what it keeps back for calls from then on is the outlook and never the call. The 14.6 note that the pension fund "puts almost nothing to work" was wrong and is corrected: it invests a period's contributions the period after they arrive. **Found and fixed on the way:** the money fund of a bank in `exp-f` winds up at a NAV of nothing in period 16 and a redemption at that price divided by it (a world stop); a share struck at nothing is owed nothing, said on the record (`funds/index.ts redeem`), and the pool's empty book is positioned at 21.26. Nothing new is red: eighteen suites at 90 of 196, the same set.
