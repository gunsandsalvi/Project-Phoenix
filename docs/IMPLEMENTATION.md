...
- [ ] 9.2b **The fee clears.** It is a `placeholder` per fund whose own `why` says *"no manager competes for the mandate, so the number stands where a competition should be"*. Two managers bidding for one mandate is what kills it, and it needs a draw that produces managers not tied 1:1 to a pool.
