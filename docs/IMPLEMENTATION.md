...
- [x] 12a.1 `register/arrears.ts`: instrument kind `ARREAR` (issuer = payer, holder = payee, `carriedAtCost`, no market; terms: the failed instruction id, the payment CLASS — wage, tax, transfer, rent, invoice, coupon, fee, service). Written by `ledger/settlement.ts` in the same pass as the fail (one writer). Ranks in the estate by class. **Done:** `register/arrears.ts` — kind `ARREAR`, terms (the failed instruction, the payment class off the leg's receipt, payer, payee), carried at cost, no market, ranked by a class ORDER (senior first; a fact about the law, data); `settlement.writeArrears` issues one per unfunded money leg of a refused payer in the same pass as the fail, by an issuance instruction with cause `default` (one writer, on the ledger); registered as a kernel kind beside money. **Measured:** the payer's estate pays what it has against the row and the rest stands, held by the payee or its own estate.
- [x] 12a.2 `world/failure.ts stillOwed` = `register.issuedBy(self)` filtered `ARREAR`; delete `failedPayments(0)` from it. **Done:** `stillOwed(view, ccy)` is the live arrears the party issued in that money, off the register; the walk over this period's failed instructions and `FROM_THE_BEGINNING` are gone (a payer that missed a wage last week and had the money this week was never short).
- [x] 12a.3 `runCorporateActions` presents every `ARREAR` of a payer before any new due of the same class; a paid arrear is redeemed. `trade-credit invoiceKind.due` presents the maturity every period from the due date until redeemed; `defaultOn` = the buyer missed it; `treasury.shortfall` (transfers), the unpaid wage, the unpaid estate transfer and the unpaid rent all write the row. **Done:** the arrear kind's `due` presents the whole of it at par every period; `runCorporateActions` walks arrears before the rest of the record, so a payer's money goes to what it missed before what is new; a paid arrear is redeemed by the kernel's own `redeem` and settlement writes no row on a row; the invoice is presented from its due date every period until it is paid. **The rest is settlement's:** the unpaid wage, the unpaid rent, the treasury's transfer and the estate's hand-over are all money legs a refused payer could not fund, and settlement writes the row for every one of them — the households module's `estateUndelivered` agreement for an unfunded hand-over was a second representation and is written no more (it stands for a hand-over that failed for another reason).
- [x] 12a.4 Delete the guard at `housing/index.ts:1558`; the mortgage is a loan row per (lender, cell) in totals; `households/profile.ts` subtracts service and rent before the basket (0f.7 already); the lender's limit reads the dwelling print × its haircut (17.0's view). **Finding (11.1):** `housing/index.ts foreclose` reads `t.borrower` off the mortgage and treats a borrower that is not alive as one that FAILED; a household cell that merged (its bank moved) is not alive and is succeeded, and its dwelling would be foreclosed on for having moved bank (Register F2: resolve the successor; an estate is the one successor that means failure). **Done:** the guard was `homeBid`'s `owedIn + cash <= 0` (the file had moved under it), read at a cell as one roof for all of its members ever — deleted; a cell short of roofs asks every period, on a schedule, secured on what the loan would buy; `foreclose` follows a `credit.default` recorded on the row (a registry read of the kernel's own event) and names the borrower as it is now — the 11.1 finding closed. The lender's haircut on the print is 17.0's credit view, as the step said. **Measured:** in twelve periods of the scale model household cells ask every period and are declined on `appetite` every time — no bank quotes a household name — so no mortgage is written and three cells stay short of a roof (from every cell every period): the ask is the households'; the answer is 17.0's.
- [x] 12a.5 `fails: ['cash']` on the household kind; failed members move to probate and to `credit record = defaulted`; consumer credit is the same row unsecured. **Done:** the household kind declares `fails: ['cash']` and `borrows`; the households module resolves its own failures (`resolves: [HOUSEHOLD]`): a living cell the kernel says cannot pay what it still owes (`failedWhy`) is re-keyed whole to `estate = probate, credit = defaulted` — the same standing cell death goes to — its holdings handed to the probate office, and the event recorded with the kernel's reason; consumer credit is `ctx.request` unsecured, `repays: 'atOption'`, for what the basket is short between its members. **Two stops fixed on the way:** `ceaseCell` now reseats what the ceasing cell issued onto its successor (a coupon addressed to a dead cell was Money E4's refusal; the four-country world stopped in period 7), and `handToProbate` leaves a claim the cell issued on itself where it is (an arrear whose payer and payee merged into one cell is nobody's to receive). **Finding:** with holdings as totals a cell fails as ONE — every member of a twenty-thousand-household cell goes to probate for a miss that was some of theirs; the per-member miss needs the per-member read (XI-15 `perMember`) and sits under this item until 12b's register gives a member an income of its own.
- [x] 12a.6 The treasury: `fails: ['cash']` conditional on currency (`failedWhy` reads the arrear's currency; it fails only in a money it does not issue); a domestic-money miss is `treasury.shortfall`, never `credit.default` (`credit-events` skips payers whose kind `sovereignIn` the currency); `treasury/index.ts` refuses to auction while an `ARREAR` of the treasury stands (Sovereign G5). Probate and the deposit insurer declare what they fail on or name the XI-3 exception in `why`; `securitisation` vehicles fail on the senior coupon's cash shortfall. **Done:** the treasury kind fails on cash; `failedWhy` walks every money the party has live arrears in and skips the one it is `sovereignIn` (kernel read: the kind borrows on the state's credit and the money is the one of where it sits); `credit-events` records no default for a sovereign payer in its own money; the treasury module resolves its own kind (G3: no estate), records `treasury.defaulted` with the kernel's reason and `treasury.excluded` naming the rows, and announces no auction while an arrear in a money it does not issue stands (G5 — a domestic miss is a shortfall and it keeps funding itself, G1). `PartyKindProfile.cannotFail` names the XI-3 exception for a kind that fails on nothing, and the registry refuses one that does not (central bank, probate, deposit insurer, estate name theirs). A vehicle's senior coupon it cannot fund has been an arrear since 12a.1, so `cash` already fires on it — the stale comment fixed. **Found on the way:** `banks/capital.ts riskWeightOf` read "cannot fail" off the kind's `fails` list, so the treasury's own paper would have weighed as an ordinary exposure the moment the kind could fail anywhere; it now reads `sovereignIn` for the paper's own money (Sovereign E5, G2: the same treasury's foreign-money paper weighs what any exposure does).
- [x] 12a.7 `short-term-debt drawBackstops` writes a loan row (the banks module's kind) at the line's rate with a maturity; `BackstopTerms.drawn` is a read of that row; the fee is charged on `limit − row.outstanding`; `grantBackstops` grants on the first `paper.offered` only. **Done:** a drawing is a `LOAN` row (the credit registry's kind) issued by the issuer and held by the committing bank — one row per line, drawn on again, at the line's `rate` (the bank's quote when it committed), maturing `shortTermDebt.lineMonths` out, repaid at the issuer's option; `drawnOn` reads the row's outstanding off the register and `BackstopTerms.drawn` is gone; the fee falls on `limit − drawnOn`; a line is granted to an issuer the period it first offers paper, at the rate its bank quoted it that period, and to nobody else. **Measured:** in the `coalRaw` scale model every paper issuer is in an estate before its paper matures (twenty periods), so the draw is never reached there and `backstop.test.ts` builds the line, the paper and the run out of the doors; the equity read of the period-nine issuers is exactly 0 at `paper.issue` — they are the ones that die by period twenty-one (a finding for 17.0, where the credit view sizes the line).
- [x] 12a.8 A buyer of second-hand plant: `registry/capital.ts` bids in a vintage's market when the print per unit of remaining life is below new-build cost at its hurdle; the estate's ask is two states (last print while it has time; `market` in its last period), never a linear path. **Done:** `project` buys each kind of plant from ONE place — the offer asking least per year of the service it would count at its horizon, a dead firm's vintage or a new build (tie to the new build) — where before it bid in every market its plant could come from and spread its money across them, buying the gap twice when both cleared; the bid itself was already the service left at its hurdle. The estate's ask is `estateAsk`: the last print while it has time, `market` in its last period, and the slide between them is deleted (Appendix B: no written price path). **Measured:** `small-business.test.ts:316` (the `sb-found` world, six periods) now throws where it ran — bank.c's wage bill is 5.1e15 at period 6 and its line publisher cannot tick a negative room — and the cause is on the record at the baseline too: the banking labour venue prints 67 billion an hour at period 3 on one bank's bid `anywhere`, and bank.c pays 2.7e12 of wages a period from period 4. Positioned under 12b.3.
