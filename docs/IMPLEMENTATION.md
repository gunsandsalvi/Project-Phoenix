...
- [ ] 0e′.4 The journal is a LOG, not a store. **The step's list of eight was written from five findings and the count is 25** (measured 0e′.3; the classifier is `SAME-PERIOD` in the run below). A module reading back an event of a LATER period than the one it wrote is remembering a public fact and that is Law 19 working — 17 of those, and they stay. A module reading back an event **it wrote in the same period** is using the log to get from one of its own phases to the next, which is a `working` store with no declaration, invisible to `registry/nouns.ts` and to the phase-order check. Those are the 25:

  - [ ] 0e′.4a `firms.plan` (index.ts ×2, produce.ts) and `households.plan` (index.ts ×2) — the two biggest, and the same shape: a decide phase publishes a plan and a produce/spend phase reads it back. **BLOCKED ON A KERNEL DOOR THAT DOES NOT EXIST**, found on taking the step: two of the three `firms.plan` reads are inside `markets` and `orders`, which are PARTICIPANT callbacks. A participant is handed a `ParticipantView` and nothing else — no `MechanismContext`, so no `ctx.state` — so the journal is not the wrong store here, it is the ONLY store a participant can reach. Writing the plan to a `working` store would leave the participant reading the log exactly as it does now.

    The door has to come first, and the derivation is `ParticipantView`'s own: a participant is evaluated AS its party, `lastOwn` is already a door onto that party's own record, and a plan it took this period is that party's own private state (Observer A4 — nobody else may see it, and a working store keeps it that way where a public event does not). So the missing mechanism is **a per-party working store on `ParticipantView`, written by the owning module's phase and readable only by that module's own participants** — one kernel change, in `world/context.ts` and `world/world.ts`, inserted at this position because every other sub-step of 0e′.4 that touches a participant callback waits on it. `banks:bank.lines`, `equity.plan` and `fx.arbitrage` need checking against the same question before 0e′.4b and 0e′.4c are sized.

    Note what this says about the event itself: `firms.plan` is recorded with `isPublic: false` and read back by its own writer, which means today it is a PRIVATE event serving as a store. That is the defect in one line, and it is why the fix is a store and not a public read.
  - [ ] 0e′.4b `banks`: `bank.capitalPlan`, `bank.capital`, `credit.written`, `bank.buffer` ×2, `bank.lines` — six in one module.
  - [ ] 0e′.4c `equity`: `equity.plan` ×2, `payout.declared`. `securitisation`: `tranche.writtenDown`, `securitisation.absorbed`.
  - [ ] 0e′.4d the singles: `labour.print`, `fund.struck`, `research.estimate`, `estate.paid`, `credit.default`, `credit.impaired`, `margin.call`, `fx.arbitrage`, `treasury.programme`.
