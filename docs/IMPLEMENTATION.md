...
actions. §18 E1 is a FORBID saying that if the notional moves it is a loan, and a swap that pays
somebody else's schedule is that FORBID failing quietly. Building it is 0r's: a contract, a
reference somebody transacts, the two legs, the fixing, the net.
...
- [ ] `Sovereign I1.a` MISSING — VERIFICATION 12.2, 9.1: the `sovereign` system runs `mechanisms/sovereign.rs Sovereign`, which reads the treasury`s schedule, computes how a shortfall would be handled and journals it — it proposes no instruction. `mechanisms/sovereign.rs Auction` and `Missed` have no caller, and there is no central bank (14.1) for the monetary boundary
...
- [ ] `Private Equity A2.a` MISSING — VERIFICATION 12.1: `private_equity` runs `mechanisms/private_equity.rs Calling`, which proposes capital calls; `mechanisms::private_equity` is imported by nothing, so `Buyout`, `Called`, `Exit`, `Failure`, `Mark`, `exit`, `mark_against_exit`, `sources_and_uses` and `transformed` are unreachable (11.1). No buyout happens, nothing is held, nothing exits
