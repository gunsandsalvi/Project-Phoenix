...
- [ ] 15.5 Tenancy: **Finding (12.2, moved from 15.1):** the scale model's lettings venue prints `noSupply` in eleven periods of twelve and `noDemand` in the other — no owner offers a dwelling to let, so no rent has ever cleared there, no household can form (12.2) and no tenancy is signed. `TenancyTerms.until`; re-let at expiry; a landlord ends a tenancy after failed rents by its own outlook of the tenant; `letIn` uses the solver's fills; orders in the venue's unit; `mortgagesOf`/`foreclose` read the loan's `security.instrument` and `lien.id` (no `startsWith`/`includes`); `foreclose` reads `register.ofKind(LOAN)`.
