...
- [x] 4.1 **A-10**. `currency.ts:revaluationAddsUp` builds `booked` and `implied` from the same event's `delta`, `carried`, `was` and `now` — and `world/revalue.ts:revalueForeign` computed `delta` as exactly `carried × (now − was)`. It is one path twice. Reach the second half from the REGISTER: walk the holdings, take each one's carrying and the period's rate move, and compare the total against what the equity and revaluation accounts actually moved by. **DONE.** The positions come from the REGISTER — the holder's own lots, at the carrying the valuation derives from the period BEFORE, so re-reading it after the marks have run gives the number the FX step used. Only the two RATES come from the event, because the rate a period OPENED at is a fact nothing else stores. **And it gained coverage**: a foreign position the FX step never looked at now reports as itself (Currency D2), which nothing checked before.
- [x] 4.2 **A-14**. `labour/index.ts:workforceIdentity`'s B5 line accumulates `acc.states` as `(E ? w : 0) + (¬E∧W ? w : 0) + (¬E∧¬W ? w : 0)` over the two booleans — mutually exclusive and exhaustive, so `acc.states === acc.people` by construction on every iteration. Replace with a second, INDEPENDENT record: the counts the labour mechanism itself acts on when it hires and separates, against the cells. (The two neighbouring checks, `row.headcount !== weight` and `inRows !== acc.employed`, are real — they read the employment book against the parties store. Only the B5 line is empty.) **DONE.** The three states each come from their own source: EMPLOYED from this module's employment book, INACTIVE from the registry's age bands against the parties store, POPULATION from the cells — so an inactive count that overlaps the employed one is a real disagreement. **And a second reading of employment**: what the module BILLED this period (`labour.wages` headcounts, its own published record) against what its book says it employs. They agree only where `labour.release` took out the rows of employers that ceased.
- [x] 4.3 **A-42**. `goods/index.ts:unitsIdentity` and `capital-programme/index.ts:461` both count **every weight event in the world** and switch themselves off when there is one — and `households/lifecycle.ts:age()` journals a `weight` event every period from the first ageing, so neither family has reported anything since. **Delete the exemption rather than narrowing it**: of the three ways a weight moves, `splitCell`/`reKeyCell` preserve the total, `dieCell` refuses a cell that still holds anything, and `weightEvent` is called by nobody (A-17). The guard protects against a case that cannot occur and pays for it with two families. **DONE**, both, and deleted rather than narrowed. The `weights > 0` guard is gone from `goods:unitsIdentity` and from `capital-programme`.
- [x] 4.4 **A-48**. `funds/index.ts:equityIsZero`'s docstring claims the zero *"falls out of the wire"*. It does not: `fundShareKind.owes: 'value'` with `derive → navOf().perShare` makes `assets − liabilities = 0` an algebraic identity. Rewrite the docstring to say what the family CAN catch — a disagreement between two valuation paths — and keep the check. (Item 2.11 makes that disagreement real by fixing `navOf`'s currency.) **DONE.** The docstring says what the family can actually catch — a disagreement between the equity walk (what was booked, period by period) and `navOf` re-derived now — and no longer claims the zero falls out of the wire when `owes: 'value'` makes it an algebraic identity.
- [x] 4.5 **A-11**. `flows.ts`'s `copied` set exempts EVERY holding of a cell named in a split, promotion or merge for the whole period. What has no leg behind it is the copy of the parent's book at the instant of the split, not everything that cell then does. Compare against the parent's remembered per-member state: the split is exact, so the new cell's opening position IS the parent's and any difference is a leg. **DONE.** A cell opened by a split or a promotion is measured from its PARENT's remembered per-member book, which is exactly what it was given, so everything it then does needs a leg like anybody else's. Only a MERGE's absorbed cell is still exempt: its book really is forgotten with no instruction behind it.
- [x] 4.6 **A-12**. `names.ts`: `m.id === i.market.valueOf()` compares a string to an Option object and is always false; the second disjunct does the whole job. Delete the first. **DONE.** The always-false disjunct is gone and the market name is taken out of the option before the closure, so nothing is narrowed by assumption.
- [x] 4.7 **A-13**. Part XII's units family is *"including a population: the sum of cell weights equals the population it stands for, and every represented party sits in exactly one cell."* Neither is in the kernel family; the only contributor that counts people is `labour`, through employment. Add the identity for every cell in the world, not only household cells that have a job — `firms`' small-business pools (item 11) and XI-15 cells generally can lose or invent members today and no family looks. **DONE**, both halves, in the KERNEL family and for every cell. The population is a period-over-period identity: what the cells stand for now, against what they stood for at the last audit plus what the weight events said. The audit's own memory is the second record — nothing stores a population (Appendix B). And two live cells of one kind carrying the same key are one population wearing two states, which nothing looked for.
- [x] 4.8 **A-4**. `registry/environment.ts:conditionsFor` claims *"assembly is where it fires"* and `world/assemble.ts` checks module ids, dependency cycles, money issuance and bank choices and nothing else — `grep -rn exposedTo world/` returns nothing. A recipe naming a fact no environment module declares silently multiplies by 1 for ever. Check it at assembly and **delete the paragraph** (Law 12: the fix removes the comment). **DONE, and NOT at assembly** — see the record. `conditionsFor` throws `Missing` (Goods B4) at the read when a line names a fact the region publishes no condition for, and the paragraph claiming assembly fires is deleted. It could not be an assembly check and be this one: which facts a region has is a fact about the period's published conditions and not about the module list, and a fact can be regional. A world with NO environment at all still answers 1, because "this model has no weather" is an answer and "this line's weather went missing" is not.
...
- [x] 9.3 **A-9** — the insurer, four independent blockers, each alone fatal. (1) `insurers/index.ts:124` returns the CURRENCY CODE where a `UnitId` is wanted, via the file's one `as unknown as` cast — so `Instruments.add` throws `Missing [Appendix A] unit USD does not exist` and **no policy can be registered in any world**; the module declares `COVER` and never uses it. (2) `phases: []`, `participants: []` — nothing runs. (3) `runCover` clears a book and **discards `outcome.fills`**: a session that strikes a price and moves no money. (4) `derive` returns `priced.some ? priced.value : 0` directly under a comment forbidding exactly that. (5) `claimsSeen` reads the insurer's OWN HOLDINGS for policies — a policy is its LIABILITY, the beneficiary holds it — so `written` is always empty. Use `view.instruments.issuedBy(self)`. **DONE, all five.** (1) `unit: () => COVER` — it returned the CURRENCY CODE through the file's one `as unknown as` cast, so `Instruments.add` threw and no policy could be registered in any world; the cast is what let it compile. (2) The module has a phase now: every insurer quotes what a unit of cover costs it, per money, before the markets. (3) `runCover` turns its FILLS into instructions — a policy issued to the buyer against a premium paid to the insurer, both legs, one instruction (Law 5) — instead of counting them and throwing them away. (4) `derive` THROWS `Unpriced` where the curve prices nothing, because a zero reads as a liability the institution has discharged, which is what the comment above it already forbade. (5) `claimsSeen` reads `instruments.issuedBy(self)` — a policy is the insurer's LIABILITY and the beneficiary holds it, so walking its own holdings found nothing, ever, and the price of cover was made of claims over nothing.
...
- [x] 9.5 **B-2** — `insurers()` has no `seed`, so no `insurance` party is ever created. Give it one. **DONE.** One insurer per region that has a bank to hold its money, named and banked like any other institution — because an insurer is one: it has a balance sheet, it can fail, and what it writes is a claim on it.
