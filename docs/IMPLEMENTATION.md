...
- [x] 17.0a **Who sees what: full quarterly financials, published or shared** (the owner's rule, 2026-09-16: *every company prepares full quarterly financials, and they are published or shared only when necessary*; and *a bank won't give more financing to an entity if its public bonds trade in the 60s*; and *you need a better way to screen who has access to what information*). Inserted before 17.0 because the credit view reads what a bank may see.

  **What exists, measured.** Two tiers only: PUBLIC events and a party's OWN view. `journal.visibleTo(party)` already returns public events plus private ones the party is a subject of — the kernel carries the primitive for a bilateral disclosure and nothing writes one. Modules peek: `ctx.participant(<other>)` / `ctx.blind(<other>)` at 96 sites in 41 mechanism files (`tools/check-forbids.ts PEEK_BASELINE`); the assessor graded EVERY issuer from its private `earned`/`owedIn`/`failedPayments`; a lender read the borrower's holdings as collateral; ARCHITECTURE 4.9b fenced schedules (`venueParticipants`) and nothing else. Reporting A1.a made a company public by LISTED SHARES only — a firm with public bonds published nothing — and the report was a trial balance: the equity movement grouped by instruction cause, a balance sheet, cash by counterparty, shares.

  **What a complete quarterly report contains, and where each line comes from here.** Every line is a READ of settled instructions, marks, the register, the agreements store or the party's own outlooks (Reporting A2, A2.a); a line the world has no mechanism for is stated ABSENT with the item that builds it, never composed.

  | section | lines | source | status |
  |---|---|---|---|
  | Income statement | revenue (by product line = the kind of what was sold; by geography = the buyer's region), gains on disposal (proceeds − basis, `Settled.realised`), production (create/destroy legs at basis), wages (by occupation, `employment.by`), rent, interest received/paid, dividends received, tax paid, claims paid, wear (destroy legs on plant) and spoilage (destroy on goods), write-offs (`default` legs at nothing), write-downs / write-ups (marks), other; net income = the equity account's movement (G2) | equity entries over the span, each attributed by the RECEIPT the wire wrote on the money leg (`Receipt`), the leg kinds, or the cause | READ. Income TAX on profit: ABSENT — Treasury C1 levies households' realised gains and income only; a corporate profit tax is the polity's (19). Stock-based compensation: ABSENT (no clause names it) |
  | Comprehensive income | FX translation of holdings in other moneys (Currency D2), unrealised gains on securities (marks) | revaluation entries by the instrument's money vs the party's own | READ |
  | Balance sheet | cash by currency; securities at marks with FAIR-VALUE LEVEL (1 = printed this period, 2 = a stale print carried from a named period, 3 = carried at cost / unpriced); inventories by kind at cost; plant by vintage (kind, region, in service, retires, cost, accumulated wear = cost − carrying); loans held; own debt by instrument (face, next payment, last print, holders); lease commitments; equity = assets − liabilities; shares issued and outstanding | register lots and liens, `Print.provenance`, `PlantTerms`, `agreements.owedBy` + `markOf` + `isLeaseTerms` | READ. Receivables, payables, deferred revenue: ABSENT (17.5 trade credit). Goodwill and intangibles: ABSENT — a combination (`control.combined`) ceases the target into the buyer at book (§35 A4) and research is expensed as wages; R&D capitalisation has no clause. Deferred tax: ABSENT with the tax |
  | Changes in equity | opening, net income, dividends paid, shares issued (`equity.float`, issuance legs), buybacks (own shares bought), closing | equity entries; own-share legs | READ |
  | Cash flow | operating in/out by receipt; investing: plant bought (capex), securities bought/sold; financing: drawings and issues, principal repaid, interest paid, dividends, capital returned; FX effect on cash; opening and closing cash; free cash flow = operating − capex | money legs of the span, classified by receipt and by what the asset leg carried | READ |
  | Segments | per product line: revenue, units sold, realised price, cost of inputs consumed, units made; per geography: revenue | sale legs paired with the asset leg's kind; production instructions | READ (operating income per segment needs wages per line, which no employment row carries: stated as gross margin per line) |
  | Debt note | each own liability: kind, face, money, next payment date and amount, last print, holders; committed facilities drawn/undrawn | instruments issued, `cashFlows`, prints, `holdersOf` | READ; undrawn: ABSENT (17.3) |
  | Leases | plant leases (units, rent, until) and every other standing commitment as debtor (kind, what it is, the kernel's mark) | `agreements.owedBy`, `isLeaseTerms`, `agreements.kind(id).what`, `markOf` | READ |
  | Derivatives & guarantees | own contracts: class, counterparty or house, notional, value to it, margin posted; commitments as creditor (backstops, guarantees) | `contracts.openOf`, `valueTo`, `agreements.owedTo` | READ; hedge accounting: none by design (Clearing D4: everything at fair value through income) |
  | M&A | deals in the span and since: tenders launched, contested, failed, acquired (shares, paid, price), combined, owned; subsidiaries and controller | `control.*` events (registry read `dealsInvolving`), `control.subsidiariesOf`/`controllerOf` | READ; goodwill: ABSENT (above) |
  | Employees | headcount, hours, wage bill by occupation and region, notice given (`leaving`), severance in arrears | `employment.by(party)`, `labour.severanceInArrears` rows | READ |
  | Share capital | shares issued, holders, dividends declared (`payout.declared`) and paid, buybacks, EPS = income / shares (both reads, G5) | register, equity events | READ; diluted EPS: ABSENT (no convertible on a share line) |
  | Related parties | controller, subsidiaries, owners of a small firm (`smallBusiness.ownership`), the banks that keep its accounts | control register, agreements, money holdings' issuers | READ |
  | Subsequent events | public events naming the party between the close and the publication (a default, a rating action, a deal, a failed payment) | `journal.forSubject` in the lag | READ |
  | Regulatory (banks, insurers) | what the party itself PUBLISHED about its capital, liquidity, buffer and reservations; valued commitments (reserves) | registry `capitalPublished`, `liquidHeld`, `bufferHeld`; `markOf` | READ where published; nothing invented for a party that published none |
  | Management commentary | every outlook the party holds (subject, expected, unit, confidence, formed) — the numbers its decisions read (B4); KPIs: units made and sold per line, realised prices, inventory units, plant units, hours worked; concentration: revenue by counterparty share; currency exposure: holdings by money | `outlookVariables`/`outlook`, the span's legs, the register | READ |
  | Non-GAAP | EBITDA (a sum of named lines before interest, tax, wear, write-offs and marks; distributions excluded), free cash flow, net debt, interest coverage = EBITDA / service, leverage = debt / EBITDA | sums of the lines above, stated so a covenant and a reader read one sum (Law 4) | READ |
  | Comparatives | the previous statement is on the record under its quarter label; a change is two levels read (Law 8) | `lastOwn(REPORT)` | READ |

  **Who may read it.** (a) PUBLIC when any market-priced claim the company issued — share or debt — is held outside it (Reporting A1.a broadened; G4 still holds: nothing is published by a company nobody outside holds paper of). (b) SHOWN, and the showing is the kernel's own record (`ctx.disclose(from, to, event)`, kind `disclosed`, private to both; read back by `view.disclosedToMe(kind, from)`): to every lender of record (holder of a bilateral claim on it — Banks Lending A5), to the banks that keep its accounts (they see the flows anyway), to whoever it asks for credit (the ask names the statement: `CreditRequest.statement`), to the assessor it pays (Ratings A5; the assessor grades the statement it was shown and an issuer that has prepared none is unrated). M&A due diligence (a target that agreed to talk) is 17b's. (c) OWN, as before. A FORBID ratchet (`PEEK`) holds the count of `ctx.participant(other)` reads per file and it may only fall; the sites are finding 21.57.

  Steps: [ ] kernel: `disclose`, `disclosedToMe`, `latestReportOf`, `journal.get`, `CreditRequest.statement` · [ ] `registry/statements.ts`: the statement's shape and parser, `REPORT`, `DISCLOSED`, `statementVisibleTo`, `dealsInvolving` · [ ] `reporting/statement.ts`: one pass over the span's settled records per company builds every section above · [ ] every non-terminal party prepares; public or private; shared as (b) · [ ] ratings grades the shown statement · [ ] `PEEK` ratchet · [ ] tests: a private firm's statement is unreadable to a stranger and readable to its lender of record; a bondholder-only company is public; the lines sum to the equity movement; a test never names a party.
...
- [x] 17.1 The leveraged loan as a floating-rate note (second kind in `corporate-bond`; fixing from `index.benchmark`).
- [x] 17.2 The arranger: underwriter, fee from proceeds, risk between commitment and placement, syndicate per-member limits (`lines.ts roomFor`), backstop.
- [x] 17.3 The committed facility: the lender sets the LINE (delete `shortTermDebt.line`; the read is the granted limit on the row), undrawn headroom, the fee, the capital an undrawn line consumes; `ctx.restateInstrument(id, terms)` for rolling a maturity while the relationship performs (the margin loan too).
- [x] 17.4 Issuing reasons (and **21.49**, from 16.4: a FIRM issues in its home money only — `corporate-bond place()` takes the money of its published need; Cross-Border C2's reason for a firm is the lender base and the cost it reads in each money, which is this item's credit view per money): a management target approached at its own pace (A2.b); coverage tests interest plus scheduled principal (A3.a); tenor, size, diversification as separate reasons; a tap goes to the standing line within the tenor band, never a new date per period; covenants carry the holder's headroom as a term of its bid (placeholder naming Corporate Credit B2).
- [x] 17.5 Trade credit: a seller short of cash ships for cash; the days are the seller's preference; rows named (seller, buyer, period, seq); `overdue` memoised.
- [x] 17.6 Short-term debt: an issuer's alternative is its OWN last borrowing cost (so a bank can issue); a buyer's alternative is its kind's (a bank's: the deposit facility); the E3 family excludes lines in an open estate.
- [x] 17.7 **The re-agreement** (`ctx.reagree(instrument, terms, why)`, `Instruments.reterm`, `InstrumentKindProfile.reagree`, `banks/workout.ts`, phase `lending.workout`): the one door out of terms fixed at issuance, narrowed by the KIND (what a re-agreement of it may not change) and by the STATUS (a `rolled` line is performing, a `restructured` one is not). Two decisions behind it: E3's workout, where a creditor takes the bigger of what an estate would return and what new terms would pay less the capital carrying them costs; and 21.59's roll, where a relationship reaching its maturity is extended at what the lender's view now requires rather than repaid and rewritten. Closes **21.59**.
- [x] 17.7a **The seller's own terms, and the price of paying early** (Trade Credit A3, B5; closes **21.61**): every seller declares its own days and its own early-payment discount under its own name the first time it ships on terms, drawn from the module's stated spreads — the param register is the home every seller has, whatever kind of thing it is, and it is where a drawn preference already lives (`firms/born.ts`). `tradeCredit.days` as one number for the whole world is deleted. A3: the discount and the two dates ARE an interest rate, derived once (`impliedRate`); a buyer that holds the money free pays early when that rate beats what its money earns where it sits (its own kind's deposit class, at its own bank's published board), and the row is redeemed below par before its day.
- [x] 17.7b **Factoring** (Trade Credit A3, §42 A4, A5): a seller sells a receivable to a named factor at what that factor requires of the BUYER's name (the obligor is the buyer), against the alternative of offering its own discount (17.7a's rate). Split out of 17.7a: the rate has to exist before anything can be priced off it.
- [x] 17.7c **Security reaches the price** (Banks Lending A4, D5; Housing C5.a; Corporate Credit E5; Law 15): a bank's required yield on a DATED CLAIM accounts for what that claim is secured on — the share the security does not cover at the market's own price of it, taken off the expected loss it already published — so a secured claim from a name is dearer than an unsecured one from the same name, mechanically and not by assumption. Read off `ranking(i).secured`, so it is every kind's and no kind's branch; `worthToItsLender` stops reading a loan's own terms for it. INSERTED HERE because a secured claim whose security does not reach its price is a shape (Law 2): it is what 17.7d's repo haircut is taken over, what a mortgage-backed senior note is worth to the buyer that holds it (§42 D3), and what any secured claim costs its issuer.
...
- [x] 17.9 `banks/lines.ts`: a line's ask is a SIGNED number and both floors are deleted — a line past its own appetite gives its capital back, what the bank is still over the rules by is shed on top (worst earner first, and no line sheds more than it is using), and `banks/dealing.ts mustRaise` is the desk reading it: a book above the room it was allotted is a sale at market, beside the overnight session that refused it.
- [x] 17.9a `banks/index.ts runRepayments`: PREPAYMENT (C9) — a borrower whose published shortfall is NEGATIVE pays its lines down out of the money it said it does not need, dearest first, through the same door and the same lag its ask goes through. **21.63 is answered rather than fixed** (see below).
- [x] 17.9b `world/world.ts spentClaimsOf`: THE END OF A CLAIM (E5, E5.a) — a terminal party succeeds itself, so when an estate has paid everything away every row it promised that has nothing outstanding ceases with it, on that date and in the journal; a row with something outstanding is a debt nobody can pay and stays standing. **21.62 moves**: an agreement has no cure, and nothing in this world asks for one — building the door would be building a door nobody opens (Law 14). Repositioned to 19, where a payment plan on an arrear with the treasury is the first thing that will ask.
- [x] 17.10 **A credit index over the rated universe** (Indices A1, C2, C2.a; Ratings C2): the boundary the market draws across its own scale is DATA beside the scale (`registry/grades.ts INVESTMENT_GRADE`), a basket rule can read what the assessors published (`IndexWorld.graded`, through the kernel like a print), and each money carries a line on each side of it. A name nobody has graded is in NEITHER: unrated is the absence of an opinion, not a side. `gradeOn` moved from `banks/credit-view.ts` to `registry/notices.ts`, where the three readers of a grade can reach it.
- [ ] 17.10a **The assessors say nothing** (closes **21.68**): three assessors exist in every scale model and not one of them ever publishes a `rating.action` — so every name is unrated, every claim carries the regulation's ungraded weight, and both rated indices are empty. Inserted HERE, before the tracker: a tracker on an empty index is nothing, and a rating that never happens is what makes it empty.
- [ ] 17.10b **The tracker over the rated universe** (Indices C1, C2; Fund Shares E3): a vehicle on the investment-grade line, launched in kind the period its index first answers, like the size trackers. It needs `drawTrackers` to draw a CREDIT tracker — a debt blueprint and a basket of bond lines, where today it draws an equity one — which is the seed's, not the index's.
...
- [ ] 21.68 `mechanisms/ratings/index.ts` (17.10's census of four seeds, twenty periods each): THE ASSESSORS SAY NOTHING. Three assessor parties exist in every scale model and not one `rating.action` is ever published — in `rated-a`, `trade`, `spv-a` and `idx-D` alike, zero in twenty periods. Everything downstream then takes the unrated branch and says so honestly: the credit view weights every claim at the regulation's ungraded loan weight (17.0), a supervisor's rule refers to an opinion nobody holds (Ratings C2), and both of 17.10's rated indices are empty, so `w.index` reports Missing for each. Why no assessor grades anything is a read to make against a run and not a number to chase (Law 11); positioned at 17.10a, which is that read (17.10).
