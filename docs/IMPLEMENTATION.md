...
- [ ] 9.9b **The three that are one noun: `expectations.outlooks`, `ratings`, `research`.** The plan said *"four kernel nouns were built"* and for `View` that is **false** — `docs/RECORD.md`'s own item 6 entry says so in as many words: it built `Subject`/`about`/`subjectOf`, the VOCABULARY, and *"`credit` is expressible and nothing forms one yet … folding `ratings`, `research` and `banks/reserves` into the noun — three private stores that are this thing in three shapes — follows the same way."* So this is a BUILD and not a migration, and it has two halves that are not the same: `expectations.outlooks` IS the noun's implementation (adaptive, from the wire, §46 B1) and wants re-declaring rather than moving; `ratings` and `research` are JUDGEMENTS one party holds about another (formed from `ctx.blind`, not from an observable), which is what `Subject.credit` was made expressible for. Decide whether `View` is one store or two mechanisms sharing a subject, **and say which in the record before writing it**.
