...
**INSERTED (Law 10), named by 21.38–21.42.** Twenty-five of the forty-seven rows in `systems.rs` are
wired as `Reads { Counts::… }`: an honest count of something real, and no decision and no write. Four
have been converted by the items that needed them — `recipe` at 22, `funds` at 21b, `estate` at 21c,
`employment` at 12b. The reading systems are right to stay out of the books (handing a rating agency a
schedule invents demand nobody has); what is wrong is that **the read each performs is not the read
the system is for**. The seven `Closing` rows are the same absence the other way up: nothing opens
four of the seven.
