...
- [x] 12b.1 One employment register (noun `labour.employment`, home `register/employment.ts`): employer, employee cell, hours, wage, since, notice. Delete `labour/register.ts`, `banks/staff.ts`'s `staff` map, the `labour.wages` tally; wages are instructions that READ it; `view.employs()`. **Done:** `register/employment.ts` is the noun's home — the kind (`EMPLOYMENT`, declared by the kernel at assembly beside the arrear), its terms (occupation, region, wage, hours, `since`, `productiveFrom`, `notice`, headcount) and every read over the rows (`EmploymentReads`: of a worker, by an employer, at a trade and place, in a trade, all; hours at, going rate, employed, an employer's payroll, ever employed), on `ctx.employment` and `view.employs()`; `labour/register.ts` and its private index are deleted (the labour module keeps only the SKILL, noun `skill`); the `labour.wages` tally is deleted with its eight `reads` declarations and the units family's second reading — `registry/wages.ts` reads the register for what is due and the ledger for what was paid; the `labour.wagesInArrears` agreement written beside settlement's arrear on a failed wage is deleted (Law 4). `banks/staff.ts` had no `staff` map left to delete; it read the tally, and reads the register. **Measured:** three tests that were red are green (a control buyer that never met a payroll, a small-firm cell's wage bill, the state's own payroll), no new red. **And (12a.5):** with holdings as totals a household cell fails as ONE — every member of a twenty-thousand-household cell goes to probate for a miss that was some of theirs; the per-member miss needs a member with an income of its own, which is this register.
- [x] 12b.2 The venue matches NET changes: an employer posts the change it wants; a separation runs `notice` periods with wages due, then ends (Labour C3). **Done:** every employer posts the CHANGE against what it will have (`register/employment.ts netChange`, one read for firms, small firms, banks' desks, fund managers and the treasury) — more hours is a bid, fewer is a SELL the venue reads as a cut; the venue no longer reads an employer's rows against its posting. A cut gives notice: the row is restated with how many are leaving and the period the notice ends (`EmploymentTerms.leaving`, `ends`), nobody moves, wages run to the end (`payWages` reads the row), and `labour.release` separates them when it runs out with nothing more owed; `hoursAt` is the hours an employer will keep. The lump severance is gone; a CEASED employer still owes the notice it could not run as the unsecured claim (C4, Firm Birth D2.b). **Found and fixed on the way:** `payFrom` paid a wage to every member of the worker CELL, not to the people on the ROW — the standing cell of the employed key holds every employer's staff at once (0f.4), so a firm with a row of ten paid two hundred and ten (the treasury's hires in the same cell), twenty-one times its bill in the labour scale model; the wage is the row's headcount now, and the `sb-found` throw of 12a.8 is gone with it. **Measured:** `labour.test.ts` at its five reds — the A4.c findings of the merged cell (12b.2a); `treasury.test.ts:239` newly red: with wages paid to the row the treasury misses twelve coupons in period 52 of the `tsy-g` scale model (XI-9; positioned at 12b.6 to re-measure when the register is whole).
- [x] 12b.2a **The employed key names the employer.** The standing cell of a household key with `employment = employed` holds EVERY employer's staff of that region, cohort and bank at once (0f.4: a hire re-keys the hired onto the standing cell), so a row of ten sits on a cell of two hundred and ten and the units family reports it every period (`Labour A4.c`: "row carries 10 of a cell of 210" — the 142, 73, 62 and 15 findings the labour scale model's tests are red on). A cell holds one job (B3); the key must say WHOSE: the `employment` dimension's value is the employer for the employed (`employed:<employer>`), `unemployed` otherwise, moved by `labour.hire` and `labour.separation` as now. Then `hoursAt`, the per-member wage and A4.c all read true, and `labour.test.ts`'s five reds are re-measured. **And (12b.2):** `labour.test.ts:316` divides a wage leg by the worker cell's weight to get the per-member wage — the row's headcount is the divisor once the cell is the row's. **Done:** `register/employment.ts employedKey(employer, occupation, period, round)` is the one writer of the value — the four things that make one row's terms — spelled by the hire (`labour/matching.ts hire`) and by the household lattice's opening rule; a second hire into the same job in the same period lands on the same standing cell and moves the row's headcount rather than opening a second row (B3). **Measured:** the labour scale model's audit carries no Labour finding any more — its remaining reds are the world's own (`Clearing F2` prints, `Audit B5` accounts, `Clearing D1` names, `Money D3` flows), which is 12b.6's and Part XII's; three more tests green across the touched suites (the treasury's coupons in period 52, a small-firm plant bid, a fund's fee), none new red.
- [x] 12b.3 A wage bid is from the employer's outlook on its revenue (§46), not `earned(1)`; the firm's ask is from its outlook on its own sell price formed from fills and the venue print, its cost the reservation (F15). **And (12.1):** power, transport, logistics, facilities, telecoms and IT print `noSupply` from period 0 in the scale model — the small firms in them never offer, because their inputs never trade there. **And (12.4a.2):** an employment row's terms name the worker cell that signed; after that cell merges the name is stale and the row is not succeeded (the store succeeds debtor and creditor, not terms) — `separate` and the wage leg resolve it through the successor for now; the row's worker should be its creditor, never a term. **And (12a.8):** in the `sb-found` scale model the banking venue prints 67,246,706,045 an hour at period 3 — one bid, a bank's, `anywhere` — and bank.c pays 2.66e12 of wages every period from period 4; after 12a.8's change of the estate's ask the same world's period-6 wage bill was 5.1e15 and `banks/lines.ts publishLines` threw on a negative room (Law 8) — 12b.2 found that bill: a wage paid to every member of the standing employed cell rather than to the row, fixed there, and `small-business.test.ts:316` is green again; the 67bn/h print stands. A bank's bid for hours is a price it must be able to pay out of what its desks earn, never `anywhere`. **Done:** a bank's two desks and a fund's manager bid an hour against their own outlook on their EARNINGS (`registry/expectation.ts expectedEarningsOf`, §46: formed adaptively from what the book made, at their own memory) and never `earned(1)`; a price a party watches and did not trade at this period reaches it as the venue's print (§46 A2.a: one more thing observed, never the outlook itself — `expectations.score` reads the period's prints for the variables it already holds); a firm's sell reservation is its COST, lot by lot off the register (`firms/decide.ts sellSchedule`), and what it expects to fetch bids for hours and sizes batches only (F15). **Measured:** the banking venue prints 43 million an hour at period 4 of `sb-found` and falls to 13 million by period 8, from sixty-seven billion; `sb-line` (and five more `sb-*` scale models) THROW in period 4 — `capacityFrom` counts a capacity of 9.7e15 pieces a period for a line whose machinery cost 0.616 a piece where the print had been 630,879 — positioned at 12d.1 (the price level) and 21.1 (a count of pieces past 2^53 is a grid, not a capacity); `labour.test.ts` at its five reds, `expectations.test.ts` at its two.
- [x] 12b.4 `labour/matching.ts match` in whole hour-pieces; the remainder is unsold hours journaled `noDemand`; the labour print read for the reader's own region (`treasury/index.ts wageItFaces(region)`). **Done:** `match` returns the hours it hired in whole people, and what the book cleared beyond them is journaled `labour.unsold` with outcome `remainder`; a venue that did not clear journals the same event with the solver's own outcome (`noDemand`, `noSupply`, `noOverlap`) and the hours offered and wanted, so nothing offered into a venue is dropped without a word. The print read for the reader's own region was already so since 12b.1 (`wageFacingParty(…, region)` → `wagePrintedIn(journal, region)`); nothing to change. **Measured:** six periods of the scale model — 304 venue-sessions `noDemand`, 8 `noSupply`, 7 whole-person remainders, 8 prints: the labour market of this world is mostly seekers and no employer, which is 12b.3's price level and 22.3's buyers to read against.
- [x] 12b.5 Research analysts are `analysis`-trade staff hired in the venue; `research/index.ts pay()` deleted. **Done:** a bank posts for analysts in the `analysis` venue as its other desks post (`research/index.ts analystOrders`: the change against what it has, for the hours the names it wanted last period take, at what an hour is worth to it, in whole people), pays them with everybody else's wages off the employment register, and covers no more names than their hours reach (`namesItCanCover`); a name it cannot staff is dropped with that reason. `pay()` — a transfer to every household that banked with it, split per member, at the printed wage, for hours nobody had contracted — is deleted with its `flows` family. **Found and fixed on the way (Law 8):** a desk that needs thirty-two hours of a thirty-five-hour week and posts thirty-two hires nobody — the fill does not make a person — so every employer's posting rounds UP to whole people (`registry/wages.ts wholePeople`, the labour module's hours-per-member named in the registry for the purpose). **Measured:** in the research scale model no bank wants a view of anybody — `needsTheView` holds for no (bank, company) pair — so nothing is initiated in sixty periods with or without this change (`research.test.ts` 8 of 9 red at the baseline, 7 now); the fund managers hire analysts in the same venue, and one that is the only bidder pays its whole bid (D1): `funds.test.ts:227` newly red — positioned at 14.
- [ ] 12b.6 Test: a bank with one bad period keeps its desk; a separation pays notice; one register answers who works where. **And (12b.2, closed at 12b.2a):** `treasury.test.ts:239` was red for one item — twelve coupons missed in period 52 of `tsy-g` once wages were paid to the row — and green again once the employed key named the job; the labour scale model's five reds are the world's own audit findings (`Clearing F2`, `Audit B5`, `Clearing D1`, `Money D3`), to be read here against a whole register.
...
- [ ] 12d.1 `expectations`: every public print and public event about a party the observer is exposed to enters its outlook as an observation (§46 A2.a): the region's wage, the retail prints it buys at, its bank's board, the dwelling print, the reports of companies it holds. The exposure set is a read of the party's rows and holdings. **And (12b.3):** with a firm's ask at its cost, a machinery maker asks 0.616 a piece where the print had been 630,879 — the print was a belief asking the market to agree — and a buyer's gap takes thirty-nine million pieces of it in the `sb-line` scale model at period 3; the price level of this world is what these asks were holding up, and six `small-business.test.ts` scale models throw in period 4 on the capacity that plant gives a line (see 21.1).
