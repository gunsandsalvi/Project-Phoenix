...
- [x] 21.4 `estate/index.ts:608`: delete the stale `standsInFor: { noun: 'Process' }`. **Verified absent**: no `standsInFor` survives the port, and `processes` is a kernel store, so the noun it stood in for has its home.
- [x] 21.5 Dead reads deleted with their docstrings: `money-market/session.ts struckRate, findSession`; `cds/participants.ts cdsBookOrders`; `households/portfolio.ts ownUncertainty`. **Verified absent**: none of the four names exists in the engine.
...
- [x] 21.8 `funds/index.ts:1296` cites `Clearing C1.b` (absent): fix; `check:spec` covers prose citations. **Closed by the check, which is what this asked for.** `npm run check:spec` did not exist (`CLAUDE.md` said it must pass); `phoenix-check` now resolves every `@spec` citation against the specification — a law, a numbered system with a clause, a system by its short NAME, a Part XI mechanism, an appendix, a range, a whole lettered group — and the short names are DERIVED from the document's own headings rather than declared a second time. It found four: `Central Bank A3.b` in `sovereign.rs` (§31 has A3 and A4; the clause the module is about is **E2, no automatic overdraft**), a bare `4.9b` in `module.rs` and `stores.rs` that names no document, and `22b.8` in `num.rs`, which is a PLAN ITEM cited where a clause belongs. All four fixed.
- [x] 21.9 String parsing gone: `derivative-layer/margin.ts:1499` structured `secures`; `securities-lending receivedOn/lienFor` by lien id; `observer.ts:688` reads the book's tenor from its terms. **Verified absent**: no fact in the engine is recovered from a string.
...
**Eleven of these are VERIFIED ABSENT and are ticked below** (21.115a): every one whose shape was
...
`O(parties × markets)` default the finding is about cannot be written. What is left in this group is
the walks over PARTIES and INSTRUMENTS, which do survive — `running.rs` has them — and those are a
re-read against the engine that exists, not a verification.

- [x] 21.115.BF1 **Verified absent (21.115a).** `bond-futures/index.ts:1731 bondCarryOf`: `journal.ofKind('index.benchmark').filter(...).at(-1)` full scan per (deliverable, read) — called in `netBasis` per book measure and in `futureOrders`? (no: futureOrders uses outlook). Fix: `lastOf`.
- [x] 21.115.BK16 **Verified absent (21.115a).** `treasury.ts:652 classesSeen` reads the last `deposit.classes` event: O(events of kind) per call via `ofKind`; called once per period. Fine.
...
- [x] 21.115.CB5 **Verified absent (21.115a).** corporate-bond/index.ts:535 `testCovenants` walks ALL instruments per period and `alreadySaid` walks `forSubject('covenant.breached', bond)` per bond — O(bonds × breaches). Index live corporate bonds (register ofKind) and keep the last-tested quarter on the instrument's working state.
- [ ] 21.115.CD11 `cds/series.ts:1200 weightShare`, `runningShare`: raw `/` divisions (not `ratioOf`) on `number` weights — type-erased.
- [x] 21.115.CF3 **Verified absent (21.115a).** commodity-futures `carryFromWorld.lastPublic` uses `journal.ofKind(kind).at(-1)` — full scan of all events of the kind every read; must be `lastPublic` / indexed.
...
- [x] 21.115.IR1 **Verified absent (21.115a).** `irs/index.ts:1623 openBooks`: `journal.ofKind('index.benchmark').some(...)` full scan per currency per period. Fix: `lastOf`.
...
- [x] 21.115.IX3 **Verified absent (21.115a).** indices/benchmark.ts `benchmark(ccy, secured)`: walks ALL `moneyMarket.print` events ever (ofKind) per (ccy, secured) read; must be `ofKindIn(kind, period)` on the last printing period or a journal lastOf index.
...
- [x] 21.115.OB2 **Verified absent (21.115a).** observer.ts:1071 `indicesOf`: `ofKind('fund.launched')` full scan per snapshot; :1152 `statementsOf`: `ofKind('research.estimate')` and `ofKind('research.surprise')` full scans PER COMPANY (O(companies × events)). Index by subject (`forSubject`) or by (kind, period).
...
- [x] 21.115.OM4 **Verified absent (21.115a).** central-bank-omo/index.ts:1171 `lastRemittance`: `ofKind('centralBank.remittance')` full scan + filter + sort EVERY period for each central bank. `lastOf(kind, cb)`.
...
- [x] 21.115.SZ4 **Verified absent (21.115a).** securitisation `windUp`: gating on `cease` event presence via ofKind scan; the deal state should carry its wind-up flag.
...
- [x] 21.115.WK5 **Verified absent (21.115a).** world.ts:2150 `classifyAsset.gradesOn`: `journal.forSubject('rating.action', obligor)` walk per classification; `classify` is called per instrument per mandate check per fund per period. Keep the latest grade per (assessor, obligor) in a journal-side index (or the ratings module publishes it as a read).
