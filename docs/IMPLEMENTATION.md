...
- [x] 17.10a **The assessors say nothing — ANSWERED, not a defect** (closes **21.68**): they speak from period 20, and what was missing was periods. A quarter is thirteen weeks and nothing is reported on one that began before the world did (Seed A2), so the first close is between one and four quarters in — period 20 in the rig, with the first rating at 21 and the first rated index level at 21. The censuses that found silence stepped twelve and twenty periods. A test now pins the whole chain and its ORDER: a company closes a quarter, shows it to the assessor it pays, the assessor grades what it was shown, the index reads the grade.
- [x] 17.10b **The tracker over the rated universe** (Indices C1, C2; Fund Shares A4, E3, E3.a): a vehicle on each side of the line, launched in kind the period its own index first answers. Two things made it possible: a tracker's launch basket is now its INDEX's constituents rather than a list written down at the seed (`funds/index.ts launchBasket`) — a credit index holds paper issued years into the run, with ids nobody could name in advance — and `drawTrackers` carries what its vehicles may HOLD, the house they belong to, and whether the seed may open them. Each vehicle is named for the line it follows (Law 9).
