...
- [ ] 17.9a `banks/loan.ts`: the loan's own lifecycle — PREPAYMENT (a line is repaid at the borrower's option, C9) and the WRITE-OFF (E5: the loan leaves the book on a date and the loss hits capital, E5.a: principal less recovery less provisions already taken). Split out of 17.9 and inserted here, immediately after it: the treasury's allocation and what happens to one row are two bounded changes, and the second needs nothing of the first. **21.63** lands here (a row repaid to the last unit stays live, which is the same door) and so does **21.62** (an agreement has no cure, which is the write-off's sibling transition).
