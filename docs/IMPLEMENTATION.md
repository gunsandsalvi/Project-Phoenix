...
- [ ] 17.9b `banks/loan.ts`: THE WRITE-OFF (E5, E5.a) — the loan leaves the book on a date and the loss that reaches capital is principal less recovery less provisions already taken. Split out of 17.9a and inserted here, immediately after it: a repayment is a borrower acting and a write-off is a lender giving up, and the second needs the end of an estate (what is left to recover when the estate that succeeded the borrower has closed). **21.62** lands here — an agreement has no cure, which is the write-off's sibling transition — and is built only if something in this world asks for one (Law 14: a door nobody opens is not a mechanism).
