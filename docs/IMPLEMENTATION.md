...
- [ ] 21.61 `trade-credit/index.ts` `tradeCredit.days` (17.5, listed there and not built): the item says *"the days are the seller's preference"* and they are still one declared number for the whole world — thirty days, a technology, the same for a mill and a corner shop. B5 says a seller DECIDES its terms, so the days belong to the seller, drawn like its hurdle and its horizon. What stopped it: a seller in this world is a firm, a small-business cell or a merchant, and the firms module draws numbers for firms only — so the preference needs a home every seller has, or a read that answers nothing for a seller that has none, and "nothing" cannot mean "no terms" without deleting the tier that lives on them (§36 A4, 11.0b). Positioned at 17.7, where factoring and the rest of trade credit land (17.5).
...
- [ ] 21.59 `ctx.restateInstrument(id, terms)` (17.3, listed there and not built): a loan or a margin loan whose relationship is PERFORMING at maturity is rolled — the same row, a later date — rather than repaid and rewritten, which is what a relationship lender actually does and what stops a healthy borrower failing on a date. The kernel has no door that restates an instrument's terms (`restate` is a split, `fixCoupon` is a coupon), and the shape of the door is the question: a general terms-restatement lets any module change anything about any line. Positioned at 17.7, where restructuring is the subject and an agreement transition is already the work (17.3).
