...
- [ ] 0f.11 ARCHITECTURE 4.4 rewritten; COVERAGE re-marked for §41, §42, §46, XI-15, XI-16; record. **Positioned here by 0f.7:** `expectations` still forms the `benefit` outlook (what reached a party that it did not work for) and since 0f.7a nothing reads it — an outlook with no reader is a stale representation (Law 16); delete the subject in `world/context.ts` and its forming in `expectations/index.ts`. **And a measurement 0f.7b makes possible and does not take (Law 11):** a household cell now asks its bank for a roof; 13d.1 recorded that a cell with a mortgage and nothing set aside misses at once and drifts from the lender's book — that is 13f's, and 0f.7c's `due` before the basket is the household half of it.
...
- [ ] 11.0 Small-firm profile (`small-business/profile.ts`). Decisions are threshold rules per cell from its own state and outlook; orders are totals; nothing branches on the kind.
  - [ ] 11.0a Sell and produce: a capacity line (`GOODS` with `output: 'capacity'`) is its members' hours at the cell's drawn productivity (a TECHNOLOGY draw beside size), created by the kernel's `create` leg into the line's good at cost and offered in the region's goods venue at the cell's own ask — its outlook on the sell price, the last print where it has none, nothing where neither. The six `ofKind(FIRM)` sites walk `PRODUCING_KINDS`.
