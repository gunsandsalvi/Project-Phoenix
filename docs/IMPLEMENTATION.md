...
| 16 | Cross-border, the rest — 16.0–16.6 **done** | 16.7 the central bank's holdings are its own |
...
- [x] 16.2 `abroadWorld` 30 periods: per country banks, firms, households, listed lines, sessions cleared; the table written here. Seed `abroad-16-2`, 1,105 parties, 1,167 markets, 84 s; "firms" are named firms alive at period 30, "people" the members of the household cells, "dead" every party whose status is not alive (estates, ceased cells, resolved names), "sessions" every market session of the thirty periods in that money and "cleared" those that printed a trade:

  | money | banks | firms | household cells | people | listed lines | dead parties | sessions | cleared |
  |---|---|---|---|---|---|---|---|---|
  | EUR | 1 | 27 | 10 | 254 | 1 | 107 | 6825 | 78 |
  | GBP | 1 | 71 | 18 | 262 | 1 | 120 | 6432 | 43 |
  | JPY | 1 | 140 | 3 | 203 | 1 | 185 | 11965 | 48 |
  | USD | 1 | 90 | 3 | 203 | 1 | 131 | 6491 | 46 |

  Two stops found on the way and fixed where they were: a fund seeded in kind with lines in two moneys summed them, and a bank drawn to hold a foreign line added it to its dollar book (both a REPORT in the party's own money now, Currency C4); and a second stock loan between the same two names on the same line took the FIRST loan's lien id (`lienFor` matched the shared reason), so the return of the second threw `Register D5` — the lien a loan holds is the one no open loan already holds. Findings 21.47.
- [x] 16.3 Sourcing across regions: a buyer in one place buys from a seller in another; merchants bid the far print less their outlook of the freight rate on that leg with one budget across lines; producers ship only unsold stock on the dearest leg.
- [x] 16.4 Foreign-currency issuance: a treasury or firm issues in a money it does not print and can fail in it (12a.6). The treasury half is built: it borrows where it reads the cost lowest — its own curve at home, the benchmark plus its own outlook of the pair's move abroad — and the proceeds land in its account in that money; it fails there as 12a.6 made it. The firm half is 17.4's (21.49).
- [x] 16.5 The swap line; `ctx.transact([books])` — one instruction whose trade legs come from two or three sessions, atomic or not at all (FX arbitrage sized by cash held in each money).
- [x] 16.6 FX participation is a question over every party kind (no kinds list); `needOrders` prechecks a foreign balance or liability before `owedIn`.
