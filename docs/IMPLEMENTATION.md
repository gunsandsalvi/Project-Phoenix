...
- [x] 12d.1 `expectations`: every public print and public event about a party the observer is exposed to enters its outlook as an observation (§46 A2.a): the region's wage, the retail prints it buys at, its bank's board, the dwelling print, the reports of companies it holds. The exposure set is a read of the party's rows and holdings. **Done:** `expectations/index.ts exposed(ctx, party)` — the print of every line it HOLDS (a dwelling it owns among them), what a company whose paper it holds PUBLISHED it earned a period (the period after, the lag a statement has; `reported.<company>`, §46 C2.a), what an hour cleared at in every venue its employment rows put it in, either side (`wage.<venue>`, off the published going rate, itself about the period that closed), and the board of the bank it banks at on its class (`deposit.<bank>`); three subjects added to the kernel's vocabulary (`world/context.ts about/subjectOf`), the reads through the registry (`goingRatePublishedAt`, `depositRatePostedAt`, `published.lastStatement`), never an event name. Each enters as an observation the score corrects towards at the party's own memory; a first sight is the outlook. **Not built here:** the retail prints a cell buys at before it has ever bought (its basket is the households module's costing, not a row or a holding) and the dwelling print for a cell that rents (its tenure row names a venue, not the line) — both are the exposure set's next read and are 12d.2's to take with the anchored predictors. **Measured:** twenty-two periods of the scale model — every party holds an outlook on every line it held that printed; every party on an employment row, either side, on the going rate where it works or hires, and a cell that never worked on none; depositors on their bank's board in `per annum`; the holders of the one company that reported by period 20 on what it published, the period after, and nobody else (`observation.test.ts`, four green). Nothing new is red: the ten suites at 56 of 109 and `expectations.test.ts` at its two, the same sets. **And (12b.3):** with a firm's ask at its cost, a machinery maker asks 0.616 a piece where the print had been 630,879 — the print was a belief asking the market to agree — and a buyer's gap takes thirty-nine million pieces of it in the `sb-line` scale model at period 3; the price level of this world is what these asks were holding up, and six `small-business.test.ts` scale models throw in period 4 on the capacity that plant gives a line (see 21.1). **And (12b.3, from 12a.8):** the banking venue's print — sixty-seven billion an hour on one bank's bid at any price — is forty-three million an hour at period 4 of `sb-found` and thirteen million by period 8 once the bid is the bank's outlook on its earnings; still a price nobody else in the world pays for an hour, to be read here against the price level.
- [x] 12d.2 A party keeps its adaptive predictor and one or two public-anchored predictors per variable and follows the one with the smaller surprise over its memory; no intensity coefficient. Killer: §46 E2/E3 in a 52-period run — if the aggregate moves before the surprises, this step is deleted. **Done:** one anchored predictor beside the adaptive one — the last PUBLIC level of the variable (the venue's print for a price; a going rate, a board and a statement are public and anchor themselves; income, earnings, own sales have none) — each with its own track of surprises over the same memory; at the top of the period the party follows the narrower track, a switch and never a blend (B1.b), decided from history only (B4) and left alone on a tie or while either track is empty. The outlook read, its confidence and the published dispersion are the followed predictor's; the surprise event carries the surprise it took, which predictor it followed, and both tracks' entries, so the switch is checkable off the record. **Measured (the killer held):** over fifty-two periods of the scale model every change in a variable's published dispersion follows a surprise on that variable in the period the statistic is about, or a party arriving or leaving it — the aggregate never moved first (`predictors.test.ts`); some parties follow the public level and some their own history, and the recorded choice replays exactly from the recorded tracks. Nothing new is red: ten suites at 56 of 109, `expectations.test.ts` at its two, the same sets.
- [x] 12d.3 The central bank, insurers and households read the environment condition where their decisions need it (18a.1, 14.2, heating in the basket). **Done (the households' half):** a basket row declares what a member takes MORE of to stand against the weather (`ConsumptionDecl.standsAgainst`; the two fuel rows name `warmth`, every other row nothing), and the basket costs the period's quantity as a normal week's over how the condition stands, read off the region's public state from the participant's own view (`registry/environment.ts conditionsSeen, conditionsStanding` — the same product a crop's yield reads through `conditionsFor`, from a view instead of a context). A physical relation, not a coefficient: a week at four fifths of its warmth burns a quarter more. **Positioned, not built here:** the central bank's read goes with the decision that needs it — its outlook on the price level and the rate step, 18a.1; the insurers' read with the buyer that bids at its own outlook of the loss against its plant's exposure, 14.2. Neither decision exists yet, so a read into it would be a read into nothing; both lines carry the note. **Measured:** six periods of the scale model, every living cell — the fuel line's need and want are the row's litres over the week's warmth to twelve places, every other line is its normal week's, and the environment moved off normal in some week so the read bit (`heating.test.ts`). Nothing new is red: ten suites at 56 of 109, the same set.
...
- [ ] 14.2 The buyer: a firm standing in a physical fact bids for cover at its own outlook of the loss (the environment condition against its plant's `standsWind`); a household cell bids for life cover from the mortality its cohort observes. Bids are schedules (`rungsUpTo`), not points. **And (12d.3):** the condition is read through `registry/environment.ts conditionsStanding` from the buyer's own view, the door the household basket reads its warmth through; nothing here parses the event.
