...
- [ ] 12b.2a **The employed key names the employer.** The standing cell of a household key with `employment = employed` holds EVERY employer's staff of that region, cohort and bank at once (0f.4: a hire re-keys the hired onto the standing cell), so a row of ten sits on a cell of two hundred and ten and the units family reports it every period (`Labour A4.c`: "row carries 10 of a cell of 210" — the 142, 73, 62 and 15 findings the labour scale model's tests are red on). A cell holds one job (B3); the key must say WHOSE: the `employment` dimension's value is the employer for the employed (`employed:<employer>`), `unemployed` otherwise, moved by `labour.hire` and `labour.separation` as now. Then `hoursAt`, the per-member wage and A4.c all read true, and `labour.test.ts`'s five reds are re-measured. **And (12b.2):** `labour.test.ts:316` divides a wage leg by the worker cell's weight to get the per-member wage — the row's headcount is the divisor once the cell is the row's.
...
- [ ] 12b.6 Test: a bank with one bad period keeps its desk; a separation pays notice; one register answers who works where. **And (12b.2):** re-measure `treasury.test.ts:239` — with wages paid to the row the treasury misses twelve coupons in period 52 of the `tsy-g` scale model (XI-9: the funding constraint bit where it did not before); and `funds.test.ts`'s fee-ratio test moved from its first assertion to its last.
