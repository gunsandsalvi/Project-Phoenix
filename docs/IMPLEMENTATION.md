...
- [x] 17.7c **Security reaches the price** (Banks Lending A4, C5.a; Corporate Credit E5; Law 15): a bank's required yield on a DATED CLAIM accounts for what that claim is secured on — the share the security does not cover at the market's own price of it, taken off the expected loss it already published — so a secured claim from a name is dearer than an unsecured one from the same name, mechanically and not by assumption. Read off `ranking(i).secured`, so it is every kind's and no kind's branch; `worthToItsLender` stops reading a loan's own terms for it. INSERTED HERE, before the covered bond: a covered bond whose security does not reach its price is a shape (Law 2), and this is the mechanism that stops it being one.
- [ ] 17.7d **The covered bond**: a bank's secured funding whose pool of its own loan rows is pledged and STAYS on its book — dual recourse, the honest opposite of §42 C1's transfer into a vehicle (Banks Funding A2, C1).
- [ ] 17.7e **Senior notes and commercial paper as repo collateral** (§42 D3, Banks Funding C1.a, D1): what the secured market will take, and at what haircut.

  Split out of 17.7 and inserted here in dependency order, before 17.8: each is an instrument shape or a price that needs 17.7's terms door and nothing later. **The index-linked schedule left 17.7 too and is inserted at 18a.1**, where the first published price level in this world is — a schedule that pays off an index cannot be written before somebody publishes one.
...
- [ ] 21.66 `mechanisms/banks/capital.ts`, `mechanisms/banks/lines.ts`, the seed (17.7b's census): EVERY BANK IN THE SCALE MODEL OPENS IN CAPITAL BREACH, IN PERIOD ONE. `bank.capital` reports `breach: true` and NEGATIVE headroom for all three banks from the first period they publish — bank.a at a weighted ratio of 0.089 against a minimum of 0.08 plus its own buffer of 0.039, bank.c below the requirement outright — so every line's ask is negative, both lines are allotted ZERO room in every bank in every period, and NOTHING can be taken onto any bank's book at all. That is what stops an arranger backstopping an issue (17.2), what stops a factor bidding for a receivable (17.7b), and it is upstream of every bank being insolvent by period six (21.64). Relaxing the standard-setter's two ratios to a thousandth gives one bank room and the factoring market opens immediately — seven receivables sold in six periods — so the mechanisms behind it work and what is missing is the response to a breach. Positioned at 17.9, whose subject is exactly this: a negative want publishes `mustShed`, and the floor that turns it into a zero is deleted (17.7b).
...
- [ ] 21.63 `register/instruments.ts`, `world/actions.ts redeem` (17.7's census): A ROW REPAID TO THE LAST UNIT STAYS LIVE. Four of the rig's twenty-five loans hold nothing and are owed to nobody — `heldTotal` 0, no holder — and all four are still `status.live`, two of them still `performing: false` from a miss before they were repaid. Nothing reads them (`creditorOf` answers NOBODY and every mechanism skips them, the workout included), but a line that promises nothing to anybody is spent and says it is alive. Positioned at 17.9, where a row leaving the book is the subject (17.7).
- [ ] 21.62 `register/agreements.ts` (17.7): AN AGREEMENT HAS NO CURE. `breached` reaches only `discharged` (paid in full) or `terminated` (written off), so a commitment renegotiated after a missed payment has nowhere to land — the transition 17.7 built for a ROW in the register has no twin for the row beside it. Nothing in this world re-agrees one yet, which is why it was not built with the instrument's (Law 14); the first that will is an arrear a payer is given time on. Positioned at 17.9, beside the write-off, which is the sibling transition (17.7).
