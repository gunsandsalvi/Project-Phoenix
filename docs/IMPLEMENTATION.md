...
- [ ] 12b.1 One employment register (noun `labour.employment`, home `register/employment.ts`): employer, employee cell, hours, wage, since, notice. Delete `labour/register.ts`, `banks/staff.ts`'s `staff` map, the `labour.wages` tally; wages are instructions that READ it; `view.employs()`. **And (12a.5):** with holdings as totals a household cell fails as ONE — every member of a twenty-thousand-household cell goes to probate for a miss that was some of theirs; the per-member miss needs a member with an income of its own, which is this register.
