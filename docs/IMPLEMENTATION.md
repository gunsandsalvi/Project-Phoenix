...
- [x] 15.1 Land: the treasury's ask is its own outlook of what ground fetched; it releases per period what a planning policy (owner `parliament` after 19) says, never everything it holds; a local authority present in each place sells that place's ground. `capital-programme` refuses to start a vintage whose `landPerUnit × units` exceeds free hectares held and pledges the ground to the vintage. Delete `CENT_TICK` asks in `land/index.ts:197,251`. **Done:** a `localAuthority` party kind (`itsOffice`, fails on cash), one per place made by the land module's seed and banked at the place's bank or its country's, holds the place's unbuilt ground; the treasury holds none. Its ask is its own outlook of what ground here has fetched, the last print where it has formed none, and `'market'` where nothing has ever fetched anything (a seller with no opinion names no level); it offers a period what the planning policy says (`land.planning.releasePerPeriod`, POLICY, parliament's, a square kilometre) and never more than it holds. The firm's bid is part of its PROJECT (`registry/capital.ts project`, a `GroundForProject` read from `firms/decide.ts`): the whole hectares the plant it wants would stand on, less what it holds beyond its standing plant, at the RESIDUAL — what a unit of capacity is worth to it over the plant it takes, spread over the hectares a unit of capacity takes — and ground comes first: short of it, the project bids for the hectares this period and for the plant when it holds them. The programme stands plant only on ground the firm holds (`commissionOne` → `groundToCarry`: the pieces its free hectares carry, the rest refused and said, `capital.refused`, the machines staying goods it holds), pledges the ground under a new vintage to the authority of the place in the vintage's name in the same instruction, and releases the lien when the vintage retires. The seed states the plant and the ground under it together: what the opening's plant stands on is its holder's at nothing, the rest the authority's. Both one-cent asks are gone; the names of the ground moved to `registry/land.ts` (`LAND, landId, landMarket, LOCAL_AUTHORITY, authorityIdFor, PLANNING_RELEASE, hectaresOf, hectaresUnder`), and `groundUnder` is the one arithmetic for a vintage held, one to be commissioned and a project's bundle. **Measured** (`land.test.ts`, six green): the authority of each place holds what nobody has built on and every firm holds at least what its plant stands on, at nothing; over a year the authority sells a period at most the release, never everything, only its own ground, and where a hectare has a price a firm paid it; with a hundred hectares and a machine written by hand for one firm and a machine alone for another, the first commissions its vintage with a lien on its ground in the vintage's name for the hectares it takes and the second is refused and still holds the machine, and nobody stands plant on ground it does not hold. In the rig's own draw the first hectare clears in period 2 at a firm's residual bid. `capital.test.ts` and `housing.test.ts` green before and after. Nothing new is red and thirteen are newly green (six of `small-business.test.ts`, six of `equity.test.ts`, one of `funds.test.ts`): eighteen suites at 77 of 196, measured and not diagnosed (Law 11). **Findings:** (a) the ground under the opening's plant is the firm's but NOT pledged to those vintages — the seed cannot write a lien — so an opening vintage's ground can be sold from under it (21.27); (b) machines the programme refuses stay as goods and the project does not count them: it bids for ground and then for machines again (21.28); (c) a pool of small firms holds no ground and its rooms stand on nobody's — the programme refuses nothing of it because the pool commissions through no purchase, and a lease is 15.3's (21.29). The lettings finding from 12.2 (no owner offers a dwelling to let; no rent clears; no household forms) is a tenancy's and moves to 15.5.
- [x] 15.2 A port with an owner and a berth; congestion an outcome of berths. **Done:** a place's quay is its local authority's (`registry/ports.ts portOwnerOf`), and it works so many BERTHS a period — a call being a vessel loading to sail from it or landing at it. The calls a quay has worked this period are read off the ledger's settled voyage legs (`callsAt`: `sail` legs whose first tile is the place, `land` legs whose voyage ends there), never kept; a vessel that has arrived at a quay with every berth worked waits at anchor — first sailed, first alongside — and a cargo that cleared a session at a quay with no berth left does not load, the session's fill going unexecuted (D6: capacity rations quantity at the quay as hulls do at sea). Every call turned away is public with the owner named (`port.congested`). The berth count is a PLACEHOLDER with its death at 22a (`port.berthsPerPlace`, two: a berth is capital the authority builds and wears out, and until it is a vintage of the authority's own it is this number). **Measured** (`freight.test.ts`, three new green): the berths are a placeholder naming their mechanism; the call count reads sails from and landings at a place off a hand-made ledger and ignores a refused instruction; over eight periods no quay works more calls than it has berths, and every congestion said names the authority that owns the quay. **Findings:** (a) the owner earns nothing — port dues want a cleared price, a berth session the carriers bid in, and no berth market exists yet (21.30); (b) three of `freight.test.ts`'s older tests are red at 15.1's HEAD before this step touched the file — `runs every leg every period`, `prints the same grade separately in every place`, `sources locally` — the rig's legs and places have moved under them (21.31).
- [x] 15.3 Commercial property: a building built to LET; a lease as an agreement with a term; a landlord kind (a mass sector: a third lattice profile); CRE lending as a secured row. **Done:** a `property` module. A `landlord` party kind — cells with a lattice (region, bank; size banded on cash a member: the third lattice profile), one cell per bank seeded holding premises (the room a shop or an office is, Capital Programme A2) spread over three vintages at what is left of a new building's price, a building's worth of cash a member, and (15.1's land seed, which now gives cells their ground per member where it comes to whole hectares) the ground under them. A LEASE is a row (`registry/physical.ts LeaseTerms`: region, kind, units, rent a unit a period, until) from a named tenant to a named landlord, signed in a lettings book per place (`lettings.premises:<region>`) where landlords offer what they hold and have not let at their reservation — the rent the book last struck, never below what a unit costs them to hold (the wear) — and a firm whose own plan says its capacity binds on premises bids for the units that lift it to its run rate at what a piece earns it a period (its margin over the premises a unit of output takes); rents are quoted per room on the money's grid because a piece of a room rents for less than a piece of money. A lease is signed in whole units a landlord that pay each a whole piece a period (XI-15: a cell's fill is every member's alike), the rest turned away and said (`property.unlet`); the rent is collected every period as `rent` (a merge that leaves a landlord below a piece is said, not silent); the tenant runs on the leased room as on its own (`rentedRoom` reads its lease rows beside the storage it rents); a lease ends when its day passes or a side ceases. A landlord whose premises are all let builds: it prices a building at the rent the book struck over its horizon at its own cost of capital (XI-4), bids a building a member a period for what its money reaches — ground first, at the residual, as a firm's project does — and asks its bank for what its pace costs over what it holds, SECURED on its premises vintages through the one credit door (Corporate Credit A1), the bank's to answer. **Measured** (`property.test.ts`, three green): the cells open holding premises, the ground under them and cash; a hand-posted tenant bid clears against the landlords' asks, leases are signed in whole units a landlord, the rent moves every period as a `rent` receipt in whole pieces a landlord, the tenant's rented room is the leased units, and a two-period lease has ended by the third period with the room back; a landlord fully let and short of a building a member asks its bank secured on its premises. **Findings:** (a) no firm's plan in the rig binds on premises in thirty periods — every firm is bound by demand or labour — so no tenant bids of its own and the lettings books print `noDemand`; the tenant's reason exists and nothing in this world has it yet (21.32); (b) a bank resolution merges the landlords of the failed bank into another bank's cell, and a lease signed with the smaller cell then pays each landlord less than a piece — said as unpaid, not fixed (21.33); (c) a landlord's build needs a builder selling buildings in its place; where none does, the pace is a bid nobody fills (21.34).
- [x] 15.4 A retail firm sells from a lease. **Done:** a recipe's plant need can be LEASED (`RecipePlantDecl.leased`, carried to `RecipePlant` and `PlantNeed`): a shop's premises are, so a firm's project never builds them (`registry/capital.ts project` filters leased needs out of the bundle it prices) and its bid for room is the lettings book's (15.3). The foundation seeds the landlords of each bank — cells, their premises over three vintages at what is left of a new building's price — BEFORE the firms' plant, and a firm whose recipe leases its room opens holding a LEASE of it from the landlords of its bank instead of the vintages: whole rooms a landlord, rounded up, at the landlords' cost a room a period (a new building over its life, on the money's grid), until the lease term's day, no room let twice (`leaseFromLandlords`); the landlords' opening cash is the property module's seed's to give, after the banks' sheets are built. **Found and fixed on the way — two world stops:** (a) `world/revalue.ts` booked a CELL's marks per member at the TOTAL over its lots — the register holds a cell's total and its equity account is per member (0f.1) — so a landlord cell of two hundred wore its buildings two hundred times a period and was insolvent in five, and every household cell's equity walk left its balance sheet by the weight on every mark; the holdings and foreign-position marks now divide by the weight as `revalueRows` always did (the issuer's side crosses back by the weight and is right now too); (b) 21.19: `securities-lending` marked paper AT THIS PERIOD from a phase before the paper's market ran — `NotYetProduced` stopped the opens rig in period 10 once cells' marks were right — and every one of its five reads marks at the LAST print now (`lastMarkOf`), money at one, a line never printed at nothing a lender can lend against; 21.19 is closed. **Measured** (`property.test.ts`, four green): a shop opens owning no premises but a lease of them from the landlords of its bank in whole rooms a landlord, runs on the leased room, and pays its rent from the first period; the rig's three shops lease 400, 200 and 200 rooms at 159 a room a period and every rent is paid over twelve periods while the landlords live. (c) `banks.prime` priced a broker's line off every published default (`seenDefaults`, C1.b) as every other line the module writes does and had not declared the read; the research rig threw `Clearing F1.a` at the first default that ran before it on the changed path, and the phase declares it now. **Measured, the eighteen suites:** 73 red of 200, from 76 — four `funds.test.ts` tests green now that a cell's marks are per member (the NAV falls with the book, is a read of the book over the shares, pays the manager out of the holders, shows on the surface); one red that was green, `control.test.ts` *bids for nothing*, a 180 s timeout (21.36). **Findings:** the `accounts` family's cell finding is only half closed — household cells' equity walks still leave their balance sheets by the weight at the SETTLEMENT side (a wage paid to a cell as a total moves its per-member account by the total), which the equity probe shows from period 1 (21.35); a winding estate breeds arrears on arrears (21.36).
- [x] 15.5 Tenancy: **Finding (12.2, moved from 15.1):** the scale model's lettings venue prints `noSupply` in eleven periods of twelve and `noDemand` in the other — no owner offers a dwelling to let, so no rent has ever cleared there, no household can form (12.2) and no tenancy is signed. **Done:** the finding's cause was that NO DWELLING EXISTED — forty thousand households and a dwelling line nobody held (`holdersOf` empty in every period). The rental stock has dwellings behind it now (Housing A3): the housing seed gives each landlord member (15.3) so many whole dwellings of its place at the last print (`housing.seed.dwellingsPerLandlord`, a PLACEHOLDER with its death at 22a), and a landlord is in the lettings book as an owner with nobody of its own to house — the same `ordersOf` a household answers with, no kind of its own (a cell whose key has no cohort houses nobody, Law 15). `TenancyTerms.until` (`housing.tenancyTermPeriods`, a convention of the contract): `collect` ends a tenancy whose day has passed and a tenant still short bids again in the next session. A LANDLORD ENDS a tenancy on its own view of the tenant: the expectations module now observes, for every money leg that pays what was owed under a standing commitment (`ledger/instruction.ts paysWhatWasOwed`: wage, rent, interest, dividend, tax, claim, pension, contribution — never a trade), the share of what fell due that arrived, as `credit.<payer>` of the party owed it (Expectations A2, Banks Lending A2: the one belief a party holds about another, formed from the instructions the two were sides of); `collect` compares the rent struck at that view with the wear a period of occupation puts on the roof and ends the tenancy when the first is below the second (`housing.tenancy.ended`, two of its own reads, no threshold). `letIn` signs at the solver's own `outcome.price` (Law 19), walks its fills, and carries the last rent struck onto a print that struck none. Every order, tenancy and rent is in the book's unit — occupancy (`DWELLING_WEEKS`) — converted once from the register's dwellings (`occupancyOf`/`dwellingsOf`); `housing.shortfall` publishes the register's pieces for the buyer of the thing. `mortgagesOf` and `foreclose` compare the security's instrument to the roof of the borrower's place by name and the lien's `secures` to the loan by equality (no `startsWith`/`includes`), and `foreclose` reads `instruments.ofKind(LOAN)` — an index by kind added to the register (`Instruments.byKind`, Law 18). The rent leg carries `receipt: rent`. **Two stops found and fixed where they were:** (e) `occupancyOf`/`dwellingsOf` divided before multiplying and published a short of 2,112,800,000.0000002 pieces — `asQty` refused it (Law 8); the whole counts are multiplied first now, exactly; (f) `estate/index.ts close`: a write-off leg that did not settle — the holder's units of the claim were bound to somebody else — was walked past and the estate ceased with the claim standing, so its next maturity addressed a ceased party (`Money E4`, the institutional-allocation rig at period 13); the estate records `estate.residual` and stays open, as it does with paper nobody bought. **Measured** (`housing.test.ts`, eleven green, four new): every landlord opens holding dwellings and every dwelling has a live holder (E1); tenancies are signed at the level the book printed in the period of signing, run to a day, and the rent moves tenant to landlord as rent every period after; with a term of two a tenancy ends on its day and the same tenant is re-let; a tenant drained of its money fails three rents, is in probate the same period (XI-3), the landlord's view of it falls from 1 to 0.62 in two periods, and it ends the three tenancies when the rent at that view (0.62 × 0.0032 a piece) is below the wear (0.0022) — `housing.tenancy.ended`, by the landlord, saying both numbers. In the rig's own draw the book clears in period 2 at 3,240 a dwelling a period for 7,996 dwellings (three landlord cells, two household cells), lets the landlords' whole stock by period 5 and prints `noSupply` with bids standing after — a shortage, which is the world's state. `mortgage.test.ts` buys the roof its row stands on by hand now (21.39) and the CRE test's drain runs just before the landlords next ask, since their dwelling rents refill them every period. **Findings:** (a) a household that RENTS never buys — `short = needs − owned − rented` is zero once it is housed, `homeBid` never fires, and owner-occupation cannot arise from a renting household; Housing B1's buyer needs the reason a tenant has to own rather than rent (21.39); (b) a singleton's rent is its whole income — the tenant's reservation is what it expects to earn over the roof it lives under, and with `marginalBid` pricing a session with one rich bidder clears at that (period 3 of the rig: 3.117 a piece, 3.1m a dwelling a period, for 1.2 dwellings) (21.38); (c) the dwelling good is subdivided a million ways like a tonne while its own declaration says a whole thing has no piece below one of itself (21.37); (d) a household cell that holds a roof is lent to on its request and drawn on its line at the bank's discretion, so a by-hand default can no longer be staged in `mortgage.test.ts` — *when it cannot pay* is red and says so (21.40). **The eighteen suites:** 73 red of 200, from 73 — *bids for nothing* green again, and `treasury.test.ts` *funds itself over a year when the market is there* red: eleven failed instructions in a year of the rig (21.41).
- [x] 15.6 A region with people and no dwelling line is a seed finding reported at the seal. **Done:** the housing module contributes a `names` family (`roofs`, Housing A1, E1, Seed D1): every region with living household cells and no `good.dwelling.<region>` line is a violation owned by the region, sized in the people who live there — the goods are placed where firms were (`settled(placed)`) and the people where the banks are, so a region with a bank and no firm opened with households and no roof and nothing said so. The seal runs the audit at period 0, so it is reported there. And E1's silent half: a dwelling whose holder has ceased is a house without an owner. The family never repairs. **Measured** (`housing.test.ts`, twelve green): at the seal the family is built, every peopled region either has its line or is named, and asked about a world with no dwelling line anywhere the check names every peopled region with its people.
