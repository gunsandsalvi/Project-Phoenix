...
- [ ] 12.4a **Inserted (12.4's dependency, Law 10): a named firm can be BORN.** Today a named `FIRM` is a `FirmDecl` the firms module draws at the seed and indexes once (`indexOf(rows)`), and its three drawn numbers are params declared at assembly (`firm.hurdle.<firm>`, `firm.horizon.<firm>`, `firms.labourScale.<firm>`) in a register that is built once and never grows. So nothing can be promoted INTO. Three bounded changes: (i) the parameter register accepts a declaration after the seal through a kernel door (`ctx.declare(decl)`, journaled `param.declared`), because XI-14 is about declaration and not about timing; (ii) the firms module's rows are a store that grows (a NOUN, `firms.register`, homed at this item) and `lineOf` reads it; (iii) `bearFirm(ctx, decl, owner, pieces)`: the party enters (`ctx.enter`, named), its share line is issued to its owner, its numbers are drawn from `FIRM_SPREAD` under its own name and declared, and the pieces it arrives with are moved by the promotion event (12.4). Test: a firm born mid-run decides, produces and is quoted like a seeded one.
- [ ] 12.4 Promotion fires in a 52-period run (0f.8); the promoted member's pieces become a named `FIRM`'s holdings. **Measured (12.1–12.3, a 52-period year of the scale model):** no small-firm cell crosses a size edge in the year; 23 of 72 surviving firms sit in the top size band, where the seed put them. So what would be promoted exists and nothing can receive it: 12.4a first.
...
- [ ] 21.1 `households/decide` → `ordersFrom` throws on its own malformed row; one `wholeOrderOf(spend, price)` in `clearing/` for firms and households. **And (11.2a.2):** `PlannedOrder` declared three times (`registry/capital.ts`, `firms/decide.ts`, `households/index.ts`); the registry's is the one.
