...
**Steps closed and deleted: 58** — 21.118 (the ladder's statistic) at 0g.17; and fifteen at *21, the
...
- [ ] 21.137 **RAISED at 21.47–21.85: seventeen findings and five coverage rows were positioned into
  ...
  traded (21j). **And the check this wants is a check**: `plan:check` should refuse a *positioned at*
  naming an item that is closed, the way `check:spec` refuses a citation that resolves to nothing —
  otherwise the next closing item drops the next finding and nothing says so. That is the repair, and
  it is `tools/`, so it goes with 21.119's fixture work, which is the next thing to touch that file.
