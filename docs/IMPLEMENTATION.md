...
- [ ] 17.7a The covered bond; factoring on §42 receivables (with **21.61**: Trade Credit A3 says the early-payment discount is an implicit rate and *"without it there is no rate, and no factoring market can exist"* — so the seller's own days and its own discount are the same item as factoring); senior notes and CP as repo collateral. Split out of 17.7 and INSERTED HERE, immediately after it and before 17.8: each is an instrument shape that needs the terms door 17.7 built and nothing later. **The index-linked schedule left 17.7 too and is inserted at 18a.1**, where the first published price level in this world is — a schedule that pays off an index cannot be written before somebody publishes one.
...
- [ ] 21.61 `trade-credit/index.ts` `tradeCredit.days` (17.5, listed there and not built): the item says *"the days are the seller's preference"* and they are still one declared number for the whole world — thirty days, a technology, the same for a mill and a corner shop. B5 says a seller DECIDES its terms, so the days belong to the seller, drawn like its hurdle and its horizon. What stopped it: a seller in this world is a firm, a small-business cell or a merchant, and the firms module draws numbers for firms only — so the preference needs a home every seller has, or a read that answers nothing for a seller that has none, and "nothing" cannot mean "no terms" without deleting the tier that lives on them (§36 A4, 11.0b). Positioned at 17.7a, where factoring lands: A3 makes the early-payment discount the implicit rate a receivable is sold at, so the seller’s own days and its own discount are the same item as the factoring market (17.5, 17.7).
