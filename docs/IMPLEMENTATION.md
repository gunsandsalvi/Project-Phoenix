...
- [x] **13.5c DONE.** §29 A2.a in both halves, and 13.5 had built only the first rung of the ladder.
  - **It holds liquidity against a call**: what its last call TOOK comes off what it can put to work,
    which is its own experience and the same read the claim buffer already is (A4.c). It is **never
    the undrawn commitment** — money set aside against a commitment in full is money already paid,
    and the whole of A2 is that capital is committed and not paid. Nothing here reads a commitment's
    size, and no ratio is declared anywhere.
  - **And it sells if it must.** 13.5 built the obligation and the DEFAULT: the call goes to the wire
    for the whole amount and a refusal is a recorded state (the fifth silent FORBID). What was
    missing is the rest of A2.b's sentence — *"from its own liquidity ladder, selling if it must"*.
    An investor that defaults and does nothing about it defaults again next time. It asks for its
    money back out of the pools it is in, at NO price (XI-2), through the same redemption channel a
    household short of its own cushion uses — one of it in this world, not one per holder (Law 4).
  - **The pool that called it is the one it cannot redeem from.** A closed-end fund has no
    redemption; the money was committed for its life. So the trap A2.a describes is real and arrives
    by itself: it has to sell something ELSE, and the refusal on the closed one is recorded.
  - **The lag is the clause.** A call arrives and settles in one pass and the response is the next
    period — *"in a stress the calls and its own troubles arrive together"*, from inside.
  - A2.a is a spec NOTE rather than a requirement, so `coverage-existence` reports its MET row beside
