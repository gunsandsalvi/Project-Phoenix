...
- [ ] **`13b.1-2`: two publications of one bank's book are compared, and they are not the same moment.** `tradingBookIsCapitalised` holds `bank.capital`'s `weighted` against a re-walk of `bank.dealing`'s `lines`, and the whole book comes out 2^-19 SHORT of the dealing excess inside it — which is impossible unless the two events are about different instants. They are written in different phases and the bank traded in between; an exact negative power of two is a few pieces of one line at that line's mark. The derived dust is right to refuse it (Law 7). The fix is in the check, but the right comparison needs each publication to name the moment it is about, and what a bank publishes and when is §48's — so it is measured here with the rest of the bank book, and the family stays as it is until then, because what it reports is true and only its conclusion is wrong
- [ ] **`13b.1-6`: a mean-preserving spread moves what the household sector decides, by nineteen pieces a member.** Two worlds seeded with the same total money reaching the same people, spread differently between cells: the mean of what the sector then decides should be the same in both to within ONE piece per member, which is arithmetic and not a band (moving the same total between cells moves which member holds which cent, and that is all it can move). It is not A2.f failing — the same test still shows that no cell decided at the mean and that the decisions are many. It is the SUM of a nonlinear decision under a spread, which is what `Σ f(xᵢ)·wᵢ` is supposed to do, but only when a cell crosses a threshold — and the test's own comment says this world has none its cells straddle. So either a threshold appeared with a redraw and that comment is stale, or a household's plan is not homogeneous of degree one in what it holds when it should be. Which of the two, is the measurement
- [ ] **`13b.1-10`, the part that is not the central bank's**: eight modules post a quantity with no level (funds, the tracker, estates, subordinated debt, the dealing desk, spot FX, firms, households). Clearing A2.a says what that is — "a market expressed as 'here is the quantity I want' has no level, only a shape, and forces every venue to invent its own rule" — and `resolveMarketOrders` is that invented rule, now stated where it lives. Each poster needs the level at which its OWN reason stops, which is a mechanism per module; read them together here, because what they have in common is a participant that answered "how much" without answering "and if it were cheaper?"
- [ ] **`13b.1-11`: an index publishes a level and then cannot answer for it.** `indices.test.ts` reads an id off the module's own published `indices.level` event and `w.index(id)` comes back Missing. E1/E2 say the publication is an OBSERVATION of a rule and never a stored level, so the two are one rule asked twice and must agree. Likely a constituent that stopped answering between the publication and the read — a de-listing, or a line whose print was not there yet, which is a phase-order question (Clearing F1.a) — in which case "the index says nothing" is the honest answer and "something published a number this period" is not. If instead the two are asking DIFFERENT rules, it is Law 4 and belongs where the publication is written
- [ ] **`13b.1-12`: only some of the three deposit classes ever moves bank.** A1.a/A1.b/A1.c give three classes three different reasons to leave — a household for the rate it is not paid, a firm off a bank that drew the window, a fund off one whose session refused it — and `deposits.test.ts` asserts all three fire, because A1.d's point is that a bank's funding is not one substance. At least one never fires. A reason that CANNOT fire is a mechanism finding, a door wired to a condition nothing produces; a reason that could and did not in one seed's run is a draw. Establish which before anything is changed
