...
| Chain declaration | Producer | Legal observation | Decision | Settled state | Consumer | Lag | Falsification |
|---|---|---|---|---|---|---|---|
| Credit tightening → cost of capital → investment → output | lenders tighten admitted credit | loan offers and cleared funding prices | firms choose projects against the observed cost | investment process and later produced batches | goods buyers and accounts read output | the declared build process completion | Hold the tightening absent while keeping prior state; the investment and lagged-output response must disappear. |
| Pension rate move → liability, hedge and cash | the cleared rate market moves | the pension observes the rate print and its own liability duration | the pension submits its hedge instruction | the liability mark, hedge position and cash settlement are separately recorded | the pension funding read consumes all three | hedge settlement is current; liability cash follows its schedule | Hold the rate print fixed; none of the three places may move from this cause. |
| Low inventories → backwardation | physical settlement changes inventory | traders observe owned inventory and dated futures books | buyers and sellers post by delivery date | near and deferred contracts clear separately | inventory holders read the printed curve | delivery dates provide the lag | Keep inventory unchanged; the claimed inventory-driven curve change must disappear. |
| Sovereign issuance → clearing price → debt cost | the treasury offers the issued quantity | bidders observe the auction and their funding | bidders submit quantity-price schedules | the auction settles bonds and prints price | the treasury reads the print for later debt service | coupon cash follows the issued bond schedule | Keep offered issuance unchanged; the issuance-size response must disappear. |
| Fiscal balance ↔ private net saving | named fiscal payers and recipients originate flows | both sides observe only their own legal obligations | treasury and private parties submit payments | the wire settles both legs in one currency | sector-account readers reconcile the named parties | same settlement period | Remove one named fiscal instruction; exactly the flow invariant must identify its missing leg. |
| Commodity shock → margins → inflation → policy | commodity books print the shock | firms observe input prints and households observe sale prints | firms price from costs; the polity reacts to published observations | input purchases, goods sales and policy actions settle in that order | firms, households and policy institutions consume successive prints | one stage between each link | Hold the commodity print fixed; downstream movement attributed to this chain must disappear. |
| One defect → one invariant family | the defective state transition originates the discrepancy | independent audit families read their own sources | no participant decision is permitted in an audit | the period closes before all families run | the diagnostic consumes the family reports | same period close | Inject one pure-function discrepancy; exactly one family classifier must claim it. |
| Hedged firm → smaller shock | the underlying market prints the shock | the firm observes its owned hedge contract | the firm chose the hedge before the shock | underlying cash and hedge cash settle separately | the firm account consumes both positions | hedge predates shock; cash settles by contract | Set hedge quantity to zero; the difference between hedged and unhedged cash response must disappear. |
| Swap spread and credit basis differ by state | rate, bond and credit-protection books produce prints | dealers observe distinct cleared books | dealers submit separate schedules to each book | each contract and cash leg settles on its own wire instruction | risk holders consume the distinct marks | contractual payment dates | Hold stress observations fixed; the calm-to-stress difference must disappear in both measures. |
| Futures converge through delivery | spot and dated delivery books produce prices | holders observe deliverable title and carriage cost | holders choose delivery or offset | delivery transfers the underlying against payment | contract holders consume delivery or final cash | expiry date | Make the underlying non-deliverable; convergence must no longer be attributed to delivery. |
| Rate rise → mortgage cash and house prices → consumption | the rate market prints the rise | borrowers observe reset cash dues and housing bidders observe prices | households separately choose payments, housing bids and consumption | mortgage cash, house title and consumption settle separately | lenders, sellers and goods firms consume the flows | reset and price channels retain distinct declared lags | Hold the rate print fixed; both attributed consumption responses must disappear. |
| Bank run → information about peers | depositors at one bank submit withdrawals | other depositors legally observe published failures and prices only | each depositor revises its own outlook before acting | withdrawals settle bank by bank | peer banks and their depositors consume observed actions | at least one observation period | Hide the first bank’s public outcome; peer withdrawals attributed to it must disappear. |
| Fund loss → bank → other funds | one fund realizes a named loss | its lender observes the contractual payment outcome | the bank and exposed funds decide from their own positions | each loss, margin call and sale settles to a named holder | the next named counterparty consumes the settled state | one settlement or observation period per hop | Remove the initial loss; every downstream event carrying its cause must disappear. |
| Downgrade → sale → capital pressure → funding loss → state | the second-opinion process publishes the downgrade | mandated holders and funders observe that publication | holders sell and funders choose terms | sales, capital changes and funding flows settle separately | the borrower and its next assessment consume the resulting state | reassessment follows settled consequences | Hold the published grade fixed; downgrade-caused sales and the feedback must disappear. |
| Surprised parties act first → others later | each party compares its own outlook with observation | only surprised parties observe their own forecast error | they update outlooks and submit actions | their orders clear and settle before others can observe them | other parties consume public prints and actions | one or more observation periods | Equalise forecast errors; the ordering between first and later responders must disappear. |
| Downturn → transfers/receipts → government → spending | employment and income settlements produce the downturn | fiscal institutions observe named receipts and statutory transfer claims | voters decide from their own histories; government forms under the mandate | taxes, transfers, election result and appropriations are recorded in sequence | households and suppliers consume the later spending | automatic flows are current; election and budget span a term | Hold employment and income fixed; the attributed fiscal, electoral and spending sequence must disappear. |
