...
| `Labour D3` | MET | packages/engine/src/mechanisms/labour/matching.ts (whole people are matched from a queue of seekers; hours that do not make a person are no hire); 12b.4: the hours the book cleared that do not make a whole person are journaled `labour.unsold` as a `remainder`, never dropped |
| `Labour D4` | MISSING |  |
| `Labour D5` | MISSING |  |
