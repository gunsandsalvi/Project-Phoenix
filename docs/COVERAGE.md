...
| requirement | status | where / why |
|---|---|---|
| `Small-Business Pools A1` | PARTIAL | packages/engine/src/mechanisms/small-business/profile.ts (11.0a: it MAKES its line out of its members' hours and the inputs it holds and SELLS it in the region's book — a party kind whose objective is `theResidual`, which fails on cash and solvency and borrows). Still to arrive: it employs beyond its members (11.0c), it buys on terms (11.0b), it borrows and defaults (11.0e) |
...
| `Small-Business Pools A3` | PARTIAL | packages/engine/src/mechanisms/small-business/index.ts (0f.9: SIZE is what a firm opened with — its drawn multiple of `smallBusiness.opening.cash`, a placeholder that dies at 12.1 — held in the register and read as the cell's size band; SECTOR and REGION are its key). LEVERAGE and COVERAGE are outcomes of loans this sector does not have yet: 11.0e |
| `Small-Business Pools A4` | PARTIAL | packages/engine/src/mechanisms/small-business/profile.ts, packages/engine/src/mechanisms/trade-credit/index.ts (11.0a–b: it buys its inputs from and sells its line to the rest of the world through the same books, and it is shipped on terms and ships on terms by the same judgement a named firm is — `buysOnTerms` on the kind, `termsOffered` for the seller). Employing people is 11.0c |
