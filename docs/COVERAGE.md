...
| `Audit B5` | MET | packages/engine/src/audit/families/accounts.ts (`balanceSheet` is the ONE read: the audit checks it, a report publishes it and the seed states the opening account as it), packages/engine/src/ledger/settlement.ts, packages/engine/src/register/register.ts, packages/engine/src/world/revalue.ts, packages/engine/test/balance-identity.test.ts (every party, a year, four moneys that are not the same size); 15.4: a cell's marks are booked per member at the total over its lots divided by the weight (`world/revalue.ts`), which is half of what this family found on cells — the settlement half is 21.35 |
...
| `Treasury C1` | MET | packages/engine/src/mechanisms/treasury/index.ts; 14.3: an indemnity paid on a claim is a receipt class of its own (`claim`) and nobody's income |
...
| `Capital Programme A4` | MET | packages/engine/src/mechanisms/capital-programme/data.ts (a registry of kinds, each with its own unit, its own good and its own life), packages/engine/src/mechanisms/capital-programme/capacity.ts (a use that needs several is limited by the scarcest of them); 14.2: the survival relation is the registry's (`survivesWind`), read by the weather that scraps plant and by the firm that insures against it — one writer |
...
| `Labour A4` | MET | packages/engine/src/register/employment.ts (a relationship with a firm, a worker, a wage and a start date); 12b.1: the register has ONE home — `register/employment.ts`: the kind, its terms (employer, worker cell, hours, wage, since, notice, headcount) and every read over the rows, on `ctx.employment` and `view.employs()`; the labour module's private index is gone; 12b.2a (A4.c): a row's headcount and its cell's weight are the same count again — the merged employed cell of every employer's staff is gone with the key that named the job |
...
| `Labour C3` | MET | packages/engine/src/mechanisms/labour/matching.ts `giveNotice`, `endNotices` (12b.2: a cut gives NOTICE — the row says how many are leaving and when, nobody moves, wages run to the end, and the people are separated when it runs out with nothing more owed; the cost of a firing is the notice, `EmploymentTerms.notice`, a term of the job), packages/engine/src/register/employment.ts |
...
| `Housing A5` | PARTIAL | packages/engine/src/mechanisms/goods/index.ts `goods.spoilage`, packages/engine/src/mechanisms/goods/data.ts (15.7: depreciation is the dwelling's own spoilage — `spoilagePerPeriod`, about one per cent a year, units leaving whoever holds it every period (Goods E4) — and it is the real cost the owner's reservation reads (B5). PARTIAL: MAINTENANCE, an outlay that stops the wear, is not built; nobody can spend against the spoilage (21.43)) |
