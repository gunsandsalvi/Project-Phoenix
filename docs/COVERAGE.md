...
| `Equity E3` | PARTIAL | packages/engine/src/mechanisms/equity/data.ts, packages/engine/src/mechanisms/equity/index.ts (item 10f.1: the STATE a delisting ends in is real — a line with no market, which never trades and never prints, and this world opens with most of its firms in it. `EquityDecl.listed` is the whole of the difference between a public company and a private one). The tender that gets there — holders who can refuse, and A5's vote as what a take-private must obtain — is 10f.3 |
