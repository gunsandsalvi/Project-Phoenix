...
| `Equity E3` | PARTIAL | packages/engine/src/mechanisms/equity/float.ts, packages/engine/src/register/instruments.ts (item 10f.2: a line's market is a fact that CHANGES — `Instruments.list` seats one on a line that had none and `delist` takes it away, and `ctx.list` is the one door that opens the book in the same call so the two halves cannot disagree), packages/engine/src/mechanisms/equity/data.ts (item 10f.1: the state a delisting ends in is real, and this world opens with most of its firms in it). **The flotation is built and the take-private is not**: `delist` exists and nothing calls it, because what walks through it is the tender, which is 10f.3 |
...
| `Prime Brokerage D4` | PARTIAL | every link of the chain is a named pair on a public or own-name event — `prime.line`, `prime.call`, `fund.struck`, the print, the next `prime.line` — so the path is traceable party by party by construction. What is not done is the MEASUREMENT that it actually propagates in this world, which is 13.4b with item 23 (Law 11), and which is waiting on the same thing 23.0a is: pools that have gathered anything (`E-22`) |
...
| `Insurers B2` | MET | packages/engine/src/mechanisms/insurers/index.ts (the present value is the schedule discounted at the curve, through a new `curve` read on DerivedReads) — **NEVER REACHED as last measured**, and that measurement predates the phases items 9.5 and 14.0 gave this module; it is due to be re-taken at 23.0 (`E-25`); packages/engine/src/mechanisms/insurers/allocate.ts (item 14.0: an institution puts what it can spare under management rather than investing it — one subscription at the door of one pool, at the same venue a household uses and under the same refusals. What it keeps back is what its OWN last claim cost it (A4.c), never a ratio anybody stated: an insurer sitting on cash is an insurer whose promises are unfunded, which is what this clause is about) |
| `Insurers B2.b` | MET | packages/engine/src/mechanisms/insurers/index.ts (a policy with no schedule is refused at assembly and reported by the names family — there is no cash-balance liability here) — **NEVER REACHED as last measured**, and that measurement predates the phases items 9.5 and 14.0 gave this module; it is due to be re-taken at 23.0 (`E-25`); packages/engine/src/mechanisms/insurers/allocate.ts, packages/engine/src/mechanisms/funds/index.ts (item 14.0: DURATION MATCHING IS THE WHOLE OF THE DECISION — it picks the pool whose published mandate duration is nearest the FURTHEST thing it has promised, and a pool that states no duration cannot be matched and is refused rather than used as a fallback. The promise is a SELECTION over real dates and never a weighted average of them, because an average duration is a number no promise in the book actually has (Appendix B: no decision at an average). A pool publishes its mandate's duration on `fund.struck` for exactly this reader — a fund's mandate is public in the world this models, which is what a prospectus is) |
| `Insurers B3` | MISSING |  |
| `Insurers B4` | MISSING |  |
| `Insurers C1` | MISSING |  |
| `Insurers C2` | MISSING |  |
...
| `Private Equity A5` | PARTIAL | packages/engine/src/mechanisms/funds/data.ts (item 13.5: the mandate is UNLISTED equity — `listed: false`, the read that separates a private company from a public one), packages/engine/src/mechanisms/equity/data.ts (item 10f.1: **and now there is unlisted equity for it to hold** — every firm has a share line and only the ones this world floated have a market, so eleven of every twelve firms in it are private companies with named owners and no price). *“Acquired firms are held in named vehicles”* is still not built: that is 10f.3's, where a buyer takes a company private |
...
| `Private Equity D1` | PARTIAL | packages/engine/src/mechanisms/equity/float.ts (item 10f.2: *“it sells … to the public market”* — the flotation is built, and it is the SAME mechanism a firm's own IPO is, which is the point: a PE exit by listing is an application of it and not a private-equity mechanism). Selling to another fund or to a corporate buyer is the tender, which is 10f.3 |
