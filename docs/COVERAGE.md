...
| `Insurers B3` | PARTIAL | packages/engine/src/mechanisms/expectations/index.ts `exposed`, packages/engine/src/mechanisms/households/index.ts `insureLife` (14.2: the schedule's uncertainty on the BUYER's side — a cell's own outlook of its cohort's mortality, formed from the deaths it observes, is what it prices life cover at; the insurer's side of the uncertainty is 14.4's experience) |
...
| `Insurers D4` | MISSING |  |
| `Insurers D5` | MISSING |  |
| `Insurers E1` | MET | packages/engine/src/register/agreements.ts (14.5: a policy is a row with a named creditor or it is not a row — E1 holds by construction; the `names` family checks that a standing row promises something) |
| `Insurers E2` | MISSING |  |
| `Insurers E3` | MET | packages/engine/src/world/revalue.ts `revalueRows` (14.5: solvency reads the rows at the curve every revaluation; the only thing kept beside a row is the mark it was last brought to, so the change can be booked — never a stored liability value anybody reads) |
| `Insurers E4` | MISSING |  |
