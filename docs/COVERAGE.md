...
| requirement | status | where / why |
|---|---|---|
| `Small-Business Pools A1` | PARTIAL | packages/engine/src/mechanisms/small-business/index.ts (item 11.1–11.4: the sector EXISTS — a party kind whose objective is `theResidual`, the same thing a named firm is for, which fails on cash AND solvency and borrows. *“Small firms are firms”* stated as a kind rather than as a comment). **What it does not yet do is the clause's verbs**: it sells (11.5), employs (11.6) and borrows (11.7), and each is a step that turns one of them on. A module with phases that ran and did nothing would be the “built on paper and dead in the world” state Part 0 exists to find, so it declares none |
| `Small-Business Pools A2` | MET | packages/engine/src/mechanisms/small-business/data.ts (item 11.4: a pool with a DISTRIBUTION — one population per (region, bank, line), cut into `CELLS_PER_KEY` cells, each drawing its own size from the sector's own tail. The dispersion is WITHIN a key and not only across keys, which is what makes a key more than one firm) |
| `Small-Business Pools A2.a` | MET | packages/engine/src/mechanisms/small-business/data.ts, packages/engine/test/small-business.test.ts (item 11.4: *“no representative small firm.”* Nothing about any one cell is written down: what is stated is the WIDTH — a tail, and a FATTER one than the named sector's, because the small tier is where the distance between the biggest member and the median is greatest. The test asserts the dispersion rather than the draw, because a flat draw would satisfy every other clause in §42 and remove the sector's whole credit content without failing anything) |
| `Small-Business Pools A3` | PARTIAL | packages/engine/src/mechanisms/small-business/data.ts, packages/engine/src/mechanisms/small-business/index.ts (item 11.4: SIZE, SECTOR and REGION are observable — drawn per cell and declared in the parameter register under that cell's own name, so a reader can see the distribution rather than a mean). LEVERAGE and COVERAGE are outcomes of loans this sector does not have yet: they arrive with 11.7, and stating them at the seed would be seeding an outcome (Seed E1) |
| `Small-Business Pools A4` | MISSING | they employ people and buy from and sell to larger firms, including on trade credit: 11.5 and 11.6 |
| `Small-Business Pools A5` | PARTIAL | packages/engine/src/mechanisms/small-business/index.ts (item 11.2: `borrows: true` and a `bank` in the key — the relationship A5 is about is a dimension of what a cell IS, and there is no other funding channel anywhere near it: nothing in this sector can reach a bond market, because a bond issue reads `firms.funding` and these publish none). The LOAN is 11.7, and A5.a's *“where a credit tightening bites first and hardest”* is only measurable once they borrow |
| `Small-Business Pools A6` | MET | packages/engine/src/parties/party.ts, packages/engine/src/registry/profiles.ts, packages/engine/src/mechanisms/small-business/index.ts (item 0b: the seed OPENS the sector, 144 cells over (region, bank, line), and the module is declared after the foundation seed because its cells are keyed on a bank the foundation makes; item 11.2: each is a named party with a WEIGHT — an integer count of the firms it is — and the kind is declared where a module that does not own it can still say its name, because A6.c's promotion needs the two kinds to be able to name each other) |
| `Small-Business Pools A6.a` | MET | packages/engine/src/mechanisms/small-business/data.ts, packages/engine/src/registry/kinds.ts (item 0b: `PartyKindProfile.cellKey` — the dimensions belong to the KIND, so a population of firms and a population of people are each cut as they are, which one world-wide list made mutually exclusive), packages/engine/test/small-business.test.ts (item 11.2: *“every relationship that must be named is either a dimension of the cell's key or a register row, never an attribute averaged inside it.”* The key is region, bank and LINE — the third is a data change A6.a explicitly permits and it is forced, because a cell whose members were in different lines would have an averaged cost base, an averaged customer and an averaged labour venue, which is A2.a one level down. The LENDER is not in it and the test says so: a lender is a loan row per (lender, cell), and lifting it into the key is the relationship the model would then be unable to name) |
