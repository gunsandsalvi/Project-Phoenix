...
/// A 32-bit count as an index; every supported target's pointer is at least 32 bits wide.
...
/// An index as a 32-bit count of chunks, units or positions.
pub(crate) fn to_u32(n: usize) -> u32 {
    let Ok(v) = u32::try_from(n) else {
        capacity_exceeded!("32-bit counts", u32::MAX, n);
    };
    v
}

/// A 64-bit value known to be small, as an index.
pub(crate) fn index(v: u64) -> usize {
    let Ok(i) = usize::try_from(v) else {
        capacity_exceeded!("index width", usize::MAX, v);
    };
    i
}

/// An index as a 64-bit count.
