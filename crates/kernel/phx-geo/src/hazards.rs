...
use crate::prims::{
    DROUGHT_COASTAL, DROUGHT_INLAND, DROUGHT_RATE, DROUGHT_SEVERITY_A, DROUGHT_SEVERITY_B, DROUGHT_SPREAD,
    FLOOD_COASTAL, FLOOD_INLAND, FLOOD_RATE, FLOOD_SEVERITY_A, FLOOD_SEVERITY_B, FLOOD_SPREAD, QUAKE_COASTAL,
    QUAKE_INLAND, QUAKE_RATE, QUAKE_SEVERITY_A, QUAKE_SEVERITY_B, QUAKE_SPREAD, STORM_COASTAL, STORM_INLAND,
    STORM_RATE, STORM_SEVERITY_A, STORM_SEVERITY_B, STORM_SPREAD,
};
...
/// inland exposure, coastal exposure, rate, spread and the two severity shapes.
#[derive(Clone, Copy, Debug)]
pub struct HazardSpec {
    pub decl: HazardDecl,
    pub event: EventKindDecl,
    pub tables: [&'static PrimDecl; 6],
}
...
pub const HAZARDS: &[HazardSpec] = &[
    HazardSpec {
        ...
        tables: [&FLOOD_INLAND, &FLOOD_COASTAL, &FLOOD_RATE, &FLOOD_SPREAD, &FLOOD_SEVERITY_A, &FLOOD_SEVERITY_B],
    },
    HazardSpec {
        decl: tile_hazard("GEO.storm", "GEO.storm_rate", "GEO.toml"),
        event: catastrophe("GEO.storm"),
        tables: [&STORM_INLAND, &STORM_COASTAL, &STORM_RATE, &STORM_SPREAD, &STORM_SEVERITY_A, &STORM_SEVERITY_B],
    },
    HazardSpec {
        decl: tile_hazard("GEO.earthquake", "GEO.earthquake_rate", "GEO.toml"),
        event: catastrophe("GEO.earthquake"),
        tables: [&QUAKE_INLAND, &QUAKE_COASTAL, &QUAKE_RATE, &QUAKE_SPREAD, &QUAKE_SEVERITY_A, &QUAKE_SEVERITY_B],
    },
    HazardSpec {
