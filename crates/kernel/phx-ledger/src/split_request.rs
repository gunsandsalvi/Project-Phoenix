...
/// the population turns it into parts, since the ledger names no population type.
#[clause("REP.8", "REP.16")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl IntentDef for SplitRequest {
    ...
    fn encode(&self, out: &mut Vec<u64>) {
        ...
        out.extend([self.holder.get(), u64::from(self.line.get()), side, u64::from(self.count), cause]);
    }
}
...
impl SplitRequest {
    ...
    pub fn decode(words: &[u64]) -> Option<SplitRequest> {
        let [holder, line, side, count, cause] = *words else { return None };
        let side = match side {
            0 => Side::Asset,
            1 => Side::Liability,
            _ => return None,
...
mod tests {
    ...
    fn a_split_request_round_trips() {
        for cause in [SplitCause::Failed, SplitCause::Kink(7)] {
            let r =
                SplitRequest { holder: PartyId::new(41), line: LineId::new(3), side: Side::Liability, count: 2, cause };
