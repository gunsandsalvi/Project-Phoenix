...
export const PLATFORMS: readonly PlatformDecl[] = [
  {
    ...
    positions: [
      ...
      { on: 'treasury.buffer.periods', value: 12, why: 'A deep buffer: a state that cannot pay its transfers because an auction failed has failed the households it was paying.' },
      ...
      { on: 'law.arrears.gracePeriods', value: 8, why: 'A long grace: a household that is late is usually a household that is short, and calling that a breach costs it the roof as well.' },
  ...
  },
  {
    ...
    positions: [
      ...
      { on: 'treasury.buffer.periods', value: 4, why: 'Cash in hand is borrowed money sitting idle, and it is paid for by whoever the interest is taken from.' },
      ...
      { on: 'law.arrears.gracePeriods', value: 2, why: 'A short grace: a contract that cannot be enforced quickly is a contract nobody will sign with somebody who has no record.' },
  ...
  },
  {
    ...
    positions: [
      ...
      { on: 'treasury.buffer.periods', value: 8, why: 'Two months of outlays, which is what the treasury has always held and what it has never needed all of.' },
      ...
      { on: 'law.arrears.gracePeriods', value: 4, why: 'A month. Long enough for a payment to have gone astray, short enough that a creditor knows where it stands.' },
