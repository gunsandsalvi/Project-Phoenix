...
import { dealerIn, listedIn, mergeModules, rigFor, rigSpec } from './rig.js';
...
/**
 * Dealer Desks A1, A3: A DESK THAT HAS A POSITION, which is what four of these tests are about.
 *
 * A dealer's inventory is an OUTCOME — what it has bought and not yet sold — and in this world it
 * never gets one. 11.5 moved every listed line's float off the desks and onto the household cells
 * that save, because the float on a desk raises nothing at the central bank's window and put every
 * bank in every seed under the liquidity standard (`foundation.ts`, item 11.5). That was right, and
 * it left a desk opening with nothing and no way to trade into a position: a saver names no price
 * for a share until the issuer publishes (12d-7), which is period 25 of any run.
 *
...
function opensWithAPosition(): SystemModule {
  return {
    id: 'test.inventory',
    spec: 'Dealer Desks A1 Dealer Desks A3',
    requires: ['equity', 'banks'],
    instrumentKinds: [],
    partyKinds: [],
    curveFamilies: [],
    units: [],
    ...
    families: [],
    seed(ctx) {
      const opening = ctx.prices.latest(LINE, ctx.period);
      if (!opening.some) return;
      const each = downTick(div(ctx.instruments.get(LINE).issued, A_TENTH, 'a desk\'s own book'));
      if (each <= 0) return;
      for (const b of rowsOf()) {
        if (!b.makes.includes('equity.share')) continue;
        ctx.endowUnits(partyId(b.bank), LINE, each, opening.value.price);
      }
    },
  };
}

/** The world these tests measure: the rig, with a desk that has something on its book. */
function dealingWorld(): World {
  const spec = rigSpec('dealing', RIG.banks, RIG.firms);
  return assemble({ ...spec, modules: mergeModules(spec.modules, [opensWithAPosition()]) });
}
  ...
  it('has a finite, enumerable capacity: a dealer without a limit is not a dealer (F1, Clearing B3.a)', () => {
    ...
    for (const d of rowsOf()) {
      ...
      for (const what of ['capitalAtRisk', 'concentration']) {
        const share = w.params.ratio(lineParam(d.bank, DEALING, what));
      ...
      for (const line of [LENDING, DEALING]) {
        const share = w.params.ratio(lineParam(d.bank, line, 'capitalAtRisk'));
        expect(share).toBeGreaterThan(0);
        expect(share).toBeLessThan(1);
      }
    }
...
describe('the limits (Dealer Desks D1, D4, D4.a)', () => {
  it('shrinks the bid to whichever of its limits binds, and says which one did (D4)', () => {
    ...
    // share of the book, so what it comes to in units is that share of the book over what a unit of
    // this line is worth — which is the same arithmetic the quote does.
    //
    // D4's order is deliberate and this has to respect it: the WHOLE BOOK is asked first, so a desk
    // over its aggregate limit reports `book` whatever else is true. The aggregate was ten million
    // written down, which was more than a desk's book while this world opened its desks with
    // nothing; a desk with a position in it is worth more than that, so the book bound and the line
    // never got a say. It is read off the position instead — the desk's book is this line, and its
    // aggregate limit is twice it, so the only limit with nothing left is the concentration.
    const position = held * worth.value;
    const full = quoteFor(
      view,
      LINE,
      stateOf(w, String(BANK_A), {
        bookValue: asCash(position, USD, 'what its book is worth'),
        limitAggregate: asCash(2 * position, USD, 'twice its position'),
    ...
    const noBook = quoteFor(view, LINE, stateOf(w, String(BANK_A), { limitAggregate: asCash(0, USD, 'no book at all') }));
    ...
    const broke = quoteFor(
      ...
      LINE,
      stateOf(w, String(BANK_A), {
        cash: asCash(0, USD, 'no money'),
        limitAggregate: phx(100_000_000),
        bookValue: asCash(0, USD, 'nothing carried'),
      }),
    );
...
describe('what it publishes (Dealer Desks D5)', () => {
  it('shows inventory, the width it quoted and the room it has left, together', () => {
    ...
    // its own limit allows has less than none, and what it does about that is sell. This used to
    // say the world opens its desks holding the whole float of every line they make; 11.5 moved the
    // float onto the cells that save, and what a desk here holds is the position this file stands
    // in for (`opensWithAPosition`).
