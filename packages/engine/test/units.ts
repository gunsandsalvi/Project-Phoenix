 ...
 * a test that wrote a bare `85` would be saying eighty-five cents when it meant eighty-five USD.
...
/** A price in USD for one share, as the state holds a price. */
export const perShare = (price: number): number => (price * MONEY_PIECES) / SHARE_PIECES;

/** A price in USD for one unit of par (or of any money-denominated unit): a ratio, unchanged. */
...
/** A wage in USD for one hour, as the state holds a price: pieces of money per minute. */
