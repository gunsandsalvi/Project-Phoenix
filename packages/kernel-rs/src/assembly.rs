...
impl RunConfig {
    fn validate(self) {
        assert_eq!(
            self.days_per_period, 7,
            "Money G1: the canonical kernel tick is one week"
        );
        assert!(
...
impl World {
    ...
    pub fn with_parameters(config: RunConfig, declare: impl FnOnce(&mut Params)) -> World {
        ...
        World {
            ...
            calendar: crate::calendar::Calendar::new(
                crate::calendar::Day(config.epoch_day),
                config.days_per_period,
            ),
            period: 0,
            says,
        }
    ...
    pub fn step(&mut self, systems: &[&dyn System]) -> Stepped {
        ...
        let today = crate::calendar::Week(i64::from(self.period));
        ...
        let closes = crate::calendar::Week(i64::from(self.period + 1));
    ...
    fn brought(&mut self, what: crate::module::Brings) {
        ...
        if let (Some(coupon), Some(matures)) = (what.coupon, what.matures) {
            let issued_on = crate::calendar::Week(i64::from(self.period));
