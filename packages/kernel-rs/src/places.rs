...
//! registry DATA, so nothing here branches on a class or a party kind (Law 15): an office block, a
...
///
/// Law 19: it is the register's own rows, not a tally maintained beside them. The result is indexed
...
pub fn built_up(parties: &Parties, register: &Register, registry: &Registry) -> Vec<f64> {
    ...
    for row in register.all() {
        ...
        // Appendix A: a line with no footprint is not a structure — it is flour. That is an answer,
...
/// on empty ground.
///
/// `crowds_at` is the standing area at which a build draws TWICE — one TECHNOLOGY primitive (Law 2)
...
/// at in their own books (Law 3). No money number is set anywhere here, which is the whole reason
/// this is a draw and not a price.
///
/// Law 6: unbounded above and below one. It never refuses a build and it never caps a cost — it is
...
mod tests {
    ...
    fn a_place_fills_up_in_area_and_not_in_units() {
        // Law 8: a dwelling and a mill are counted in different things, so what they have in common
    ...
    fn a_structure_is_a_line_with_a_footprint_and_not_a_class() {
        // Law 15: what makes a line a structure is a number in the registry, so an office block, a
