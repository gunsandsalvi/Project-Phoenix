    ...
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        for &pool in ctx.parties().of_kind(kinds::FUND) {
            ...
            for &a in ctx.agreements().of_party(who) {
                ...
                if let crate::stores::AgreementTerms::FundSubscription { shares: held, .. } = ctx.agreements().terms(a) {
                    shares += held;
                }
            }
            let due: f64 = ctx
        ...
        for (pool, holder, shares, paid) in subscribing {
            ...
            ctx.agrees(crate::module::Agrees {
                ...
                one: pool,
                other: holder,
                terms: crate::stores::AgreementTerms::FundSubscription { shares, paid: paid.get() },
                until: None,
            });
        }
    }
