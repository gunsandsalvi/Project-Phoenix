...
impl Engagements {
    ...
    pub fn hired(&mut self, row: Engagement) {
        assert!(
            row.of > 0.0,
            "XI-15: an engagement of nobody is not one; a weight is a count"
        );
        assert!(
            row.wage > 0.0,
            "XI-10: employment at no wage is not employment"
        );
        self.rows.push(row);
    }
...
impl Mechanism for Wages {
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        let mut partial: Vec<(PartyId, std::num::NonZeroU32, crate::stores::AgreementId)> =
            Vec::new();
        for row in ctx.agreements().of_kind(agreed::ENGAGEMENT) {
            ...
            let (Some(wage), Some(heads)) = (terms.first(), terms.get(2)) else {
                continue;
            };
        ...
        for (employer, worker, money, wages) in owed {
            ...
            let Some(wages) = crate::ledger::Units::new(wages) else {
                continue;
            };
            ctx.propose(
                vec![Leg::Money {
...
mod tests {
    ...
    fn the_household_can_be_told_its_earner_lost_a_job() {
        ...
        let sep = e
            .separated(party(1), party(100), Ended::Dismissed, Week(90))
            .unwrap();
    ...
    fn firing_costs_something_and_quitting_does_not() {
        ...
        let fired = e
            ...
            .unwrap();
        let quit = e
            .separated(party(1), party(101), Ended::Quit, Week(90))
            .unwrap();
    ...
    fn a_dead_employers_severance_is_still_owed_and_ranks_in_the_estate() {
        ...
        e.hired(engagement(1, 100, 10.0, 500.0));
        let gone = e
            .separated(party(1), party(100), Ended::EmployerGone, Week(90))
            .unwrap();
    ...
    fn a_separation_from_an_employer_somebody_never_worked_for_is_not_an_event() {
        ...
        e.hired(engagement(1, 100, 10.0, 500.0));
        assert!(e
            .separated(party(2), party(100), Ended::Quit, Week(90))
            .is_none());
    }
    ...
    fn an_offer_above_the_going_rate_fills_more_than_one_below_it() {
        ...
        let high = cleared
            .matches
            .iter()
            .find(|m| m.employer == party(1))
            .unwrap();
        let low = cleared
            .matches
            .iter()
            .find(|m| m.employer == party(2))
            .unwrap();
