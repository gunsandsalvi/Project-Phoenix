...
mod tests {
    ...
    fn kinds_consume_distinct_accumulated_failure_states() {
        use crate::assembly::kinds;
        assert_eq!(
            trigger_for(kinds::FIRM, false, true, true, false, false),
            Some(Trigger::CouldNotPay)
        );
        assert_eq!(
            trigger_for(kinds::FIRM, true, true, false, false, false),
            Some(Trigger::CouldNotPay)
        );
        assert_eq!(
            trigger_for(kinds::BANK, false, true, false, false, false),
            Some(Trigger::CouldNotFundItself)
        );
        assert_eq!(
            trigger_for(kinds::BANK, false, false, true, false, false),
            Some(Trigger::CapitalGone)
        );
        assert_eq!(
            trigger_for(kinds::FUND, false, false, true, false, false),
            Some(Trigger::LiabilitiesExceedAssets)
        );
        assert_eq!(
            trigger_for(kinds::HOUSEHOLD, true, true, true, false, false),
            None
        );
        assert_eq!(
            trigger_for(kinds::HOUSEHOLD, false, false, false, true, false),
            Some(Trigger::Dissolved)
        );
        assert_eq!(
            trigger_for(kinds::TREASURY, false, true, false, false, false),
            Some(Trigger::WillNotOrCannotPay)
        );
        assert_eq!(
            trigger_for(kinds::BANK, false, false, false, false, true),
            Some(Trigger::PastTheWaterfall)
        );
        assert_eq!(
            trigger_for(kinds::CENTRAL_BANK, true, true, true, true, false),
            None
        );
    }
