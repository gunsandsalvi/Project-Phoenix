...
//!
//! **No instantaneous, costless transport** (E1): that collapses every location into one, and with it
//! the location basis, the arbitrage that bounds it and the working capital tied up in transit. Every
//! `Route` has a duration and a price, and `Shipment` carries an owner while it moves (E3).
//!
//! **Demand is DERIVED** (C1): freight exists because somebody is trading goods, **so it is not an
//! independent market** — and a shipper can decline to ship at all, holding the goods or sourcing
//! locally (C2), which is a real decision and not an absence of demand.
//!
//! **Capacity rations quantity, not only price** (D6): a route's fill must be able to turn somebody
//! away. `clearing` returns what did not move, and no price rises far enough to conjure a ship.
//!
//! **The arbitrage that bounds the location basis is SOMEBODY ACTUALLY SHIPPING** (D3.a), with capacity
//! and cost — so `arbitrages` needs a route with room on it and answers `None` without one, and then the
//! basis stands (D5).
//!
//! **No shipment without capacity, and no capacity without a carrier that owns it** (E2, B1).
...
/// B1, B2: **a carrier owns capital — ships, trucks, planes, warehouses, with their own lives — and
/// capacity is FIXED in the short run and expensive and slow to add**, which is why the freight price
/// is extremely inelastic (B2.a).
#[derive(Clone, Copy, Debug)]
pub struct Carrier {
    pub who: PartyId,
    pub on: Route,
    /// E2: no capacity without a carrier that owns it.
    pub units_per_period: f64,
    /// B3: **an operating cost — fuel, labour, and the capital charge.** It will not sail below it.
    pub cost_per_unit: f64,
    /// A1, D4: **the transit time is a real lag between a purchase and a delivery.**
    pub periods_in_transit: u32,
}

impl Carrier {
    /// B4: **a disruption is a REAL REDUCTION IN UNITS MOVED** — not a multiplier on a price.
...
/// A2, C1: **bought by a NAMED shipper from a NAMED carrier, at a price, in a currency** — and the
...
pub struct Booking {
    ...
    pub on: Route,
    pub units: f64,
    /// The most this shipper will pay, which comes from what the move is worth to it (D2).
    pub will_pay: f64,
}
...
pub struct Cleared {
    ...
    /// D1: the price that cleared on this route. `None` where nothing did (Law 3).
    pub price: Option<f64>,
    /// D6: **capacity rations quantity, not only price** — what did not move, because there was no
    /// room for it at any price.
    pub turned_away: Vec<(PartyId, f64)>,
}

/// D1: **it clears per route.** Cheapest carrier first, best-paying shipper first; the marginal pair
...
/// A3, A3.a, E3: **goods in transit are owned by SOMEBODY, not yet where they are going** — a real asset
...
impl Shipment {
    /// A3.a: what it ties up while it moves. A real use of working capital, for as long as the
    /// transit lasts.
...
/// C2: **a shipper can NOT SHIP** — hold the goods, source locally, or not trade at all. A real
...
/// D2: **the freight cost is part of the DELIVERED price of the good**, so it flows into what the buyer
/// actually pays (§37 D4's landed cost).
pub fn delivered(ex_works: f64, freight: f64, duty: f64) -> f64 {
    ex_works + freight + duty
}

/// D3: **freight is the mechanism behind the location basis** — the same commodity priced differently
...
/// D3.a: **the arbitrage that bounds the basis is SOMEBODY ACTUALLY SHIPPING**, with capacity and cost.
...
/// C3: **freight demand equals the volume actually moving between locations, READ from the shipments**
/// — never a separate series (Law 19).
...
mod tests {
    ...
    fn capacity_rations_quantity_and_not_only_price() {
        // D6: a route's fill must be able to turn somebody away, and no price conjures a ship.
    ...
    fn a_shipper_that_will_not_pay_the_cost_does_not_sail() {
        // B3, Law 3: the carrier will not sail below its operating cost, and nothing clears.
    ...
    fn a_disruption_is_a_real_reduction_in_units_moved() {
        // B4: not a multiplier on a price. And with less capacity, the same demand prints dearer.
    ...
    fn goods_in_transit_are_owned_by_somebody_and_tie_up_working_capital() {
        // A3, A3.a, E3: a real asset on a real balance sheet, for as long as the transit lasts.
    ...
    fn a_shipper_can_decline_to_ship_at_all() {
        // C2: hold the goods, source locally, or not trade — real decisions, and the reason freight
    ...
    fn the_freight_is_part_of_the_delivered_price() {
        // D2: it flows into what the buyer actually pays.
    ...
    fn the_basis_is_bounded_by_somebody_actually_shipping_and_stands_when_nobody_can() {
        // D3, D3.a, D5: the arbitrage needs capacity and cost, and without them the gap persists —
    ...
    fn freight_demand_is_read_from_the_shipments_that_actually_move() {
        // C3, Law 19: never a separate series.
        let shipments = [
    ...
    fn transport_is_never_instantaneous_or_costless() {
        // E1: that would collapse every location into one, and with it the basis, the arbitrage and
