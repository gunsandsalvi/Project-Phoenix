    ...
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        for (who, ccy, short) in bringing {
            ...
            ctx.brings(crate::module::Brings {
                ...
                convention: Convention::MoneyMarketWeekly,
                units: short,
...
mod tests {
    ...
    fn bill(price: f64, weeks: i64) -> Paper {
        Paper {
            issuer: party(9),
            face: 100.0,
            price,
    ...
    fn the_discount_is_the_whole_return_and_the_yield_comes_from_the_price() {
        ...
        assert!(
            cheap.yield_on(Convention::MoneyMarketWeekly).unwrap()
                > dear.yield_on(Convention::MoneyMarketWeekly).unwrap()
        );
    }

    #[test]
    fn weekly_conventions_share_the_declared_week_basis() {
        let b = bill(98.0, 90);
        let money_market = b.yield_on(Convention::MoneyMarketWeekly).unwrap();
        let bond_equivalent = b.yield_on(Convention::BondWeekly).unwrap();
        assert_eq!(bond_equivalent, money_market);
    }

    #[test]
    fn a_yield_over_no_time_or_no_price_is_not_a_rate() {
        assert!(bill(98.0, 0)
            .yield_on(Convention::MoneyMarketWeekly)
            .is_none());
        assert!(bill(0.0, 90)
            .yield_on(Convention::MoneyMarketWeekly)
            .is_none());
    ...
    fn the_spread_over_the_bill_is_a_read_of_two_cleared_prices() {
        ...
        assert!(spread_over_bill(&corporate, &govt, Convention::MoneyMarketWeekly).unwrap() > 0.0);
        let unpriced = bill(0.0, 90);
        assert!(spread_over_bill(&unpriced, &govt, Convention::MoneyMarketWeekly).is_none());
    }
