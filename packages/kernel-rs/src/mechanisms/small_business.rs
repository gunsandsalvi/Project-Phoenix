...
//!
//! No representative small firm: default is a THRESHOLD event, and with one average firm it
//! either happens to all of them or to none. The pool is a set of CELLS, each a named party with
//! a weight, and a weight of ONE is a named firm — so the boundary between this sector and corporate
//! credit is a number, not a kind.
//!
//! Defaults are correlated: the same rates, the same demand, the same region hit all of them at
//! once, so the pool's loss is not the sum of independent draws — and that correlation is
//! exactly what makes the senior tranche's loss possible.
//!
//! A cell that outgrows bank-dependence — large enough to reach the bond market — is promoted
//! , which is a weight event and not a relabelling.
//!
//! No pool without underlying loans to named borrowers: a pool whose losses come from a rate
//! rather than from rows is the defect XI-11 and XI-1 both name. `Pool` holds `Loan` rows and `losses`
//! walks them.
//!
//! No tranche without a holder and no risk transfer without a transferee: if the
//! bank's exposure fell, somebody named is carrying it — and often the originating bank keeps the
//! bottom, which means the risk did not leave.
...
/// Every relationship that must be named is a dimension of the key; A6.b: a weight of one is a
/// named firm.
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Cell {
    pub who: PartyId,
    pub weight: f64,
    /// Observable characteristics — size, sector, region, leverage, coverage.
    pub size: f64,
    pub region: RegionId,
    pub leverage: f64,
    pub coverage: f64,
    /// Bank-dependent — too small for the bond market — which is why a credit tightening bites
    /// here first and hardest.
    pub bank_dependent: bool,
}

impl Cell {
    /// A weight of one is a named firm, so the boundary with corporate credit is a number.
    pub fn is_a_named_firm(&self) -> bool {
        self.weight == 1.0
    }

    /// A cell that outgrows bank-dependence is PROMOTED — large enough to reach the bond
    /// market. A weight event, not a relabelling.
    pub fn outgrew(&self, reaches_the_bond_market_at: f64) -> bool {
        self.size >= reaches_the_bond_market_at
    }
}

/// Each is a loan from a NAMED lender with a rate, a term and an amortisation — and B2:
...
/// They default, and the default depends on the individual firm's cash flow — a threshold this
/// cell crosses or does not, never an average.
...
/// Defaults are CORRELATED — the same rates, the same demand, the same region hit all of
...
/// The pool's loss, walked from the rows. A pool whose losses come from a rate rather than
...
/// The claims are tranched by seniority — losses hit the bottom first — and the tranche
...
/// The tranches are held by NAMED holders, and that is where the loss actually lands —
...
/// Losses hit the bottom first, and when the correlation is worse than the tranching
/// assumed the senior tranche takes losses it was not supposed to — which must be emergent from the
/// shock and the boundaries, never scripted.
...
/// No tranche without a holder, and no risk transfer without a transferee. Who actually
...
/// Did the risk actually leave? What the originator still holds of the deal — and if
...
/// It frees bank capital, which lets the bank lend again — so securitisation is a lending
/// channel and not only a risk transfer. What it freed is what actually left.
...
mod tests {
    ...
    fn a_weight_of_one_is_a_named_firm_so_the_boundary_is_a_number() {
        // The boundary between this sector and corporate credit is not a kind.
    ...
    fn keeping_the_bottom_means_the_risk_did_not_leave() {
        // No risk transfer without a transferee — and what the bank freed is what
        // actually left.
        let deal = [
            Tranche { what: InstrumentId::at(1), attaches: 0.0, detaches: 20_000.0, price: Some(0.9) },
            Tranche { what: InstrumentId::at(3), attaches: 50_000.0, detaches: 200_000.0, price: Some(1.0) },
    ...
    fn a_tranche_nobody_holds_takes_its_loss_and_hits_nobody_which_is_a_finding() {
        // The loss does not move to another tranche because this one is unheld; it stands where
    ...
    fn a_pool_whose_losses_do_not_come_from_named_borrowers_cannot_be_built_here() {
        // The rows are the only source of a loss. There is no rate in this module to
        // apply to a balance.
