...
pub struct Cell {
    ...
    pub leverage: f64,
    pub coverage: f64,
    /// Bank-dependent — too small for the bond market — which is why a credit tightening bites here
    /// first and hardest.
...
/// Each is a loan from a NAMED lender with a rate, a term and an amortisation — and B2: often
/// secured on the firm's assets or the owner's house.
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Loan {
    pub lender: PartyId,
...
pub fn defaults(cash_flow: f64, l: &Loan) -> bool {
    let periods = if l.periods_left > 0 {
        l.periods_left
    } else {
        1
    };
