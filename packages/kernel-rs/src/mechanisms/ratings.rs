...
//!
//! A rating is never derived from the price. If it reads the spread it is a restatement of
...
//! be supplied.
//!
//! It is coarse and sticky: a small change in state does not move it, which is what makes a
//! move meaningful and what makes it late. The stickiness is the mechanism, not an approximation of
//! one.
//!
//! It is an opinion, not a fact, and it can be wrong — a rated-safe issuer can fail. No
//! assessment that is always right: if a rating never misprices, the forced sales never surprise
//! anyone and A4 is deleted. `was_wrong` is the read that keeps that measurable.
//!
//! No rating with no consequence. A published letter that no rule refers to is decoration; the
...
//!
//! It is not the only assessment: holders run their own, and one universal rating held by
//! nobody means every participant agrees about credit by construction.

use crate::ids::{InstrumentId, PartyId};

/// An ordinal judgement — an ordering across issuers, which is what makes it usable in a
...
pub struct State {
    ...
    pub age_periods: u32,
    /// And the TREND in them, which is why a deteriorating issuer is rated below a stable one at
    /// the same level.
    pub trend: f64,
}

/// Published by a NAMED assessor, which is a party with its own incentives — and A5.a: it is
...
/// The grade this state implies, and no rating changes for no reason — every move
/// traces to a change in state. Coarse, so a small change does not move it.
pub fn grade_from(s: &State) -> Grade {
    // The bands are the ordinal judgement itself: a POLICY of the assessor, stated here
    // rather than fitted to a target distribution.
...
/// It is sticky — a move happens only when the state has moved far enough to cross a band, and
...
/// A mandate restricts what a fund, insurer or pension may hold, so a downgrade past a
...
/// Everything the downgrade sets off, at once — because a rating with no consequence is
/// decoration and the whole system is C.
...
pub struct Downgraded {
    ...
    /// And reduces how much can be borrowed against the asset — the leg of the loop that a
...
/// A per-ISSUER risk measure closes the loop, with or without a rating table. The haircut reads
...
/// A downgrade causes selling, capital pressure and funding loss; those raise the
...
/// It works the other way too — improvement widens the buyer base and cheapens funding. How
...
impl Grade {
    /// The grade as a term, so a house can STAND behind it (`standing::GRADE`). A grade is
...
/// The distribution of ratings across issuers is a READ of their states, never a target
...
mod tests {
    ...
    fn a_rating_reads_observable_state_and_there_is_no_price_to_read() {
        // If it read the spread it would be a restatement of the market and could not
    ...
    fn the_loop_is_traceable_step_by_step() {
        // The downgrade raises the cost of funds, which worsens the observable state,
    ...
    fn a_rated_safe_issuer_can_fail() {
        // No assessment that is always right. If a rating never misprices, C1's forced sales
    ...
    fn an_instruments_rating_differs_from_its_issuers_and_both_exist() {
        // The probability of failing to perform, and SEPARATELY the loss given that failure,
    ...
    fn a_rating_is_an_opinion_held_by_a_named_assessor_and_is_not_the_only_one() {
        // One universal rating held by nobody means every participant agrees about
