...
pub fn reassess(held: &Rating, now: &State, by: &Scale, week: u32) -> Option<Rating> {
...
impl Mechanism for Grading {
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        let scale = Scale {
            ...
            record_after: ctx.params().weeks(self.record_after) as u32,
        };
        ...
        for (&who, &(income, before)) in &last {
            ...
            let state = State {
                ...
                size: holds,
                age_periods: ctx.parties().age(of, ctx.week()),
