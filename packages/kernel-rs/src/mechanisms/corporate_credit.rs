...
impl Mechanism for Brings {
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        for p in 0..ctx.parties().len() {
            ...
            // Receipts read as nothing until a corporate's customers pay it on its own lines.
            let short = crate::module::must_raise(owes, cash, buffer);
            if short <= 0.0 {
                continue;
            }
...
mod tests {
    ...
    fn a_draw_taps_the_line_at_the_margin_it_was_struck_at() {
        // A draw does not mint a facility per week, and a line with no room is not a draw.
        let line = Commitment {
            lender: party(70),
            borrower: party(9),
            limit: 1_000.0,
