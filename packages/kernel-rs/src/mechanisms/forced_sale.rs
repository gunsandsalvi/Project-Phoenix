...
//!
//! And a forced seller names no price: the order carries a size and no
...
/// The order a forced seller posts. A size, and NO level — it is struck at
...
/// The price move reaches somebody else. Every other holder of the line is marked at what
...
mod tests {
    ...
    fn a_position_short_of_the_call_sells_what_it_has_and_the_gap_is_real() {
        // Not a cap. It cannot sell units it does not hold, and the shortfall is a state
    ...
    fn the_price_move_reaches_holders_who_did_nothing() {
        // The sale moves the price and the price move reaches somebody else. A bystander
