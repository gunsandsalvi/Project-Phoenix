...
/// Both sides are marked every week — when the borrowed security rises, the borrower posts more
    ...
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        ...
        for (lender, borrower, what, units, fee) in struck {
            ...
            ctx.agrees(crate::module::Agrees {
                ...
                one: lender,
                other: borrower,
                terms: crate::stores::AgreementTerms::SecuritiesLoan {
                    instrument: what,
                    units,
                    fee,
                },
...
mod tests {
    ...
    fn a_rise_in_the_borrowed_security_moves_real_money_between_two_named_parties() {
        // Both sides are marked every week, and the flow has two ends.
        let l = loan(false);
