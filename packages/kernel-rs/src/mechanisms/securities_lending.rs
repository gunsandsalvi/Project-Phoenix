...
//! registered holder — the borrower — and the borrower passes it on (A3). Without it the property is
//! inverted and **the lender pays a fee to lose its income**, so `manufactured` is what the borrower
//! owes and it is a real flow between two named parties (Law 5).
//!
//! **No short without a borrow** (E1): a negative position nobody lent is an invented security. The
//! lendable pool is a read of who actually holds the paper and is willing, and **it caps how large a
//! short can get** (B4) — which is arithmetic about a finite quantity, not a bound anybody chose.
//!
//! **No collateral that is not held** (C4): posted collateral leaves the poster's free balance and
...
//! a single default reaches parties that never traded with the defaulter (C5).
//!
//! **No free borrow** (E3): a fee of zero is a cleared price only if somebody posted it, so `Loan`
//! carries the fee that cleared and `clearing` is where one comes from.

use crate::ids::{InstrumentId, PartyId};

/// A1, A2: the lender delivers the security and the borrower delivers collateral, **and legal title
/// passes** — the borrower can sell what it borrowed, which is the entire point.
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Loan {
    pub lender: PartyId,
    pub borrower: PartyId,
    /// B3: an agent may sit in the middle and take part of the fee.
    pub agent: Option<PartyId>,
    pub what: InstrumentId,
    pub units: f64,
    /// A5: the fee, and **it is a price** that cleared (A5.a, E3).
    pub fee: f64,
    /// C1: what was posted, **worth more than the loan**.
    pub collateral: Collateral,
}

/// C1, C3: the collateral. Cash or other securities — and cash collateral is REINVESTED by the
/// lender, which is a position with its own risk.
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Collateral {
    pub value: f64,
    pub is_cash: bool,
    /// C4: **posted collateral leaves the poster's free balance.** It cannot be counted as available
    /// by both sides, so the encumbrance is on the row.
    pub encumbered_to: PartyId,
}

impl Loan {
    /// C1: **the collateral is worth more than the loan** — a haircut — because the lender must be
    ...
    /// A5, A5.b: the fee, or — where the collateral is cash — the same number seen from the other
    /// side, as a REBATE on that cash. Two forms, one price (Law 4).
...
/// A3: **the economics stay with the lender.** The issuer pays the registered holder — the borrower —
...
/// B2.a, B4: **the lendable pool is a read of who actually holds the security and is willing**, within
...
pub struct Willing {
    pub holder: PartyId,
    pub holds: f64,
    /// B2.a: its own limit on how much of that it will lend. Its own, and it may be none.
    pub will_lend: f64,
}
...
/// E1, B4: **no short without a borrow**, and the pool **caps how large a short can get** — which is a
...
/// A5.a: **the fee clears — scarce paper is expensive to borrow, abundant paper is cheap.** E3: a fee
...
/// C2, C2.a: **both sides are marked every period** — when the borrowed security rises, the borrower
...
/// C3: **cash collateral is reinvested, and this is where a lending programme actually loses money.**
...
/// C5: **re-pledging means the same security backs a CHAIN of obligations, and the chain must be
...
/// D1: **the borrower can fail to return, and then the lender keeps the collateral and buys the
...
impl FailedReturn {
    /// XI-1: the loss is what it cost less what the collateral covered, and it lands on the lender.
    pub fn loss(&self) -> f64 {
        self.bought_back_for - self.kept
    }
}

/// D2, D2.a: **a squeeze — shorts must buy, the lendable pool is small, the fee and the price both
...
/// D3: **a recall forces the borrower to find the security elsewhere or close its short.** Both are
...
/// E2: **no double-counting the loaned security.** The lender's economic exposure and the borrower's
...
mod tests {
    ...
    fn title_passes_and_the_economics_stay_with_the_lender() {
        // A2, A3, E2: two reads of ONE security. Without the manufactured payment the defining
    ...
    fn the_lendable_pool_is_a_read_of_who_holds_it_and_is_willing() {
        // B2.a, B4: within a mandate, with a limit — and a holder willing to lend more than it holds
    ...
    fn there_is_no_short_without_a_borrow_and_the_pool_caps_how_large_one_can_get() {
        // E1, B4: a negative position nobody lent is an invented security, and the limit here is
    ...
    fn scarce_paper_is_expensive_to_borrow_and_a_free_borrow_needs_somebody_to_post_it() {
        // A5.a, E3: the fee clears. Demand that reaches the dearer lender prints the dearer fee.
    ...
    fn the_fee_and_the_rebate_are_one_price_seen_from_two_sides() {
        // A5.b: when the collateral is cash the price is expressed as a rebate on that cash.
        let cash = loan(true);
    ...
    fn the_collateral_is_worth_more_than_the_loan() {
        // C1: collateral exactly equal to the loan, re-marked to the same price, means the gap
    ...
    fn a_rise_in_the_borrowed_security_moves_real_money_between_two_named_parties() {
        // C2, C2.a: both sides are marked every period, and the flow has two ends.
        let l = loan(false);
    ...
    fn posted_collateral_is_encumbered_to_one_party_and_not_available_to_both() {
        // C4: it leaves the poster's free balance.
        let l = loan(true);
    ...
    fn reinvested_cash_collateral_is_where_a_lending_programme_loses_money() {
        // C3: a position with its own risk, held by the lender — not a balance that sits still.
    ...
    fn a_re_pledge_chain_reaches_parties_that_never_traded_with_the_defaulter() {
        // C5: which is the whole reason it must be traceable.
        let pledges = [
    ...
    fn a_failed_return_terminates_and_the_buy_back_costs_whatever_it_costs() {
        // D1: the lender keeps the collateral and buys the security back at the market price.
    ...
    fn a_squeeze_is_measured_from_the_pool_and_never_scripted() {
        // D2, D2.a: a consequence of B4 and C. More shorted against the same pool is tighter.
    ...
    fn a_recall_is_met_elsewhere_or_forces_the_short_to_close() {
        // D3: both are real acts, and which happens depends on whether anybody else will lend.
