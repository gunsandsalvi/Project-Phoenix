...
//!
//! **A call bounded by the investor's spare cash is not an obligation** (A2.b). The investor committed;
...
//!
//! **The debt is the TARGET's liability, not the fund's** (B2.a), **which is why a failed buyout kills
...
//!
//! **The credit market decides which deals happen** (B2.b): the deal only happens if lenders will lend,
//! at a price.
//!
//! **An unlisted mark is not a cleared price and must never be treated as one** (C5.a). `Mark` is a
//! distinct type from a print, and `Exit` produces **the first real price the holding has had** (D2).
//!
//! **The sources and uses of a deal must balance exactly, and the money must come out of named
//! accounts** (B5) — a VERIFY, on derived dust, reporting what did not balance.

use crate::ids::PartyId;

/// A1, A2: **a fund with committed capital from named investors** — committed, **not paid**: it is
/// CALLED when a deal needs it.
#[derive(Clone, Debug)]
pub struct Fund {
    pub who: PartyId,
    /// A3: a manager earning a fee on COMMITTED capital and a share of the gains.
    pub manager: PartyId,
    pub commitments: Vec<Commitment>,
    /// A4: **the fund has a LIFE**: it invests, it holds, it exits, and it winds up.
...
/// A2: the call, pro rata on uncalled commitments. A2.a: **the investor must hold liquidity against
...
/// A2.b: **a call bounded by the investor's spare cash is not an obligation.** The investor committed;
...
/// A5, B1–B4: **the acquired firm is held in a NAMED vehicle, each a party with its own balance
...
pub struct Buyout {
    ...
    pub target: PartyId,
    pub price: f64,
    /// B2, B2.a: **the target's liability, not the fund's.**
    pub debt_on_the_target: f64,
    /// B3: the equity cheque is the rest, funded by the capital call.
    pub equity_cheque: f64,
    pub lender: PartyId,
}

/// B2.b: **the deal only happens if lenders will lend, at a price — the credit market decides which
...
/// B5: **the sources and uses of a deal must balance EXACTLY, and the money must come out of named
...
/// B4: **the target's balance sheet is transformed at the moment of purchase**: leverage up, and the
/// service that comes with it (C1).
...
/// C1: **the firm operates and services its debt out of cash flow, and the higher leverage makes that
...
/// C3: **it can recapitalise — raise more debt to pay itself a distribution** — which is a real cash
/// movement to a named holder (Law 5) and leaves the company with more debt than before.
#[derive(Clone, Copy, PartialEq, Debug)]
...
/// C4, B2.a: **it can fail: the leverage makes default a real outcome, the loss falls on the lenders
/// and the equity is wiped — and the failed buyout kills the COMPANY, not the fund** (XI-3).
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Failure {
...
/// C5, C5.a: **the holding has a value that is not a market price — no clearing, so it is a MARK** —
...
/// D1, D2, D3: **it sells — to another fund, to a corporate buyer, or to the public market — and the
...
/// C5.a, D2: what the mark said against what the exit cleared at. **The first real price the holding
...
mod tests {
    ...
    fn capital_is_committed_not_paid_and_the_call_is_pro_rata_on_what_is_uncalled() {
        // A2, A2.a: the investor must hold liquidity against calls it did not choose the timing of.
    ...
    fn a_call_bounded_by_the_investors_spare_cash_is_not_an_obligation() {
        // A2.b: the investor committed. It must find the money — and failing that it is in DEFAULT on
    ...
    fn the_credit_market_decides_which_deals_happen() {
        // B2.b: the deal only happens if lenders will lend, at a price.
    ...
    fn the_sources_and_uses_balance_exactly() {
        // B5: and the money comes out of named accounts.
    ...
    fn the_balance_sheet_is_transformed_at_the_moment_of_purchase_and_the_service_binds() {
        // B4, C1: leverage up, and coverage can fall below one.
        let b = buy(party(67), party(9), 10_000.0, 7_000.0, party(80), 5_000.0).unwrap();
    ...
    fn a_dividend_recapitalisation_pays_the_owner_and_leaves_more_debt_behind() {
        // C3: a real cash movement to a named holder.
        let b = buy(party(67), party(9), 10_000.0, 7_000.0, party(80), 5_000.0).unwrap();
    ...
    fn an_unlisted_mark_is_not_a_cleared_price_and_the_exit_is_the_first_real_one() {
        // C5, C5.a, D2: a separate type is how that is kept true, and the exit is what tests the mark.
    ...
    fn the_proceeds_are_distributed_in_cash_to_the_investors_that_paid_in() {
        // D3: into their accounts, pro rata on what was actually called.
