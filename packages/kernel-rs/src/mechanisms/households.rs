...
/// What a cell was actually PAID this week, from named payers.
#[derive(Clone, Copy, PartialEq, Debug)]
pub struct Received {
    pub wages: f64,
    pub transfers: f64,
...
impl Participant for HouseholdBuyers {
    ...
    fn orders(&self, view: &ParticipantView<'_>, m: MarketId) -> Vec<Order> {
        ...
        let Some(keeps) = view.household_keeps() else {
            return Vec::new();
        };
        ...
        let affordable =
            weighted_affordable_quantity(money, keeps, limit, view.population_weight(), already);
        if affordable <= 0 {
            return Vec::new();
        }
        vec![Order {
...
mod tests {
    ...
    fn the_same_aggregate_income_produces_different_demand_depending_on_who_has_it() {
        ...
        let sector = |income_many: f64, income_few: f64| {
            let many = Spending {
                income_now: income_many,
                wealth: 0.0,
                expects_to_keep: 1.0,
                can_borrow: 0.0,
            };
            let few = Spending {
                income_now: income_few,
                wealth: 0.0,
                expects_to_keep: 1.0,
                can_borrow: 0.0,
            };
