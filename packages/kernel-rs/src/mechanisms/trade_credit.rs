...
pub fn implied_rate(t: &Terms) -> Option<f64> {
    let d = t.discount?;
    let weeks = t.due.0 - d.if_paid_by.0;
    if weeks <= 0 || d.off <= 0.0 || d.off >= 1.0 {
        return None;
    }
    // What the buyer pays for the extra days, annualised on the calendar's own day count.
    Some(d.off / (1.0 - d.off) / Convention::BondWeekly.year_fraction(d.if_paid_by, t.due))
}
...
pub fn offer(
    ...
    assert!(
        view.of == buyer,
        "36 B5: a view of one buyer does not price another's terms"
    );
    if already_out + amount > view.will_carry {
        return None;
    }
    if view.will_wait_weeks <= 0 {
        return None;
    }
    Some(Terms {
        seller,
        ...
        amount,
        delivered,
        due: Week(delivered.0 + view.will_wait_weeks),
        discount: None,
    })
}
...
pub fn withdraw(view: &View, seller: PartyId, was_carrying: f64, on: Week) -> Withdrawn {
    Withdrawn {
        by: seller,
        from: view.of,
        working_capital_lost: was_carrying,
        on,
    }
}
...
pub fn on_death(t: &Terms, estate_paid: f64) -> Resolved {
    ...
    Resolved {
        creditor: t.seller,
        recovered: estate_paid,
        lost: t.amount - estate_paid,
    }
}
...
pub fn along_the_chain(
    started_at: PartyId,
    owed_to_each: &[Terms],
    survives_a_loss_of: f64,
) -> Vec<(PartyId, f64)> {
    ...
    while let Some(dead) = failing.pop() {
        guard += 1;
        assert!(
            guard <= owed_to_each.len() + 1,
            "36 D3: the chain cannot visit more firms than there are rows"
        );
...
impl Mechanism for TradeCredit {
    fn run(&self, ctx: &mut MechanismContext<'_>) {
        let will_carry = ctx
            .params()
            .amount(self.will_carry, crate::params::Denomination::Money);
        let will_wait = ctx.params().weeks(self.will_wait) as i64;
        let today = ctx.today();
        ...
        for (q, seller, buyer, amount) in offering {
            ...
            let view = View {
                of: buyer,
                will_carry,
                will_wait_weeks: will_wait,
            };
            // `None` is a REFUSAL, and a refusal is a decision.
            let Some(terms) = offer(seller, buyer, amount, today, &view, already) else {
                continue;
            };
            *out.entry((seller.0, buyer.0)).or_insert(0.0) += amount;
        ...
        let mut waiting: std::collections::HashMap<u32, (usize, Week)> =
