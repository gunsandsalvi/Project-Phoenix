...
pub enum Dimension {
    ...
    Periods,
    /// A whole number of weeks on the simulation calendar.
    Weeks,
...
impl Params {
    ...
    pub fn declare(&mut self, d: ParamDecl) {
        ...
        if matches!(
            d.dimension,
            Dimension::Periods | Dimension::Weeks | Dimension::Count
        ) {
            assert!(
                d.value.fract() == 0.0,
...
mod tests {
    ...
    fn a_read_that_names_the_wrong_dimension_says_both() {
        let mut p = params();
        p.declare(decl("loan.term", 84.0, Dimension::Weeks, Kind::Technology));
        assert_eq!(p.weeks("loan.term"), 84.0);
