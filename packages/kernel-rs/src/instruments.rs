...
pub fn yield_to(price: f64, repays: f64, from: Week, to: Week, c: Convention) -> Option<f64> {
    let weeks = to.0 - from.0;
    match price > 0.0 && weeks > 0 {
        true => Some((repays / price - 1.0) * c.year() / (weeks * 7) as f64),
        false => None,
    }
}
...
impl Periodicity {
    ...
    pub fn weeks(self) -> Option<i64> {
        match self {
            Periodicity::Monthly => Some(4),
            Periodicity::Quarterly => Some(13),
            Periodicity::SemiAnnual => Some(26),
            Periodicity::Annual => Some(52),
            Periodicity::AtMaturity => None,
        }
    }
}

/// Convert a civil-labelled tenor at the boundary and retain only its whole-week term.
pub fn weeks_from_months(months: f64) -> i64 {
    assert!(
        months.is_finite() && months > 0.0,
        "a tenor must be positive and finite"
    );
    let weeks = months * 13.0 / 3.0;
    assert!(
        weeks.fract() == 0.0,
        "fractional-week terms are not kernel terms"
    );
    weeks as i64
}
...
pub fn schedule_of(
    ...
    assert_eq!(
        days_per_period, 7,
        "Money G3.b: kernel terms are placed on weekly ticks"
    );
    assert!(
        matures > issued_on,
    ...
    if coupon > 0.0 {
        ...
        if let Some(weeks) = pays.weeks() {
            loop {
                ...
                out.push(Payment {
                    from,
                    due: next,
                    amount: units * coupon * (next.0 - from.0) as f64 * 7.0 / convention.year(),
                    of: Owing::Interest,
                });
                from = next;
            }
        ...
        out.push(Payment {
            from,
            due: matures,
            amount: units * coupon * (matures.0 - from.0) as f64 * 7.0 / convention.year(),
            of: Owing::Interest,
        });
    }
...
pub fn unrealised_difference(
    ...
    Some(
        worth(row, register, instruments, prints, period)?
...
mod tests {
    ...
    fn a_bond_pays_its_coupons_on_its_own_dates_and_its_principal_once() {
        ...
        assert_eq!(coupons[9].due, Week(260));
        // And each covers the interval since the one before it, with no gap and no overlap.
        assert_eq!(coupons[0].from, Week(0));
        for pair in coupons.windows(2) {
            assert_eq!(
                pair[0].due, pair[1].from,
                "one coupon ends where the next begins"
            );
        }

        // Terms are declared as whole weeks, so equal intervals have equal coupon amounts.
        assert_eq!(coupons[0].amount, coupons[1].amount);
        let first_year: f64 = coupons[0].amount + coupons[1].amount;
        assert_eq!(first_year, 1_000.0 * 0.04 * 364.0 / 365.0);
    }
