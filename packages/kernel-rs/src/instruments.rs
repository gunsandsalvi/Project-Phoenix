...
pub fn yield_to(price: f64, repays: f64, from: Week, to: Week, c: Convention) -> Option<f64> {
    ...
    match price > 0.0 && weeks > 0 {
        true => Some((repays / price - 1.0) / c.year_fraction(from, to)),
        false => None,
    }
}
    ...
    if coupon > 0.0 {
        ...
        if let Some(weeks) = pays.weeks() {
            loop {
                ...
                assert!(
                    next.0 - from.0 >= weeks_per_period,
...
mod tests {
    ...
    fn a_bond_pays_its_coupons_on_its_own_dates_and_its_principal_once() {
        ...
        // Each half is an explicit 26-week tenor on the one weekly accrual basis.
        assert_eq!(coupons[0].amount, coupons[1].amount);
        let first_year: f64 = coupons[0].amount + coupons[1].amount;
        assert_eq!(first_year, 40.0);
        assert_eq!(coupons[2].amount + coupons[3].amount, first_year);
    }
    ...
    fn what_pays_at_the_end_pays_once_and_what_pays_nothing_pays_never() {
        ...
        assert_eq!(
            ...
            Payment {
                from: Week(0),
                due: Week(13),
                amount: 1_000.0 * 0.03 * crate::calendar::weekly_year_fraction(13),
                of: Owing::Interest,
            }
        );
    ...
    fn accrued_is_the_coupon_over_the_part_of_its_interval_that_has_passed() {
        let (from, to) = (Week(0), Week(52));
        assert_eq!(accrued(from, to, 20.0, Week(0)), 0.0);
        assert_eq!(accrued(from, to, 20.0, Week(13)), 5.0);
        assert_eq!(accrued(from, to, 20.0, Week(52)), 20.0);
    ...
    fn a_day_outside_the_coupon_is_refused_rather_than_answered_with_the_end_of_it() {
        accrued(Week(0), Week(52), 20.0, Week(70));
    }
